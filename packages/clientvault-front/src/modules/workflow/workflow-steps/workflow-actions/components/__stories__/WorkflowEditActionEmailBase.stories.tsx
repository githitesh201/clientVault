...
import { ComponentDecorator, RouterDecorator } from 'clientvault-ui/testing';
...
const CONFIGURED_SEND_EMAIL_ACTION: WorkflowSendEmailAction = {
  ...
  settings: {
    input: {
      ...
      recipients: {
        to: 'test@clientvault.com',
        cc: '',
        bcc: '',
      },
      subject: 'Welcome to ClientVault!',
      body: 'Dear Tim,\n\nWelcome to ClientVault! We are excited to have you on board.\n\nBest regards,\nThe Team',
      files: [],
    },
    outputSchema: {},
    errorHandlingOptions: {
...
export const Configured: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const emailInput = await canvas.findByText('tim@clientvault.com');
    expect(emailInput).toBeVisible();

    const subjectInput = await canvas.findByText('Welcome to ClientVault!');
