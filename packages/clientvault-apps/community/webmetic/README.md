...
Automatically sync B2B website visitor data into ClientVault CRM. Identify anonymous companies visiting your website and track their engagement without forms or manual entry. Every hour, Webmetic enriches your CRM with actionable sales intelligence about who's researching your product before they ever fill out a contact form.
...
- A ClientVault API key (create one at `https://clientvault.com/settings/api-webhooks` and name it **"Webmetic"**)
...
cd packages/clientvault-apps/webmetic
clientvault app sync
```

### 2. Configure environment variables

- **First, create a ClientVault API key**:
...
- **Then configure the app**:
  ...
  - Provide values for the required keys:
    - `CLIENTVAULT_API_KEY` (required secret; paste the API key you just created)
    - `CLIENTVAULT_API_URL` (optional; defaults to `http://localhost:3000` for local dev, set to your production URL)
...
```
Webmetic API ─[hourly]→ sync-visitor-data ─[create/update]→ ClientVault CRM
                              │
...
|----------|----------|-------------|
| `CLIENTVAULT_API_KEY` | ✅ Yes | Your ClientVault API key for authentication |
| `CLIENTVAULT_API_URL` | ❌ No | Base URL of your ClientVault instance (defaults to `http://localhost:3000`) |
...
This app uses multiple ClientVault CRM APIs:
...
- **Solution**: This occurs if you previously deleted a company. ClientVault maintains unique constraints on soft-deleted records. Either restore the company from trash or contact support.
...
- **ClientVault API**: 800ms delay between requests (75 requests/minute)
...
# Sync to local ClientVault instance
npx clientvault-cli app sync

# Watch for changes
npx clientvault-cli app dev
```

### Manual trigger

Use the ClientVault UI test panel or trigger via API:
...
Built with 🍺 and ❤️ in Munich by [Team Webmetic](https://webmetic.de) for ClientVault CRM Hacktoberfest 2025.

For issues or questions:
- Webmetic API: [webmetic.de](https://webmetic.de)
- ClientVault CRM: [clientvault.com/developers](https://clientvault.com/developers)
