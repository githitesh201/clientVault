...
### Added
- **Meeting ingest utility**: New `yarn meeting:ingest <meetingId>` script to manually fetch and import specific Fireflies meetings into ClientVault
...
- **Documentation**: Added README sections on current clientvault headers forward limitations and utility scripts
...
- **Major refactoring**: Split monolithic `receive-fireflies-notes.ts` into modular architecture:
  ...
  - `clientvault-crm-service.ts` - ClientVault CRM operations (contacts, notes, meetings)
...
- Added TypeScript path mappings for `clientvault-sdk` in workspace configuration
