...
Automatically captures meeting notes with AI-generated summaries and insights from Fireflies.ai into your ClientVault CRM.

### Current Status
- Doesn't work with Fireflies webhook yet due to missing headers forwarding in clientvault serverless func
...
**Fireflies webhook → Fireflies API → ClientVault CRM with summary-focused insights**
...
```bash
# Step 1: Authenticate with ClientVault
npx clientvault-cli auth login

# Step 2: Sync the app to create Meeting object
npx clientvault-cli app sync packages/clientvault-apps/fireflies
...
CLIENTVAULT_API_KEY=your_api_key             # From ClientVault CRM settings
SERVER_URL=https://your-domain.clientvault.com
```
...
```
https://your-clientvault-instance.com/s/webhook/fireflies
```
...
- ClientVault verifies signature using your webhook secret
- Invalid signatures are rejected immediately

### Current Platform Limitation (Headers)

- ClientVault serverless route triggers currently do **not forward HTTP headers** to functions. Fireflies signatures sent in headers are stripped, so header-based verification does not work in production.
...
- Ingest a specific Fireflies meeting into ClientVault:
`yarn meeting:ingest <meetingId>` or `MEETING_ID=... yarn meeting:ingest`

- Fetch all/historical Fireflies meetings into ClientVault:
`yarn meeting:all [--from 2024-01-01] [--to 2024-02-01] [--organizer a@x.com] [--participant b@x.com] [--channel <channelId>] [--mine] [--dry-run]`
  ...
  - Controls:
    - `--dry-run`: list and transform without writing to ClientVault
...
**Integration**: Fireflies webhook → AI processing layer → Enhanced ClientVault records
