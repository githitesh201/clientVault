...
/**
 * Test script for Fireflies webhook against local ClientVault instance
 *
 * Usage:
 *   yarn test:webhook
 *   # or
 ...
 *
 * Prerequisites:
 *   1. ClientVault server running on http://localhost:3000
 *   2. Fireflies app synced: npx clientvault-cli app sync
...
const _mockFirefliesFetch = async (url: string, options?: RequestInit) => {
  ...
  // For ClientVault API calls, use real fetch
  return fetch(url, options);
};

const main = async () => {
  console.log('🧪 Testing Fireflies Webhook Against Local ClientVault Instance\n');
  console.log(`📍 Server URL: ${SERVER_URL}`);
  console.log(`🔑 API Key: ${CLIENTVAULT_API_KEY ? '✅ Configured' : '❌ Missing'}`);
  console.log(`🔐 Webhook Secret: ${FIREFLIES_WEBHOOK_SECRET ? '✅ Configured' : '⚠️  Using test secret'}\n`);

  // Validation
  if (!CLIENTVAULT_API_KEY) {
    console.error('❌ Error: CLIENTVAULT_API_KEY is required');
  ...
  try {
    ...
    console.error('   Make sure ClientVault is running: cd clientvault && yarn dev');
    process.exit(1);
  }
  ...
  if (signatureErrors) {
    ...
    if (response.ok) {
      ...
      console.log('\n📋 Next steps:');
      console.log('   1. Check ClientVault CRM for new Meeting/Note records');
    ...
    console.error('\n💡 Troubleshooting:');
    console.error('   1. Ensure ClientVault server is running');
    console.error('   2. Ensure app is synced: npx clientvault-cli app sync');
