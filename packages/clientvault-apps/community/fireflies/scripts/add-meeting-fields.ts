 ...
 * Run this after: npx clientvault-cli app sync packages/clientvault-apps/fireflies
 *
...
if (!API_KEY) {
  console.error('❌ Error: CLIENTVAULT_API_KEY not found in .env file');
  process.exit(1);
}
...
const MEETING_FIELDS: FieldDefinition[] = [
  // === Internal ClientVault Relations ===
  {
    type: 'RELATION',
    name: 'note',
    label: 'Meeting Note',
...
const findMeetingObject = async () => {
  ...
  if (!meetingEdge) {
    throw new Error('Meeting object not found. Please run "npx clientvault-cli app sync" first.');
  }
