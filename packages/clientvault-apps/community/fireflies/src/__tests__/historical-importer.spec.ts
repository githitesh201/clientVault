...
describe('HistoricalImporter', () => {
  const buildImporter = () => {
    ...
    return { firefliesClient, clientvaultService };
  };

  it('skips meetings that already exist by firefliesMeetingId', async () => {
    const { firefliesClient, clientvaultService } = buildImporter();

    firefliesClient.listTranscripts.mockResolvedValue([{ id: 'existing' }]);
    clientvaultService.findMeetingByFirefliesId.mockResolvedValue({ id: 'clientvault-id' });

    const importer = new HistoricalImporter(firefliesClient, clientvaultService);
    ...
    expect(clientvaultService.createMeeting).not.toHaveBeenCalled();
    expect(result.statuses[0].status).toBe('skipped_existing');
  });

  it('supports dry-run without writing to ClientVault', async () => {
    const { firefliesClient, clientvaultService } = buildImporter();
    ...
    clientvaultService.findMeetingByFirefliesId.mockResolvedValue(undefined);
    ...
    expect(clientvaultService.createMeeting).not.toHaveBeenCalled();
    expect(clientvaultService.createNoteOnly).not.toHaveBeenCalled();
