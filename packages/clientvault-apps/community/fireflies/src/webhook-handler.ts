...
import { ClientVaultCrmService } from './clientvault-crm-service';
...
export class WebhookHandler {
  ...
  async handle(params: unknown, headers?: Record<string, string>): Promise<ProcessResult> {
    ...
    try {
      ...
      const clientvaultApiKey = process.env.CLIENTVAULT_API_KEY || '';
      ...
      if (!clientvaultApiKey) {
        this.logger.critical('CLIENTVAULT_API_KEY not configured - this is a critical configuration error');
        throw new Error('CLIENTVAULT_API_KEY environment variable is required');
      }
      ...
      const clientvaultService = new ClientVaultCrmService(
        clientvaultApiKey,
        getApiUrl()
      );

      const existingMeetingById = await clientvaultService.findMeetingByFirefliesId(meetingData.id);
      ...
      const existingMeetingByTitle = await clientvaultService.findExistingMeeting(meetingData.title);
      ...
      const { matchedContacts, unmatchedParticipants } = await clientvaultService.matchParticipantsToContacts(
      ...
      const newContactIds = autoCreate
        ? await clientvaultService.createContactsForUnmatched(unmatchedParticipants)
        : [];
      ...
      const noteId = await clientvaultService.createNoteOnly(
      ...
      result.meetingId = await clientvaultService.createMeeting(meetingInput);
  ...
  private async linkNoteToParticipants(
    clientvaultService: ClientVaultCrmService,
    noteId: string,
    contactIds: string[]
  ): Promise<void> {
    for (const contactId of contactIds) {
      try {
        await clientvaultService.createNoteTarget(noteId, contactId);
  ...
  private async createFailedMeetingRecord(params: unknown, error: string): Promise<void> {
    try {
      const clientvaultApiKey = process.env.CLIENTVAULT_API_KEY || '';
      if (!clientvaultApiKey) {
        this.logger.debug('Cannot create failed meeting record: CLIENTVAULT_API_KEY not configured');
        return;
      }
      ...
      const clientvaultService = new ClientVaultCrmService(clientvaultApiKey, getApiUrl());
      ...
      const failedMeetingId = await clientvaultService.createFailedMeeting(failedMeetingData);
