...
import { type ClientVaultCrmService } from './clientvault-crm-service';
import type {
  FirefliesPlan,
...
export class HistoricalImporter {
  ...
  private clientvaultService: ClientVaultCrmService;

  constructor(firefliesClient: FirefliesApiClient, clientvaultService: ClientVaultCrmService) {
    this.firefliesClient = firefliesClient;
    this.clientvaultService = clientvaultService;
  }
  ...
  async run(
    ...
    for (const transcript of transcripts) {
      ...
      try {
        const existing = await this.clientvaultService.findMeetingByFirefliesId(meetingId);
        if (existing) {
          logger.debug(`Skipping ${meetingId}: already exists in ClientVault (${existing.id})`);
          skippedExisting += 1;
          statuses.push({
            meetingId,
        ...
        const { matchedContacts, unmatchedParticipants } =
          await this.clientvaultService.matchParticipantsToContacts(
            meetingData.participants,
          );

        const newContactIds = autoCreateContacts
          ? await this.clientvaultService.createContactsForUnmatched(unmatchedParticipants)
          : [];
        const allContactIds = [...matchedContacts.map(({ id }) => id), ...newContactIds];

        const noteBody = MeetingFormatter.formatNoteBody(meetingData);
        const noteId = await this.clientvaultService.createNoteOnly(
          `Meeting: ${meetingData.title}`,
          noteBody,
        );

        const meetingInput = MeetingFormatter.toMeetingCreateInput(meetingData, noteId);
        const createdMeetingId = await this.clientvaultService.createMeeting(meetingInput);

        for (const contactId of allContactIds) {
          await this.clientvaultService.createNoteTarget(noteId, contactId);
        }
