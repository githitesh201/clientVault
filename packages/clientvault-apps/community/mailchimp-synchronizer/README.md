...
Synchronizing contacts between ClientVault and Mailchimp
...
```bash
cd packages/clientvault-apps/community/mailchimp-synchronizer
yarn auth
yarn generate
yarn sync
```
...
- Check if person already exists in Mailchimp:
  - if yes, check if UPDATE_PERSON is set to true
    - if UPDATE_PERSON is true, check if ClientVault record is the same as Mailchimp record
