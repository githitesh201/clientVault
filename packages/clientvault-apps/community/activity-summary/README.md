# ClientVault CRM Activity Reporter ��

A TypeScript-based reporting bot that summarizes activity from your ClientVault CRM workspace and sends daily/periodic reports to Slack, Discord, and WhatsApp. Meet Kylian Mbaguette, your friendly CRM activity reporter!
...
- TypeScript
- A [ClientVault CRM](https://clientvault.com) account with API access
...
|----------|----------|-------------|
| `CLIENTVAULT_API_KEY` | ✅ Yes | Your ClientVault CRM API key |
...
1. **Data Collection**: The bot queries the ClientVault CRM API for activities within the specified time range
2. **Analysis**:
...
This bot uses the [ClientVault CRM REST API](https://api.clientvault.com/rest/). The following endpoints are used:
...
- **Solution**: Verify your `CLIENTVAULT_API_KEY` is correct and has necessary permissions
