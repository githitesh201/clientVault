...
const create_last_interaction = (id: string) => {
  return {
    method: 'POST',
    url: `${CLIENTVAULT_URL}/metadata/fields`,
    headers: {
...
const updateInteractionStatus = async (objectName: string, id: string, messageDate: string, status: string) => {
  const options = {
    method: 'PATCH',
    url: `${CLIENTVAULT_URL}/${objectName}/${id}`,
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${CLIENTVAULT_API_KEY}`,
    },
...
export const main = async (params: {
  ...
  const options = {
    method: 'GET',
    url: `${CLIENTVAULT_URL}/metadata/objects`,
    headers: {
      Authorization: `Bearer ${CLIENTVAULT_API_KEY}`,
    },
  };
  try {
    ...
    const messageOptions = {
      method: 'GET',
      url: `${CLIENTVAULT_URL}/messages/${recordId}?depth=1`,
      headers: {
        Authorization: `Bearer ${CLIENTVAULT_API_KEY}`,
      },
    };
