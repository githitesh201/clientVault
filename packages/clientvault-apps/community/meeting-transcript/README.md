...
When unstructured meeting notes are received via a **webhook**, the system processes them and creates organized **notes, tasks, and assignments** directly in ClientVault CRM.
...
- [ClientVault CLI](https://www.npmjs.com/package/clientvault-cli) - Install globally: `npm install -g clientvault-cli`
- ClientVault CRM instance with API access
- API key from [Settings > API & Webhooks](https://clientvault.com/settings/api-webhooks)
...
2. **Configure environment variables:**
   ...
   - `CLIENTVAULT_API_KEY`: Generated from your ClientVault CRM instance
   - `CLIENTVAULT_API_URL`: Your ClientVault CRM instance URL (e.g., https://your-instance.clientvault.com)
...
4. **Deploy to your ClientVault workspace:**
   ```bash
...
### 🔗 Webhook Setup (inside ClientVault)

Navigate to:
**ClientVault → Workspace Settings → APIs & Webhook → + New Webhook**
...
| **APIs** | ClientVault CRM REST API + GraphQL (for member resolution) |
...
| `CLIENTVAULT_API_KEY` | Yes | Yes | ClientVault CRM API authentication token |
| `CLIENTVAULT_API_URL` | Yes | No | Base URL for ClientVault CRM instance |
...
The demo below shows the full workflow where a raw meeting transcript is processed and automatically converted into structured **Notes + Tasks** inside ClientVault CRM.
