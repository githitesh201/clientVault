...
type ClientVaultApiResponse = {
  id: string;
};

const AI_PROVIDER_API_KEY = process.env.AI_PROVIDER_API_KEY;
const WEBHOOK_SECRET_TOKEN = process.env.WEBHOOK_SECRET_TOKEN;
const CLIENTVAULT_API_URL = process.env.CLIENTVAULT_API_URL;
...
const getClientVaultApiConfig = () => {
  const apiKey = process.env.CLIENTVAULT_API_KEY;
  if (!apiKey) {
    throw new Error('CLIENTVAULT_API_KEY environment variable is not set');
  }

  const baseUrl = CLIENTVAULT_API_URL;
  if (!baseUrl) {
    throw new Error('CLIENTVAULT_API_URL environment variable is not set');
  }
...
const lookupWorkspaceMemberByName = async (
  ...
  const { apiKey, baseUrl } = getClientVaultApiConfig();
...
const createNoteInClientVault = async (
  summary: string,
  keyPoints: string[],
  relatedPersonId: string,
  meetingTitle?: string,
  meetingDate?: string,
): Promise<ClientVaultApiResponse> => {
  const { apiKey, baseUrl } = getClientVaultApiConfig();
  const noteTitle =
    meetingTitle ||
...
const createTaskInClientVault = async (
...
const createTasksFromActionItems = async (
  ...
  for (const actionItem of actionItems) {
    try {
      ...
      const task = await createTaskInClientVault({
        ...actionItem,
...
export const main = async (
  ...
  try {
    ...
    log('📄 Creating note in ClientVault CRM...');
    const note = await createNoteInClientVault(
      analysis.summary,
      analysis.keyPoints,
      relatedPersonId,
      meetingTitle,
