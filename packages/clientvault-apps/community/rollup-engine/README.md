...
General-purpose rollup engine for ClientVault workspaces. The bundle ships a
...
- an API key with access to your workspace. Generate one at
  `https://clientvault.com/settings/api-webhooks`.
...
- To script the setup, export `CLIENTVAULT_API_KEY` (and `CLIENTVAULT_METADATA_BASE_URL` if you are not targeting `http://localhost:3000/rest/metadata`) and run `yarn setup:metadata`. The helper script calls the Metadata API to ensure each field exists on the Company object, skipping anything already provisioned.
...
2. **Configure environment variables**
   ...
   - Provide values for the manifest-defined keys:
     - `CLIENTVAULT_API_KEY` (required secret; create it in ClientVault → Settings → API & Webhooks).
     - `CLIENTVAULT_API_BASE_URL` (optional; leave blank to use `https://app.clientvault.com/rest`).
...
- Opportunities expose `companyId`, `amount.amountMicros`, `amount.currencyCode`,
  ...
  config.
- Amount totals are stored in micros (integers) to align with ClientVault’s composite
  currency fields.
...
Opportunity data, and asserts the PATCH payload that would be sent to ClientVault.

## Troubleshooting
- **`Test your function` reports `mainFile.main is not a function`:** delete the app
  (`clientvault app delete`) and sync it again (`clientvault app sync`) to clear any stale bundle.
