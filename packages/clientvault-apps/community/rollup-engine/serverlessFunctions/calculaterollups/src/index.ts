...
import { buildChildRecordIndex, ClientVaultClient } from './client';
...
const getApiCredentials = () => {
  const apiKey =
    process.env.CLIENTVAULT_API_KEY ??
    process.env.CLIENTVAULT_API_TOKEN ??
    process.env.API_KEY;
  ...
  const baseUrlRaw =
    process.env.CLIENTVAULT_API_BASE_URL ??
    process.env.CLIENTVAULT_REST_BASE_URL ??
  ...
  const baseUrl =
    typeof baseUrlRaw === 'string' && baseUrlRaw.trim().length > 0
      ? baseUrlRaw
      : 'https://app.clientvault.com/rest';
...
export const main = async (params: unknown): Promise<any> => {
  ...
  try {
    ...
    if (!credentials) {
      console.warn(
        '[rollup] skipping execution because CLIENTVAULT_API_KEY is not set',
      );
      return { status: 'noop', reason: 'CLIENTVAULT_API_KEY not configured' };
    }

    const { apiKey, baseUrl } = credentials;
    const client = new ClientVaultClient(apiKey, baseUrl);
    const now = new Date();
