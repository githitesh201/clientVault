...
- **Rich Note Creation**: Creates formatted notes in ClientVault with summary and key points
- **Task Generation**: Automatically creates tasks for action items and commitments, linked to the meeting note

## Requirements

- `clientvault-cli` - Install globally: `npm install -g clientvault-cli`
- `apiKey` - Go to `https://clientvault.com/settings/api-webhooks` to generate one
...
2. Edit `.env` and replace the placeholders:
   - `<SET_YOUR_CLIENTVAULT_API>` with your ClientVault API key
...
4. Sync the app to your ClientVault workspace:
...
After syncing, configure the environment variables in your ClientVault workspace:
1. Go to Settings → Apps → AI Meeting Transcript
2. Set the following environment variables:
   - `CLIENTVAULT_API_KEY` - Your ClientVault API key
   - `CLIENTVAULT_API_URL` - Your ClientVault instance URL (e.g., `https://api.clientvault.com` or `http://localhost:3000` for local development)
   - `OPENAI_API_KEY` - Your OpenAI API key

**Important**: `CLIENTVAULT_API_URL` is required and must be set to your ClientVault instance URL. For local development, use `http://localhost:3000`. For production, use your actual ClientVault instance URL.
...
- Hosted: `POST https://your-clientvault-instance.com/s/webhook/transcript`
...
```bash
curl -X POST https://your-clientvault-instance.com/s/webhook/transcript \
...
2. **Note Creation**: A formatted note is created in ClientVault with:
   - Meeting summary
   - Key discussion points
...
2. Configure it to POST to: `https://your-clientvault-instance.com/s/webhook/transcript`
