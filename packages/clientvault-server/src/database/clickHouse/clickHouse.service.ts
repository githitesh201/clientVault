...
export class ClickHouseService implements OnModuleInit, OnModuleDestroy {
  ...
  constructor(private readonly clientvaultConfigService: ClientVaultConfigService) {
    if (this.clientvaultConfigService.get('CLICKHOUSE_URL')) {
      this.mainClient = createClient({
        url: this.clientvaultConfigService.get('CLICKHOUSE_URL'),
        compression: {
          response: true,
          request: true,
        },
  ...
  private async createAndInitializeClient(
    ...
    const client = createClient({
      url: url ?? this.clientvaultConfigService.get('CLICKHOUSE_URL'),
      compression: {
        response: true,
        request: true,
      },
