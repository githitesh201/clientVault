...
export class MigrateWorkspacePicturesCommand extends ActiveOrSuspendedWorkspacesMigrationCommandRunner {
  ...
  private async migrateWorkspaceLogo({
    ...
    const isClientVaultIconLogo = workspace.logo.includes('clientvault-icons');

    if (!isClientVaultIconLogo && !isInWorkspaceLogo) {
      this.logger.log(
        `Workspace logo is not a clientvault icon or a workspace logo, skipping`,
      );
    ...
    if (isClientVaultIconLogo) {
      await this.migrateWorkspaceLogoFromClientVaultIcons({
        workspaceId,
  ...
  private async migrateWorkspaceLogoFromClientVaultIcons({
    workspaceId,
    logoUrl,
    isDryRun,
    workspaceCustomFlatApplication,
    ...
    try {
      ...
      this.logger.log(
        `Migrated workspace logo from clientvault-icons for workspace ${workspaceId} (${logoUrl} -> ${FileFolder.CorePicture}/${newResourcePath})`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to migrate workspace logo from clientvault-icons for workspace ${workspaceId}: ${error.message}`,
      );
    }
  }
