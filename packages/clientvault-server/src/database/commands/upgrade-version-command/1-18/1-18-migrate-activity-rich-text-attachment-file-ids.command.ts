...
export class MigrateActivityRichTextAttachmentFileIdsCommand extends ActiveOrSuspendedWorkspacesMigrationCommandRunner {
  ...
  override async runOnWorkspace({
    ...
    await this.clientvaultORMGlobalManager.executeInWorkspaceContext(async () => {
      const attachmentRepository =
        await this.clientvaultORMGlobalManager.getRepository<AttachmentWorkspaceEntity>(
          workspaceId,
          'attachment',
      ...
      const noteRepository =
        await this.clientvaultORMGlobalManager.getRepository<NoteWorkspaceEntity>(
          workspaceId,
          'note',
          { shouldBypassPermissionChecks: true },
        );
      ...
      const taskRepository =
        await this.clientvaultORMGlobalManager.getRepository<TaskWorkspaceEntity>(
          workspaceId,
          'task',
          { shouldBypassPermissionChecks: true },
        );
      ...
      await this.migrateActivityTable({
        ...
        workspaceId,
        clientvaultStandardApplication: clientvaultStandardFlatApplication,
  ...
  private async migrateActivityTable({
    ...
    targetColumnName,
    workspaceId,
    clientvaultStandardApplication,
    fileFieldMetadata,
    isDryRun,
  }: {
    activityRepository:
    ...
    workspaceId: string;
    clientvaultStandardApplication: { id: string; universalIdentifier: string };
  ...
  private async createAttachmentFromUrl(
    ...
    try {
      ...
      await this.fileStorageService.copyLegacy({
        ...
        to: {
          folderPath: `${workspaceId}/${clientvaultStandardApplication.universalIdentifier}`,
      ...
      const fileEntity = fileRepository.create({
        ...
        workspaceId,
        applicationId: clientvaultStandardApplication.id,
        size: -1,
        settings: {
