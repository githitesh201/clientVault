...
export class BackfillStandardViewsAndFieldMetadataCommand extends ActiveOrSuspendedWorkspacesMigrationCommandRunner {
  ...
  override async runOnWorkspace({
    ...
    try {
      ...
      // Part 2: Resolve applicationId for clientvault-standard app
      const applicationRows = await queryRunner.query(
        `SELECT id FROM core."application"
         WHERE "workspaceId" = $1
           AND "universalIdentifier" = $2`,
        [workspaceId, CLIENTVAULT_STANDARD_APPLICATION.universalIdentifier],
      );

      if (applicationRows.length === 0) {
        this.logger.warn(
          `ClientVault Standard Application not found for workspace ${workspaceId}. Skipping view backfill.`,
        );
