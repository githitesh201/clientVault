...
import { buildSystemAuthContext } from 'src/engine/clientvault-orm/utils/build-system-auth-context.util';
import { WorkspaceCacheService } from 'src/engine/workspace-cache/services/workspace-cache.service';
import { STANDARD_NAVIGATION_MENU_ITEMS } from 'src/engine/workspace-manager/clientvault-standard-application/constants/standard-navigation-menu-item.constant';
...
export class MigrateFavoritesToNavigationMenuItemsCommand extends ActiveOrSuspendedWorkspacesMigrationCommandRunner {
  ...
  constructor(
    ...
    protected readonly clientvaultORMGlobalManager: GlobalWorkspaceOrmManager,
    ...
    super(workspaceRepository, clientvaultORMGlobalManager, dataSourceService);
  }
  ...
  override async runOnWorkspace({
    ...
    const { clientvaultStandardFlatApplication, workspaceCustomFlatApplication } =
      await this.applicationService.findWorkspaceClientVaultStandardAndCustomApplicationOrThrow(
        {
          workspaceId,
        },
      );
    ...
    const { flatNavigationMenuItemsToCreate: favoriteNavItemsToCreate } =
      await this.migrateFavorites({
        workspaceId,
        clientvaultStandardApplicationId: clientvaultStandardFlatApplication.id,
        clientvaultStandardApplicationUniversalIdentifier:
          clientvaultStandardFlatApplication.universalIdentifier,
        workspaceCustomApplicationId: workspaceCustomFlatApplication.id,
        workspaceCustomApplicationUniversalIdentifier:
          workspaceCustomFlatApplication.universalIdentifier,
        flatObjectMetadataMaps,
    ...
    if (allFlatNavigationMenuItemsToCreate.length > 0) {
      ...
      const clientvaultStandardItems = allFlatNavigationMenuItemsToCreate.filter(
        (item) => item.applicationId === clientvaultStandardFlatApplication.id,
      );
      ...
      if (clientvaultStandardItems.length > 0) {
        await this.createNavigationMenuItems({
          workspaceId,
          flatNavigationMenuItemsToCreate: clientvaultStandardItems,
          existingFlatNavigationMenuItemMaps,
          applicationUniversalIdentifier:
            clientvaultStandardFlatApplication.universalIdentifier,
        });
  ...
  private async migrateFavoriteFolders({
    ...
    const favoriteFolderRepository =
      await this.clientvaultORMGlobalManager.getRepository<FavoriteFolderWorkspaceEntity>(
        workspaceId,
        'favoriteFolder',
  ...
  private async migrateFavorites({
    ...
    clientvaultStandardApplicationUniversalIdentifier: string;
    workspaceCustomApplicationId: string;
    workspaceCustomApplicationUniversalIdentifier: string;
    ...
    const favoriteRepository =
      await this.clientvaultORMGlobalManager.getRepository<FavoriteWorkspaceEntity>(
        workspaceId,
        'favorite',
        { shouldBypassPermissionChecks: true },
      );
  ...
  private async getUserWorkspaceIdFromWorkspaceMemberId(
    ...
    try {
      const workspaceMember =
        await this.clientvaultORMGlobalManager.executeInWorkspaceContext(
          async () => {
            const workspaceMemberRepository =
              await this.clientvaultORMGlobalManager.getRepository<WorkspaceMemberWorkspaceEntity>(
                workspaceId,
                'workspaceMember',
                { shouldBypassPermissionChecks: true },
              );
