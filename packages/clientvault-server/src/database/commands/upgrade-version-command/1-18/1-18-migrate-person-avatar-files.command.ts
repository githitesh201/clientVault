...
import { FieldMetadataType, FileFolder } from 'clientvault-shared/types';
import {
...
export class MigratePersonAvatarFilesCommand extends ActiveOrSuspendedWorkspacesMigrationCommandRunner {
  ...
  override async runOnWorkspace({
    ...
    if (!isDefined(clientvaultStandardFlatApplication)) {
      this.logger.error(
        `ClientVault standard application not found for workspace ${workspaceId}`,
      );
        ...
        if (isDefined(existingFieldWithSameName)) {
          ...
          try {
            await this.fieldMetadataService.updateOneField({
              ...
              workspaceId,
              ownerFlatApplication: clientvaultStandardFlatApplication,
            });
          } catch (error) {
            this.logger.error(
    ...
    await this.clientvaultORMGlobalManager.executeInWorkspaceContext(async () => {
      const personRepository =
        await this.clientvaultORMGlobalManager.getRepository<PersonWorkspaceEntity>(
          workspaceId,
          'person',
        ...
        try {
          ...
          if (!isDryRun) {
            await this.fileStorageService.copyLegacy({
              ...
              to: {
                folderPath: `${workspaceId}/${clientvaultStandardFlatApplication.universalIdentifier}`,
            ...
            const fileEntity = fileRepository.create({
              ...
              workspaceId,
              applicationId: clientvaultStandardFlatApplication.id,
              size: -1,
              settings: {
