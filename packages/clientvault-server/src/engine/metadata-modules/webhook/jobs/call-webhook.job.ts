...
import { getAbsoluteUrl } from 'clientvault-shared/utils';
...
export class CallWebhookJob {
  ...
  private async callWebhook(data: WebhookJobData): Promise<void> {
    ...
    try {
      ...
      if (secret) {
        headers['X-ClientVault-Webhook-Timestamp'] = Date.now().toString();
        headers['X-ClientVault-Webhook-Signature'] = this.generateSignature(
          payloadWithoutSecret,
          secret,
          headers['X-ClientVault-Webhook-Timestamp'],
        );
        headers['X-ClientVault-Webhook-Nonce'] = crypto
          .randomBytes(16)
          .toString('hex');
      }
