...
export class AiModelRegistryService {
  ...
  constructor(private clientvaultConfigService: ClientVaultConfigService) {
  ...
  private buildModelRegistry(): void {
    ...
    const openaiApiKey = this.clientvaultConfigService.get('OPENAI_API_KEY');

    if (openaiApiKey) {
      this.registerOpenAIModels();
    }

    const anthropicApiKey = this.clientvaultConfigService.get('ANTHROPIC_API_KEY');

    if (anthropicApiKey) {
      this.registerAnthropicModels();
    }

    const xaiApiKey = this.clientvaultConfigService.get('XAI_API_KEY');

    if (xaiApiKey) {
      this.registerXaiModels();
    }

    const groqApiKey = this.clientvaultConfigService.get('GROQ_API_KEY');

    if (groqApiKey) {
      this.registerGroqModels();
    }

    const openaiCompatibleBaseUrl = this.clientvaultConfigService.get(
      'OPENAI_COMPATIBLE_BASE_URL',
    );
    const openaiCompatibleModelNames = this.clientvaultConfigService.get(
  ...
  private registerOpenAICompatibleModels(
    ...
    const apiKey = this.clientvaultConfigService.get('OPENAI_COMPATIBLE_API_KEY');
  ...
  getDefaultSpeedModel(): RegisteredAIModel {
    const defaultModelIds = this.clientvaultConfigService.get(
  ...
  async validateApiKey(provider: ModelProvider): Promise<void> {
    ...
    switch (provider) {
      case ModelProvider.OPENAI:
        apiKey = this.clientvaultConfigService.get('OPENAI_API_KEY');
        break;
      case ModelProvider.ANTHROPIC:
        apiKey = this.clientvaultConfigService.get('ANTHROPIC_API_KEY');
        break;
      case ModelProvider.XAI:
        apiKey = this.clientvaultConfigService.get('XAI_API_KEY');
        break;
      case ModelProvider.GROQ:
        apiKey = this.clientvaultConfigService.get('GROQ_API_KEY');
        break;
      case ModelProvider.OPENAI_COMPATIBLE:
        apiKey = this.clientvaultConfigService.get('OPENAI_COMPATIBLE_API_KEY');
        break;
      default:
        return;
    }
