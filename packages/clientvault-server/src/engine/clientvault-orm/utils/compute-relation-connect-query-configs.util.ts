...
import { getUniqueConstraintsFields, isDefined } from 'clientvault-shared/utils';
...
import { type ConnectObject } from 'src/engine/clientvault-orm/entity-manager/types/query-deep-partial-entity-with-nested-relation-fields.type';
import {
  type RelationConnectQueryConfig,
  type UniqueConstraintCondition,
} from 'src/engine/clientvault-orm/entity-manager/types/relation-connect-query-config.type';
import { type RelationConnectQueryFieldsByEntityIndex } from 'src/engine/clientvault-orm/entity-manager/types/relation-nested-query-fields-by-entity-index.type';
import {
  ClientVaultORMException,
  ClientVaultORMExceptionCode,
} from 'src/engine/clientvault-orm/exceptions/clientvault-orm.exception';
import { formatCompositeField } from 'src/engine/clientvault-orm/utils/format-data.util';
...
const computeRecordToConnectCondition = (
  ...
  if (
    ...
    throw new ClientVaultORMException(
      ...
      ClientVaultORMExceptionCode.CONNECT_NOT_ALLOWED,
      {
...
const checkUniqueConstraintFullyPopulated = (
  ...
  if (!hasUniqueConstraintFieldFullyPopulated) {
    throw new ClientVaultORMException(
      ...
      ClientVaultORMExceptionCode.CONNECT_UNIQUE_CONSTRAINT_ERROR,
      {
