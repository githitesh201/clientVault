import { type ObjectRecordUpdateEvent } from 'clientvault-shared/database-events';
...
describe('formatClientVaultOrmEventToDatabaseBatchEvent', () => {
  ...
  describe('UPDATED action', () => {
    it('should throw ClientVaultORMException when no matching before entity is found in array of beforeEntities', () => {
      const afterEntities = [
        {
          id: 'record-1',
      ...
      try {
        ...
        expect(error).toBeInstanceOf(ClientVaultORMException);
        expect((error as ClientVaultORMException).code).toBe(
          ClientVaultORMExceptionCode.ORM_EVENT_DATA_CORRUPTED,
        );
        expect((error as ClientVaultORMException).message).toBe(
    ...
    it('should successfully create update events when matching before entities are found and in the right order', () => {
      ...
      const result = formatClientVaultOrmEventToDatabaseBatchEvent({
        action: DatabaseEventAction.UPDATED,
        objectMetadataItem: flatObjectMetadata,
        flatFieldMetadataMaps,
        workspaceId: mockWorkspaceId,
