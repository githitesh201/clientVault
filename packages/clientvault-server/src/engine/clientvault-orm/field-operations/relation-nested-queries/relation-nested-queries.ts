...
import { RELATION_NESTED_QUERY_KEYWORDS } from 'clientvault-shared/constants';
import {
  type EntityTarget,
  type ObjectLiteral,
...
} from 'src/engine/clientvault-orm/entity-manager/types/relation-nested-query-fields-by-entity-index.type';
import {
  ClientVaultORMException,
  ClientVaultORMExceptionCode,
} from 'src/engine/clientvault-orm/exceptions/clientvault-orm.exception';
import { formatConnectRecordNotFoundErrorMessage } from 'src/engine/clientvault-orm/field-operations/relation-nested-queries/utils/formatConnectRecordNotFoundErrorMessage.util';
import { type WorkspaceSelectQueryBuilder } from 'src/engine/clientvault-orm/repository/workspace-select-query-builder';
import { computeRelationConnectQueryConfigs } from 'src/engine/clientvault-orm/utils/compute-relation-connect-query-configs.util';
import { createSqlWhereTupleInClause } from 'src/engine/clientvault-orm/utils/create-sql-where-tuple-in-clause.utils';
import { extractNestedRelationFieldsByEntityIndex } from 'src/engine/clientvault-orm/utils/extract-nested-relation-fields-by-entity-index.util';
import { getAssociatedRelationFieldName } from 'src/engine/clientvault-orm/utils/get-associated-relation-field-name.util';
...
import { getRecordToConnectFields } from 'src/engine/clientvault-orm/utils/get-record-to-connect-fields.util';
        ...
        if (
          ...
          if (recordToConnect.length !== 1) {
            ...
            throw new ClientVaultORMException(
              errorMessage,
              ClientVaultORMExceptionCode.CONNECT_RECORD_NOT_FOUND,
              {
                userFriendlyMessage,
              },
            );
