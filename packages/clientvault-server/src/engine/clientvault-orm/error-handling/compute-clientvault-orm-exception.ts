...
export const computeClientVaultORMException = async (
  error: Error,
  objectMetadata?: FlatObjectMetadata,
  entityManager?: WorkspaceEntityManager,
  internalContext?: WorkspaceInternalContext,
): Promise<Error | ClientVaultORMException> => {
  if (error instanceof QueryFailedError) {
    if (error.message.includes('Query read timeout')) {
      return new ClientVaultORMException(
        'Query read timeout',
        ClientVaultORMExceptionCode.QUERY_READ_TIMEOUT,
        {
    ...
    if (errorCode === POSTGRESQL_ERROR_CODES.INVALID_TEXT_REPRESENTATION) {
      return new ClientVaultORMException(
        ...
        ClientVaultORMExceptionCode.INVALID_INPUT,
      );
    }
