...
import { belongsToClientVaultStandardApp } from 'src/engine/metadata-modules/utils/belongs-to-clientvault-standard-app.util';
...
export class FlatSkillValidatorService {
  ...
  >): FailedFlatEntityValidation<'skill', 'delete'> {
    ...
    if (
      !isCallerClientVaultStandardApp(buildOptions) &&
      belongsToClientVaultStandardApp({
  ...
  >): FailedFlatEntityValidation<'skill', 'update'> {
    ...
    const isClientVaultStandardSkill = belongsToClientVaultStandardApp({
