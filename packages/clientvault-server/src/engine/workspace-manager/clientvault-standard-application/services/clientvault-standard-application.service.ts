...
import { computeClientVaultStandardApplicationAllFlatEntityMaps } from 'src/engine/workspace-manager/clientvault-standard-application/utils/clientvault-standard-application-all-flat-entity-maps.constant';
...
@Injectable()
export class ClientVaultStandardApplicationService {
  constructor(
  ...
  async synchronizeClientVaultStandardApplicationOrThrow({
    workspaceId,
  }: {
    workspaceId: string;
  }) {
    const { clientvaultStandardFlatApplication } =
      await this.applicationService.findWorkspaceClientVaultStandardAndCustomApplicationOrThrow(
        {
          workspaceId,
        },
      );
    const { featureFlagsMap, ...fromClientVaultStandardAllFlatEntityMaps } =
      await this.workspaceCacheService.getOrRecompute(workspaceId, [
        ...CLIENTVAULT_STANDARD_ALL_METADATA_NAME.map(getMetadataFlatEntityMapsKey),
        'featureFlagsMap',
      ]);
    const shouldIncludeRecordPageLayouts = this.clientvaultConfigService.get(
      'SHOULD_SEED_STANDARD_RECORD_PAGE_LAYOUTS',
    );

    const {
      allFlatEntityMaps: toClientVaultStandardAllFlatEntityMaps,
      idByUniversalIdentifierByMetadataName,
    } = computeClientVaultStandardApplicationAllFlatEntityMaps({
    ...
    for (const metadataName of CLIENTVAULT_STANDARD_ALL_METADATA_NAME) {
      ...
      const fromFlatEntityMaps =
        fromClientVaultStandardAllFlatEntityMaps[flatEntityMapsKey];
      const fromTo = {
        from: getSubFlatEntityMapsByApplicationIdsOrThrow<
          MetadataFlatEntity<typeof metadataName>
        >({
          applicationIds: [clientvaultStandardFlatApplication.id],
          flatEntityMaps: fromFlatEntityMaps,
        }),
        to: toClientVaultStandardAllFlatEntityMaps[flatEntityMapsKey],
      };
    ...
    if (validateAndBuildResult.status === 'fail') {
      throw new WorkspaceMigrationBuilderException(
        ...
        'Multiple validation errors occurred while synchronizing clientvault-standard application',
      );
    }
