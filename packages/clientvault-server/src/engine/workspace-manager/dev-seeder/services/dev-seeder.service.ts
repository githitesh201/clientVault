...
import { ALL_METADATA_NAME } from 'clientvault-shared/metadata';
...
import { ClientVaultStandardApplicationService } from 'src/engine/workspace-manager/clientvault-standard-application/services/clientvault-standard-application.service';
...
export class DevSeederService {
  constructor(
    ...
    private readonly clientvaultStandardApplicationService: ClientVaultStandardApplicationService,
  ...
  public async seedDev(workspaceId: SeededWorkspacesIds): Promise<void> {
    const isBillingEnabled = this.clientvaultConfigService.get('IS_BILLING_ENABLED');
    const appVersion = this.clientvaultConfigService.get('APP_VERSION');
    ...
    const { workspaceCustomFlatApplication, clientvaultStandardFlatApplication } =
    ...
    await this.clientvaultStandardApplicationService.synchronizeClientVaultStandardApplicationOrThrow(
      {
        workspaceId,
      },
    );
