...
export class WorkspaceDomainsService {
  ...
  private async getDefaultWorkspace() {
    if (this.clientvaultConfigService.get('IS_MULTIWORKSPACE_ENABLED')) {
      throw new Error(
  ...
  private getClientVaultWorkspaceUrl(subdomain: string) {
    const url = this.domainServerConfigService.getFrontUrl();

    url.hostname = this.clientvaultConfigService.get('IS_MULTIWORKSPACE_ENABLED')
  ...
  getSubdomainAndCustomDomainFromWorkspaceFallbackOnDefaultSubdomain(
    ...
    if (!workspace) {
      return {
        subdomain: this.clientvaultConfigService.get('DEFAULT_SUBDOMAIN'),
        customDomain: null,
    ...
    return {
      customUrl:
        isCustomDomainEnabled && customDomain
          ...
          : undefined,
      subdomainUrl: this.getClientVaultWorkspaceUrl(subdomain),
    };
  }
