...
export class ApplicationService {
  ...
  async findWorkspaceClientVaultStandardAndCustomApplicationOrThrow({
    ...
    const clientvaultStandardApplicationId =
      flatApplicationMaps.idByUniversalIdentifier[
        CLIENTVAULT_STANDARD_APPLICATION.universalIdentifier
      ];

    if (!isDefined(clientvaultStandardApplicationId)) {
      throw new ApplicationException(
        `Could not find workspace clientvault standard applicationId in cache ${workspaceId}`,
        ApplicationExceptionCode.APPLICATION_NOT_FOUND,
      );
    }

    const clientvaultStandardFlatApplication =
      flatApplicationMaps.byId[clientvaultStandardApplicationId];
    const workspaceCustomFlatApplication =
      flatApplicationMaps.byId[workspace.workspaceCustomApplicationId];

    if (
      !isDefined(clientvaultStandardFlatApplication) ||
    ...
    return {
      clientvaultStandardFlatApplication,
  ...
  async findClientVaultStandardApplicationOrThrow(workspaceId: string): Promise<{
    ...
    const { clientvaultStandardFlatApplication } =
      await this.findWorkspaceClientVaultStandardAndCustomApplicationOrThrow({
        workspace,
      });

    return {
      application: clientvaultStandardFlatApplication as ApplicationEntity,
      workspace,
    };
  }

  async createClientVaultStandardApplication(
    {
      workspaceId,
    ...
    const clientvaultStandardApplication = await this.create(
      {
        ...CLIENTVAULT_STANDARD_APPLICATION,
    ...
    return clientvaultStandardApplication;
  }
