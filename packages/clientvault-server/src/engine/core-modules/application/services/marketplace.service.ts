...
const GITHUB_RAW_BASE_URL =
  'https://raw.githubusercontent.com/clientvaulthq/clientvault/main/packages/clientvault-apps';
const GITHUB_API_BASE_URL =
  'https://api.github.com/repos/clientvaulthq/clientvault/contents/packages/clientvault-apps';
...
export class MarketplaceService {
  ...
  private async fetchGitHubDirectory(path: string): Promise<GitHubContent[]> {
    ...
    const response = await fetch(url, {
      headers: {
        ...
        'User-Agent': 'ClientVault-Marketplace',
      },
    });
  ...
  private async loadAppManifestFromGithub(
    ...
    const manifestContent = await this.fetchGitHubFile(
      `${appPath}/.clientvault/output/manifest.json`,
    );
    ...
    const packageJsonContent = await this.fetchGitHubFile(
      `${appPath}/.clientvault/output/package.json`,
    );
