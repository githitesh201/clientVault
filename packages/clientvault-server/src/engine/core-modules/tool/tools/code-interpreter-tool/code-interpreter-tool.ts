...
import { CLIENTVAULT_MCP_HELPER } from 'src/engine/core-modules/tool/tools/code-interpreter-tool/clientvault-mcp-helper.const';
...
export class CodeInterpreterTool implements Tool {
  ...
  async execute(
    ...
    try {
      ...
      const codeWithHelper = CLIENTVAULT_MCP_HELPER + '\n\n' + code;
      ...
      const result = await this.codeInterpreterService.execute(
        ...
        {
          env: {
            CLIENTVAULT_SERVER_URL: serverUrl,
            CLIENTVAULT_API_TOKEN: sessionToken,
          },
        },
        {
          onStdout: (line) => {
