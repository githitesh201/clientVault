...
export class RedisClientService implements OnModuleDestroy {
  ...
  getQueueClient() {
    if (!this.redisQueueClient) {
      const redisQueueUrl =
        this.clientvaultConfigService.get('REDIS_QUEUE_URL') ??
        this.clientvaultConfigService.get('REDIS_URL');
  ...
  getClient() {
    if (!this.redisClient) {
      const redisUrl = this.clientvaultConfigService.get('REDIS_URL');
