...
import ms from 'ms';
import { SendInviteLinkEmail } from 'clientvault-emails';
import { AppPath } from 'clientvault-shared/types';
import { getAppPath, isDefined } from 'clientvault-shared/utils';
...
export class WorkspaceInvitationService {
    ...
    for (const invitation of invitationsPr) {
      if (invitation.status === 'fulfilled') {
        ...
        const emailData = {
          ...
          serverUrl: this.clientvaultConfigService.get('SERVER_URL'),
        ...
        const joinTeamMsg = msg`Join your team on ClientVault`;
        const i18n = this.i18nService.getI18nInstance(sender.locale);
        const subject = i18n._(joinTeamMsg);

        await this.emailService.send({
          from: `${sender.name.firstName} ${sender.name.lastName} (via ClientVault) <${this.clientvaultConfigService.get('EMAIL_FROM_ADDRESS')}>`,
  ...
  async generateInvitationToken(workspaceId: string, email: string) {
    const expiresIn = this.clientvaultConfigService.get(
      ...
      await Promise.all(
        emails.map(async (email) => {
          await this.throttlerService.tokenBucketThrottleOrThrow(
            ...
            1,
            this.clientvaultConfigService.get(
