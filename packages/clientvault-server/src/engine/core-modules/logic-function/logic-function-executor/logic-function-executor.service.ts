...
export class LogicFunctionExecutorService {
  ...
  private async throttleExecution(workspaceId: string) {
    try {
      await this.throttlerService.tokenBucketThrottleOrThrow(
        ...
        1,
        this.clientvaultConfigService.get('LOGIC_FUNCTION_EXEC_THROTTLE_LIMIT'),
        this.clientvaultConfigService.get('LOGIC_FUNCTION_EXEC_THROTTLE_TTL'),
      );
    } catch {
  ...
  private async getExecutionEnvVariables({
    ...
    const baseUrl = cleanServerUrl(this.clientvaultConfigService.get('SERVER_URL'));
  ...
  private async handleExecutionResult({
    ...
    if (this.clientvaultConfigService.get('LOGIC_FUNCTION_LOGS_ENABLED')) {
