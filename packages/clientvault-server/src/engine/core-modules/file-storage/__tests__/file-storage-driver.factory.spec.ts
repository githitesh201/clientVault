...
describe('FileStorageDriverFactory', () => {
  ...
  describe('buildConfigKey', () => {
    it('should build config key for local storage', () => {
      ...
      expect(clientvaultConfigService.get).toHaveBeenCalledWith('STORAGE_TYPE');
      expect(clientvaultConfigService.get).toHaveBeenCalledWith(
        'STORAGE_LOCAL_PATH',
      );
    });
  ...
  describe('createDriver', () => {
    ...
    it('should throw error for invalid storage driver type', () => {
      jest.spyOn(clientvaultConfigService, 'get').mockReturnValue('invalid-type');
