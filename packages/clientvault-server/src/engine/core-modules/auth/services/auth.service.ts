...
import ms from 'ms';
import { PasswordUpdateNotifyEmail } from 'clientvault-emails';
import { PermissionFlagType } from 'clientvault-shared/constants';
import { AppPath } from 'clientvault-shared/types';
import { assertIsDefinedOrThrow, isDefined } from 'clientvault-shared/utils';
...
@Injectable()
// eslint-disable-next-line clientvault/inject-workspace-repository
export class AuthService {
  ...
  async checkIsEmailVerified(isEmailVerified: boolean) {
    const isEmailVerificationRequired = this.clientvaultConfigService.get(
    ...
    const apps = [
      {
        ...
        redirectUrl:
          ...
          NodeEnvironment.DEVELOPMENT
            ...
            : `https://${this.clientvaultConfigService.get(
                'CHROME_EXTENSION_ID',
              )}.chromiumapp.org/`,
      },
    ];
  ...
  async updatePassword(
    ...
    await this.emailService.send({
      from: `${this.clientvaultConfigService.get(
        'EMAIL_FROM_NAME',
      )} <${this.clientvaultConfigService.get('EMAIL_FROM_ADDRESS')}>`,
      to: user.email,
      subject,
      text,
      html,
