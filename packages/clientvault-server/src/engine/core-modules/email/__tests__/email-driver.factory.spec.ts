...
describe('EmailDriverFactory', () => {
  ...
  const mockClientVaultConfigService = {
    get: jest.fn(),
  };
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        EmailDriverFactory,
        {
          ...
          useValue: mockClientVaultConfigService,
        },
      ],
    }).compile();

    factory = module.get<EmailDriverFactory>(EmailDriverFactory);
    clientvaultConfigService = module.get<ClientVaultConfigService>(ClientVaultConfigService);
  ...
  describe('buildConfigKey', () => {
    it('should return "logger" for logger driver', () => {
      jest
        .spyOn(clientvaultConfigService, 'get')
        .mockReturnValue(EmailDriver.LOGGER);

      const result = factory['buildConfigKey']();

      expect(result).toBe('logger');
      expect(clientvaultConfigService.get).toHaveBeenCalledWith('EMAIL_DRIVER');
    });

    it('should return smtp config key for smtp driver', () => {
      jest.spyOn(clientvaultConfigService, 'get').mockReturnValue(EmailDriver.SMTP);
      jest
    ...
    it('should throw error for unsupported driver', () => {
      jest.spyOn(clientvaultConfigService, 'get').mockReturnValue('invalid-driver');
