...
import { type ConfigVariables } from 'src/engine/core-modules/clientvault-config/config-variables';
...
jest.mock(
  'src/engine/core-modules/clientvault-config/constants/config-variables-masking-config',
  () => ({
...
jest.mock(
  'src/engine/core-modules/clientvault-config/utils/is-env-only-config-var.util',
  () => ({
    isEnvOnlyConfigVar: jest.fn(),
  }),
);

type ClientVaultConfigServicePrivateProps = {
...
const setupTestModule = async (isDatabaseConfigEnabled = true) => {
  ...
  return {
    service: module.get<ClientVaultConfigService>(ClientVaultConfigService),
...
const setPrivateProps = (
  service: ClientVaultConfigService,
  props: Partial<ClientVaultConfigServicePrivateProps>,
) => {
...
describe('ClientVaultConfigService', () => {
  let service: ClientVaultConfigService;
