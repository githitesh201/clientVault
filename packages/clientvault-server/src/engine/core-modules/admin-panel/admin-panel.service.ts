...
import { CONFIG_VARIABLES_GROUP_METADATA } from 'src/engine/core-modules/clientvault-config/constants/config-variables-group-metadata';
import { type ConfigVariablesGroup } from 'src/engine/core-modules/clientvault-config/enums/config-variables-group.enum';
...
export class AdminPanelService {
  ...
  getConfigVariablesGrouped(): ConfigVariablesOutput {
    const rawEnvVars = this.clientvaultConfigService.getAll();
  ...
  getConfigVariable(key: string): ConfigVariableDTO {
    const variableWithMetadata =
      this.clientvaultConfigService.getVariableWithMetadata(
  ...
  async getVersionInfo(): Promise<VersionInfoDTO> {
    const currentVersion = this.clientvaultConfigService.get('APP_VERSION');

    try {
      const httpClient = this.secureHttpClientService.getHttpClient();

      const rawResponse = await httpClient.get<unknown>(
        'https://hub.docker.com/v2/repositories/clientvaultcrm/clientvault/tags?page_size=100',
      );
      const response = z
        .object({
          data: z.object({
