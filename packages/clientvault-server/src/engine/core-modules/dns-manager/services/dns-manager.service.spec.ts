...
describe('DnsManagerService', () => {
  ...
  it('should initialize cloudflareClient when CLOUDFLARE_API_KEY is defined', () => {
    ...
    jest.spyOn(clientvaultConfigService, 'get').mockReturnValue(mockApiKey);

    const instance = new DnsManagerService(clientvaultConfigService, {} as any);

    expect(clientvaultConfigService.get).toHaveBeenCalledWith('CLOUDFLARE_API_KEY');
  ...
  describe('registerHostname', () => {
    ...
    it('should register a custom domain successfully', async () => {
      ...
      jest.spyOn(clientvaultConfigService, 'get').mockReturnValue('test-zone-id');
