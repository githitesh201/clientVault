...
export class DnsManagerService {
  ...
  constructor(
    ...
    if (this.clientvaultConfigService.get('CLOUDFLARE_API_KEY')) {
      this.cloudflareClient = new Cloudflare({
        apiToken: this.clientvaultConfigService.get('CLOUDFLARE_API_KEY'),
      });
    }
  }
      ...
      records: [
        ...
        },
        {
          ...
          value:
            ...
            `${hostname}.${this.clientvaultConfigService.get('CLOUDFLARE_DCV_DELEGATION_ID')}.dcv.cloudflare.com`,
        },
      ],
    };
  }
  ...
  private getZoneId(options?: DnsManagerOptions): string {
    return options?.isPublicDomain
      ? this.clientvaultConfigService.get('CLOUDFLARE_PUBLIC_DOMAIN_ZONE_ID')
