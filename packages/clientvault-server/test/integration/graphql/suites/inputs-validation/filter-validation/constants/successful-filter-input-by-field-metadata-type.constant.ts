...
export const successfulFilterInputByFieldMetadataType: {
  ...
  [FieldMetadataType.LINKS]: [
    {
      gqlFilterInput: {
        linksField: { primaryLinkUrl: { eq: 'clientvault.com' } },
      },
      restFilterInput: 'linksField.primaryLinkUrl[eq]:"clientvault.com"',
      validateFilter: (record: Record<string, any>) => {
        return record.linksField.primaryLinkUrl === 'clientvault.com';
      },
    },
    {
      gqlFilterInput: {
        linksField: {
          primaryLinkLabel: { eq: 'clientvault - #1 Open source CRM' },
        },
      },
      restFilterInput:
        'linksField.primaryLinkLabel[eq]:"clientvault - #1 Open source CRM"',
      validateFilter: (record: Record<string, any>) => {
        return (
          record.linksField.primaryLinkLabel === 'clientvault - #1 Open source CRM'
        );
      },
    },
    {
      gqlFilterInput: { linksField: { secondaryLinks: { like: '%clientvault%' } } },
      restFilterInput: 'linksField.secondaryLinks[like]:"%clientvault%"',
      validateFilter: (record: Record<string, any>) => {
