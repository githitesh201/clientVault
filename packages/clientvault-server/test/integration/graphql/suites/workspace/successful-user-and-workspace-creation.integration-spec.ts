...
describe('Successful user and workspace creation', () => {
  ...
  it('should sign up a new user and create a new workspace successfully', async () => {
    ...
    const clientvaultStandardApp = findManyApplicationsData.find(
      (application) =>
        application.universalIdentifier ===
        CLIENTVAULT_STANDARD_APPLICATION.universalIdentifier,
    );

    jestExpectToBeDefined(clientvaultStandardApp);
    const {
      sourcePath: _sourcePath,
      sourceType: _sourceType,
      ...expectedStandardClientVaultApplication
    } = CLIENTVAULT_STANDARD_APPLICATION;

    expect(clientvaultStandardApp).toMatchObject(expectedStandardClientVaultApplication);
