...
yarn patch-commit <temp-folder> > packages/clientvault-server/patches/<package+name+version>.patch
yarn install --mode update-lockfile && yarn install
```

Reference the patch in `packages/clientvault-server/package.json`:
...
yarn patch-commit <CLEAN> > packages/clientvault-server/patches/<package+name+version>.patch
yarn install --mode update-lockfile && yarn install
```
