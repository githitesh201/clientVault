# How to deploy clientvault on podman
...
2. Start clientvault by running `podman-compose up -d`.

If you need to stop clientvault, you can do so by running `podman-compose down`.


### Install systemd service (optional)

If you want to install a systemd service to run clientvault, you can use the provided systemd service. 

Edit `clientvaultcrm.service` and change these two variables:


	WorkingDirectory=/opt/apps/clientvault
	EnvironmentFile=/opt/apps/clientvault/.env
...
Note: this script will enable the service and also start it. So it will assume that clientvault is not currently running.
