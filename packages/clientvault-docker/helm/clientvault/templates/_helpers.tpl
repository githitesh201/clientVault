{{- define "clientvault.name" -}}
{{- default .Chart.Name .Values.nameOverride | trunc 63 | trimSuffix "-" -}}
{{- end -}}

{{- define "clientvault.fullname" -}}
...
{{- define "clientvault.namespace" -}}
{{ .Release.Namespace }}
{{- end -}}

{{/* Server image fields merged with globals */}}
{{- define "clientvault.server.image" -}}
...
{{- define "clientvault.worker.image" -}}
...
{{- define "clientvault.image.repository" -}}
{{- regexFind "^([^:|]+)" . -}}
{{- end -}}
{{- define "clientvault.image.tag" -}}
{{- regexFind ":([^|]+)" . | trimPrefix ":" -}}
{{- end -}}
{{- define "clientvault.image.pullPolicy" -}}
{{- regexFind "\\|(.+)$" . | trimPrefix "|" -}}
{{- end -}}

{{/* Check if using external secret for database password */}}
{{- define "clientvault.db.useExternalSecret" -}}
...
{{- define "clientvault.dbUrl.secretName" -}}
{{- printf "%s-db-url" (include "clientvault.fullname" .) -}}
{{- end -}}

{{/* Database password secret name */}}
{{- define "clientvault.dbPassword.secretName" -}}
{{- if eq (include "clientvault.db.useExternalSecret" .) "true" -}}
{{- .Values.db.external.secretName -}}
{{- else -}}
{{- include "clientvault.dbUrl.secretName" . -}}
{{- end -}}
{{- end -}}

{{/* Database password secret key */}}
{{- define "clientvault.dbPassword.secretKey" -}}
{{- if eq (include "clientvault.db.useExternalSecret" .) "true" -}}
...
{{- define "clientvault.dbUrl.template" -}}
{{- if eq (include "clientvault.db.useExternalSecret" .) "true" -}}
{{- $scheme := "postgres" -}}
...
{{/* Compose Redis URL */}}
{{- define "clientvault.redisUrl" -}}
{{- if .Values.server.env.REDIS_URL -}}
{{- .Values.server.env.REDIS_URL -}}
{{- else if .Values.redisInternal.enabled -}}
{{- $host := printf "%s-redis" (include "clientvault.fullname" .) -}}
{{- printf "redis://%s.%s.svc.cluster.local:6379" $host (include "clientvault.namespace" .) -}}
{{- else -}}
...
{{- else -}}
{{- $svc := printf "%s-server" (include "clientvault.fullname" .) -}}
...
{{/* Tokens secret name */}}
{{- define "clientvault.secret.tokens.name" -}}
...
{{- define "clientvault.secret.tokens.access" -}}
...
{{- define "clientvault.server.containerPort" -}}
...
{{- define "clientvault.storageType" -}}
...
{{- define "clientvault.storageEnv" -}}
{{- if eq (include "clientvault.storageType" .) "s3" -}}
