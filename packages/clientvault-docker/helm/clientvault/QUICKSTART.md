# ClientVault Helm Chart - Quick Install
...
- Create the PostgreSQL database "clientvault" and schema "core" automatically
...
```bash
kubectl get secret tokens -n clientvaultcrm -o jsonpath='{.data.accessToken}' | base64 --decode && echo
```
...
```bash
helm uninstall my-clientvault -n clientvaultcrm
kubectl delete namespace clientvaultcrm
```
