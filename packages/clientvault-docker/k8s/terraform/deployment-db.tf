resource "kubernetes_deployment" "clientvaultcrm_db" {
  metadata {
  ...
  spec {
    replicas = var.clientvaultcrm_db_replicas
    selector {
      match_labels = {
        app = "${var.clientvaultcrm_app_name}-db"
      }
    ...
    template {
      ...
      spec {
        container {
          image = var.clientvaultcrm_db_image
          ...
          env {
            ...
            value = var.clientvaultcrm_pgdb_admin_password
          }
          env {
            name  = "BITNAMI_DEBUG"
            value = true
