resource "kubernetes_deployment" "clientvaultcrm_server" {
  metadata {
  ...
  spec {
    replicas = var.clientvaultcrm_server_replicas
    selector {
      match_labels = {
        app = "${var.clientvaultcrm_app_name}-server"
      }
    ...
    template {
      ...
      spec {
        container {
          ...
          volume_mount {
            name       = "server-data"
            mount_path = var.clientvaultcrm_server_data_mount_path
          }

          volume_mount {
            name       = "docker-data"
            mount_path = var.clientvaultcrm_docker_data_mount_path
          }
        }
