...
const clientvaultUiIndividualIndex = path.resolve(
  dirname,
  '../../../clientvault-ui/dist/individual/individual-entry.js',
);

const sdkIndividualIndex = path.resolve(dirname, '../../dist/sdk/index.js');

const clientvaultSharedIndividualDir = path.resolve(
  dirname,
  '../../../clientvault-shared/dist/individual',
);

const CLIENTVAULT_SHARED_SUBMODULES = [
  'ai',
  'application',
  'constants',
  'database-events',
...
const clientvaultSharedAliases = Object.fromEntries(
  CLIENTVAULT_SHARED_SUBMODULES.map((submodule) => [
    `clientvault-shared/${submodule}`,
    path.join(clientvaultSharedIndividualDir, submodule, 'index.js'),
  ]),
);
...
const storyAlias = {
  ...
  'clientvault-sdk/ui': clientvaultUiIndividualIndex,
  ...clientvaultSharedAliases,
};
...
const STORY_COMPONENTS = [
  ...
  'clientvault-ui-example.front-component',
