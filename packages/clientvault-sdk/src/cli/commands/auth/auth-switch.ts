  ...
  async execute(options: { workspace?: string }): Promise<void> {
    try {
      ...
      if (availableWorkspaces.length === 0) {
        console.log(
          chalk.yellow(
            '⚠ No workspaces configured. Use `clientvault auth:login` to create one.',
          ),
        );
        return;
      }
      ...
      if (hasCredentials) {
        ...
        if (validateAuth.authValid) {
          ...
          console.log(
            chalk.yellow(
              '⚠ Authentication credentials exist but are invalid. Run `clientvault auth:login` to re-authenticate.',
            ),
          );
        }
      } else {
        console.log(
          chalk.yellow(
            '⚠ No credentials configured for this workspace. Run `clientvault auth:login` to authenticate.',
          ),
        );
      }
    } catch (error) {
