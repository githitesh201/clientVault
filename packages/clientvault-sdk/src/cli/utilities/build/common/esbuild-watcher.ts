...
import { OUTPUT_DIR, NODE_ESM_CJS_BANNER } from 'clientvault-shared/application';
...
// Resolves clientvault-sdk/generated to the actual file path so esbuild
// bundles it instead of treating it as external (via clientvault-sdk/*)
const createSdkGeneratedResolverPlugin = (appPath: string): esbuild.Plugin => ({
  name: 'sdk-generated-resolver',
  setup: (build) => {
    build.onResolve({ filter: /^clientvault-sdk\/generated/ }, () => ({
      path: path.join(
        appPath,
        'node_modules',
        'clientvault-sdk',
