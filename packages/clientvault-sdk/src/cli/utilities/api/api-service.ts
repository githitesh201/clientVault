...
import { type Manifest } from 'clientvault-shared/application';
...
export class ApiService {
  ...
  constructor(options?: { disableInterceptors: boolean }) {
    ...
    this.client.interceptors.request.use(async (config) => {
      const clientvaultConfig = await this.configService.getConfig();

      config.baseURL = clientvaultConfig.apiUrl;

      if (!config.headers.Authorization && clientvaultConfig.apiKey) {
        config.headers.Authorization = `Bearer ${clientvaultConfig.apiKey}`;
      }
    ...
    this.client.interceptors.response.use(
      ...
      (error) => {
        if (error.response?.status === 401) {
          console.error(
            chalk.red(
              'Authentication failed. Please run `clientvault auth:login` first.',
            ),
          );
          ...
          console.error(
            chalk.red('Cannot connect to ClientVault server. Is it running?'),
          );
        }
        throw error;
      },
  ...
  async subscribeToLogs({
    ...
    const wsClient = createClient({
      url: clientvaultConfig.apiUrl + '/graphql',
      headers: {
        Authorization: `Bearer ${clientvaultConfig.apiKey}`,
        'Content-Type': 'application/json',
