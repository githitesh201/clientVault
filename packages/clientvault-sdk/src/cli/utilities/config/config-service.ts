...
type PersistedConfig = ClientVaultConfig & {
  profiles?: Record<string, ClientVaultConfig>;
...
export class ConfigService {
  ...
  async getConfig(): Promise<ClientVaultConfig> {
    return this.getConfigForWorkspace(this.getActiveWorkspaceName());
  }

  async getConfigForWorkspace(workspaceName: string): Promise<ClientVaultConfig> {
  ...
  async setConfig(config: Partial<ClientVaultConfig>): Promise<void> {
    const raw = await this.readRawConfig();
  ...
  private getDefaultConfig(): ClientVaultConfig {
    return {
