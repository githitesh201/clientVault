...
} from 'clientvault-shared/application';
...
export class ClientService {
  ...
  async generate({
    ...
    await this.injectClientVaultClient(tempPath);
  ...
  private resolveGeneratedPath(appPath: string): string {
    return join(appPath, 'node_modules', 'clientvault-sdk', 'generated');
  }

  private async injectClientVaultClient(output: string) {
    const clientvaultClientContent = `

// ----------------------------------------------------
// Custom ClientVault client (auto-injected)
// ----------------------------------------------------
...
export default class ClientVault {
...
`;
    ...
    await fs.appendFile(join(output, 'index.ts'), clientvaultClientContent);
  }
}
