...
const fetchContributorStats = async (username: string) => {
  const apiUrl = `https://clientvault.com/api/contributors/contributorStats/${username}`;
...
const fetchContributorImage = async (username: string) => {
  const apiUrl = `https://clientvault.com/api/contributors/${username}/og.png`;
...
const runCongratulate = async () => {
  ...
  const { data: pullRequests } =
    await danger.github.api.rest.search.issuesAndPullRequests({
      q: `is:pr author:${userName} is:closed repo:clientvaulthq/clientvault`,
      per_page: 2,
      page: 1,
    });
  ...
  const contributorUrl = `https://clientvault.com/contributors/${userName}`;
  ...
  const message =
    ...
    `![Contributions](https://clientvault.com/api/contributors/${userName}/og.png)`;
