...
description: Complete step-by-step guide to importing people/contacts into ClientVault.
---

## Overview

This guide walks you through importing your contacts (People) into ClientVault. **Import Companies first** if you want to link People to Companies.
...
- **Must be unique** — no duplicates in your file or in ClientVault
...
<Note>
**Use Domain URL format** (`https://acme.com`), not the label. This matches how Companies are stored in ClientVault.
</Note>
...
<Note>
**Pro tip:** Click **Download sample file** during import or export a few existing People to see the exact column names ClientVault expects.
</Note>
...
3. Wait for ClientVault to analyze your file

## Step 4: Map Your Columns

ClientVault automatically tries to match your columns to fields. Review and adjust:
...
3. ClientVault will link each Person to the matching Company
...
1. ClientVault shows your values alongside existing options
2. Match each value in your file to a ClientVault option
...
Before completing the import, ClientVault validates your data:
...
|-------|-------|----------|
| **Duplicate email** | Email already exists in ClientVault or file | Remove duplicate or update existing record |
...
Your contacts are now in ClientVault! Next steps:
...
2. ClientVault matches records by this unique identifier
...
<AccordionGroup>
  <Accordion title="Why can't two People have the same email?">
    Email is a unique identifier in ClientVault. This prevents duplicate contacts and ensures email sync correctly links emails to the right person.
  </Accordion>
  ...
  <Accordion title="Can I import the ID from my previous CRM?">
    Yes! Create a custom field marked as "unique" in your data model to store the external ID. Note: the field name `id` is reserved for ClientVault's internal ID.
  </Accordion>
