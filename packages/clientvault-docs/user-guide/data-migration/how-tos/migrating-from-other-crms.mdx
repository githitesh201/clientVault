...
description: Step-by-step guide to migrate your data from any CRM to ClientVault.
---

## Overview

This guide walks you through migrating your data from any CRM to ClientVault. The process involves auditing your data, preparing your ClientVault workspace, exporting from your current system, and importing into ClientVault.
...
Create a mapping document between your current CRM and ClientVault:

| Your CRM | ClientVault |
|----------|--------|
...
## Step 3: Set Up Your ClientVault Workspace

Before importing data, prepare your ClientVault workspace:
...
If your data includes user references (Account Owner, Assignee, etc.), those users must exist in ClientVault before import. Otherwise, those relations cannot be mapped.
</Warning>
...
Open each exported CSV in a spreadsheet application and prepare it for ClientVault.

### Remove Duplicates

1. Sort by the unique field (email for People, domain for Companies)
2. Remove or merge duplicate rows
3. Verify no duplicates exist in ClientVault already
...
Use `https://domain.com` (not `domain.com` or `www.domain.com`). This matches ClientVault's format and prevents duplicates when you connect email/calendar sync.
</Warning>
...
1. Navigate to the object in ClientVault
...
- ClientVault will start syncing emails to contact records
...
<AccordionGroup>
  <Accordion title="Can I migrate workflows automatically?">
    Not currently. Workflows must be recreated manually in ClientVault.
  </Accordion>
...
Contact [contact@clientvault.com](mailto:contact@clientvault.com) or explore our [Implementation Services](/user-guide/getting-started/capabilities/implementation-services).
