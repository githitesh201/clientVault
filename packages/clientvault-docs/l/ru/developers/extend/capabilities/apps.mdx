---
title: Приложения ClientVault
description: Создавайте и управляйте настройками ClientVault в виде кода.
---
...
Приложения позволяют создавать и управлять настройками ClientVault **в виде кода**. Вместо настройки всего через интерфейс вы определяете модель данных и логические функции в коде — так быстрее создавать, поддерживать и развёртывать в нескольких рабочих пространствах.
...
* Node.js 24+ и Yarn 4
* Рабочее пространство ClientVault и ключ API (создайте его на https://app.clientvault.com/settings/api-webhooks)
...
Смотрите также: страницы справки CLI для [create-clientvault-app](https://www.npmjs.com/package/create-clientvault-app) и [clientvault-sdk CLI](https://www.npmjs.com/package/clientvault-sdk).

## Структура проекта (сгенерированного)

Когда вы запускаете `npx create-clientvault-app@latest my-clientvault-app`, генератор:

* Копирует минимальное базовое приложение в `my-clientvault-app/`
* Добавляет локальную зависимость `clientvault-sdk` и конфигурацию Yarn 4
* Создаёт файлы конфигурации и скрипты, подключённые к CLI `clientvault`
...
* **package.json**: Объявляет имя приложения, версию, движки (Node 24+, Yarn 4) и добавляет `clientvault-sdk`, а также скрипт `clientvault`, который делегирует выполнение локальному CLI `clientvault`. Выполните `yarn clientvault help`, чтобы вывести список всех доступных команд.
...
SDK обнаруживает сущности, разбирая ваши файлы TypeScript в поисках вызовов **`export default define<Entity>({...})`**. Для каждого типа сущности существует соответствующая вспомогательная функция, экспортируемая из `clientvault-sdk`:
...
* `yarn clientvault app:dev` автоматически сгенерирует типизированный клиент API в `node_modules/clientvault-sdk/generated` (типизированный клиент ClientVault + типы рабочего пространства).
* `yarn clientvault entity:add` добавит файлы определений сущностей в `src/` для ваших пользовательских объектов, функций, фронтенд-компонентов или ролей.

## Аутентификация

При первом запуске `yarn clientvault auth:login` вам будет предложено указать:

* URL API (по умолчанию http://localhost:3000 или текущий профиль рабочего пространства)
* Ключ API

Ваши учётные данные хранятся для каждого пользователя в `~/.clientvault/config.json`. Вы можете хранить несколько профилей и переключаться между ними.
...
После переключения рабочего пространства с помощью `yarn clientvault auth:switch` все последующие команды по умолчанию будут использовать это рабочее пространство. Вы по-прежнему можете временно переопределить это с помощью `--workspace <name>`.

## Используйте ресурсы SDK (типы и конфигурация)

Пакет clientvault-sdk предоставляет типизированные строительные блоки и вспомогательные функции, которые вы используете внутри своего приложения. Ниже — ключевые части, с которыми вы будете работать чаще всего.
...
* Вы можете сгенерировать новые объекты с помощью `yarn clientvault entity:add`, который проведёт вас через настройку имени, полей и связей.

<Note>
  **Базовые поля создаются автоматически.** Когда вы определяете пользовательский объект, ClientVault автоматически добавляет стандартные поля,
...
* Ключ API во время выполнения, подставляемый как `CLIENTVAULT_API_KEY`, получается из этой роли функции по умолчанию.
...
* См. рабочий пример в приложении Hello World: [`packages/clientvault-apps/hello-world/src/roles/function-role.ts`](https://github.com/clientvaulthq/clientvault/blob/main/packages/clientvault-apps/hello-world/src/roles/function-role.ts).
...
Когда вы создаёте каркас нового приложения с помощью `create-clientvault-app`, для вас генерируется постустановочная функция по пути `src/logic-functions/post-install.ts`:
...
Когда триггер маршрута вызывает вашу логическую функцию, она получает объект `RoutePayload`, соответствующий формату AWS HTTP API v2. Импортируйте тип из `clientvault-sdk`:
...
* **Сгенерировано**: Запустите `yarn clientvault entity:add` и выберите опцию добавления новой функции логики. Это создаёт стартовый файл с обработчиком и конфигурацией.
...
Логические функции можно предоставлять как **инструменты** для ИИ-агентов и рабочих процессов. Когда функция помечена как инструмент, она становится доступной для ИИ ClientVault и может быть выбрана в качестве шага в автоматизациях рабочих процессов.
...
Фронт-компоненты позволяют создавать пользовательские компоненты React, которые рендерятся внутри интерфейса ClientVault. Используйте `defineFrontComponent()` для определения компонентов со встроенной валидацией:
...
* Фронт-компоненты — это компоненты React, которые рендерятся в изолированных контекстах внутри ClientVault.
...
* Компоненты автоматически собираются и синхронизируются во время `yarn clientvault app:dev`.

Вы можете создать новые фронт-компоненты двумя способами:

* **Сгенерировано**: Запустите `yarn clientvault entity:add` и выберите опцию добавления нового фронтенд-компонента.
* **Вручную**: Создайте новый файл `*.front-component.tsx` и используйте `defineFrontComponent()`.

### Сгенерированный типизированный клиент

Типизированный клиент автоматически генерируется с помощью `yarn clientvault app:dev` и сохраняется в `node_modules/clientvault-sdk/generated` на основе схемы вашего рабочего пространства. Используйте его в своих функциях:
...
Клиент автоматически перегенерируется с помощью `yarn clientvault app:dev` при изменении ваших объектов или полей.

#### Учётные данные времени выполнения в логических функциях

Когда ваша функция запускается на ClientVault, платформа подставляет учётные данные как переменные окружения перед выполнением вашего кода:

* `CLIENTVAULT_API_URL`: Базовый URL API ClientVault, на который нацелено ваше приложение.
* `CLIENTVAULT_API_KEY`: Краткоживущий ключ, ограниченный ролью функции по умолчанию вашего приложения.

Заметки:

* Вам не нужно передавать URL или ключ API сгенерированному клиенту. Он читает `CLIENTVAULT_API_URL` и `CLIENTVAULT_API_KEY` из process.env во время выполнения.
...
Ознакомьтесь с минимальным сквозным примером, демонстрирующим объекты, логические функции, фронт-компоненты и несколько триггеров, [здесь](https://github.com/clientvaulthq/clientvault/tree/main/packages/clientvault-apps/hello-world):

## Ручная настройка (без генератора)

Хотя мы рекомендуем использовать `create-clientvault-app` для наилучшего старта, вы также можете настроить проект вручную. Не устанавливайте CLI глобально. Вместо этого добавьте `clientvault-sdk` как локальную зависимость и настройте один скрипт в вашем package.json:

```bash filename="Terminal"
yarn add -D clientvault-sdk
```

Затем добавьте скрипт `clientvault`:
...
Теперь вы можете запускать все команды через `yarn clientvault <command>`, например, `yarn clientvault app:dev`, `yarn clientvault help` и т. д.

## Устранение неполадок

* Ошибки аутентификации: выполните `yarn clientvault auth:login` и убедитесь, что у вашего ключа API есть необходимые права.
* Не удаётся подключиться к серверу: проверьте URL API и доступность сервера ClientVault.
* Types or client missing/outdated: restart `yarn clientvault app:dev` — it auto-generates the typed client.
* Режим разработки не синхронизируется: убедитесь, что запущен `yarn clientvault app:dev`, и что ваша среда не игнорирует изменения.
