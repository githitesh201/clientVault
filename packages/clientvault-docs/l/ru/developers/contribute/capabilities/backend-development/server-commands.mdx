...
Эти команды следует выполнять из папки packages/clientvault-server.
Из любой другой папки можно запускать `npx nx {command} clientvault-server` (или `npx nx run clientvault-server:{command}`).

### Первоначальная настройка

```
npx nx database:reset clientvault-server # настроить базу данных с начальными данными для разработки
```
...
```
npx nx run clientvault-server:lint # добавьте --fix для устранения ошибок линтера
```

### Тест

```
npx nx run clientvault-server:test:unit # запуск модульных тестов
npx nx run clientvault-server:test:integration # запуск интеграционных тестов
```

Примечание: вы можете использовать `npx nx run clientvault-server:test:integration:with-db-reset`, если необходимо сбросить базу данных перед выполнением интеграционных тестов.
...
Для работы с серверной частью ClientVault в основном использует NestJS.
