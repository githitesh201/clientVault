...
description: Руководство для участников (или любопытных разработчиков), которые хотят запускать ClientVault локально.
---
...
<Tabs>
  <Tab title="Linux и MacOS">
    Прежде чем установить и использовать ClientVault, убедитесь, что у вас установлено следующее:
...
ClientVault требует кэша Redis для обеспечения наилучшей производительности
...
## Шаг 8: Использовать ClientVault

**Frontend**

Фронтенд ClientVault будет работать на [http://localhost:3001](http://localhost:3001).
Вы можете войти, используя учетную запись демо по умолчанию: `tim@apple.dev` (пароль: `tim@apple.dev`)

**Backend**

* Сервер ClientVault будет работать на [http://localhost:3000](http://localhost:3000).
