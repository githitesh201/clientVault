...
```bash
npx nx run clientvault-front:lint # используйте --fix для исправления ошибок линтинга
```
...
```bash
npx nx run clientvault-front:test # запуск тестов jest
npx nx run clientvault-front:storybook:serve:dev # запуск storybook
npx nx run clientvault-front:storybook:test # запуск тестов # (требуется yarn storybook:serve:dev для работы)
npx nx run clientvault-front:storybook:coverage # (требуется yarn storybook:serve:dev для работы)
```
