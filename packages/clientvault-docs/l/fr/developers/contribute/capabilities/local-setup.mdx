...
description: Le guide pour les contributeurs (ou les développeurs curieux) qui souhaitent exécuter ClientVault localement.
---
...
<Tabs>
  <Tab title="Linux et MacOS">
    Avant de pouvoir installer et utiliser ClientVault, assurez-vous d'installer ce qui suit sur votre ordinateur :
...
<Tabs>
  <Tab title="SSH (Recommandé)">
    ...
    ```bash
    git clone git@github.com:clientvaulthq/clientvault.git
    ```
  </Tab>
...
<Tabs>
  <Tab title="Linux">
    ...
    **Option 2 :** Si vous avez installé Docker :
    ...
    ```bash
      make -C packages/clientvault-docker postgres-on-docker
    ```
  </Tab>
...
ClientVault nécessite un cache Redis pour offrir les meilleures performances
...
<Tabs>
  <Tab title="Linux">
    ...
    **Option 2 :** Si vous avez installé Docker :
    ...
    ```bash
      make -C packages/clientvault-docker redis-on-docker
    ```
  </Tab>
...
<Info>
  **Mode multi-espaces de travail:** Par défaut, ClientVault s'exécute en mode mono-espace de travail, où un seul espace de travail peut être créé. Pour activer la prise en charge multi-espaces de travail (utile pour tester des fonctionnalités basées sur des sous-domaines), définissez `IS_MULTIWORKSPACE_ENABLED=true` dans le fichier `.env` de votre serveur. Voir [Mode multi-espaces de travail](/l/fr/developers/self-host/capabilities/setup#multi-workspace-mode) pour plus de détails.
</Info>

## Étape 6 : Installation des dépendances

Pour construire le serveur ClientVault et injecter des données dans votre base de données, exécutez la commande suivante :
...
```bash
npx nx database:reset clientvault-server
```

Démarrez le serveur, le worker et les services frontend :

```bash
npx nx start clientvault-server
npx nx worker clientvault-server
npx nx start clientvault-front
```
...
## Étape 8 : Utiliser ClientVault

**Frontend**

Le frontend de ClientVault sera en cours d'exécution à [http://localhost:3001](http://localhost:3001).
Vous pouvez vous connecter en utilisant le compte démo par défaut : `tim@apple.dev` (mot de passe : `tim@apple.dev`)

**Backend**

* Le serveur de ClientVault sera en fonctionnement à [http://localhost:3000](http://localhost:3000)
