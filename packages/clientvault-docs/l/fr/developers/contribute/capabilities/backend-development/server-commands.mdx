...
Ces commandes doivent être exécutées depuis le dossier packages/clientvault-server.
Depuis n'importe quel autre dossier, vous pouvez exécuter `npx nx {command} clientvault-server` (ou `npx nx run clientvault-server:{command}`).
...
```
npx nx run clientvault-server:lint # passez --fix pour corriger les erreurs de lint
```
...
Remarque: vous pouvez exécuter `npx nx run clientvault-server:test:integration:with-db-reset` si vous avez besoin de réinitialiser la base de données avant de lancer les tests d'intégration.
...
ClientVault utilise principalement NestJS pour le backend.
