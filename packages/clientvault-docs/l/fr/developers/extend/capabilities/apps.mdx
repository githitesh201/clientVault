---
title: Applications ClientVault
description: Créez et gérez les personnalisations ClientVault sous forme de code.
---
...
Les applications vous permettent de créer et de gérer des personnalisations ClientVault **sous forme de code**. Au lieu de tout configurer via l’interface utilisateur, vous définissez votre modèle de données et vos fonctions logiques dans le code — ce qui accélère la création, la maintenance et le déploiement sur plusieurs espaces de travail.
...
* Un espace de travail ClientVault et une clé API (créez-en une sur https://app.clientvault.com/settings/api-webhooks)
...
Voir aussi : les pages de référence CLI pour [create-clientvault-app](https://www.npmjs.com/package/create-clientvault-app) et [clientvault-sdk CLI](https://www.npmjs.com/package/clientvault-sdk).

## Structure du projet (générée)

Lorsque vous exécutez `npx create-clientvault-app@latest my-clientvault-app`, l’outil de scaffolding :

* Copie une application de base minimale dans `my-clientvault-app/`
* Ajoute une dépendance locale à `clientvault-sdk` et la configuration Yarn 4
* Crée des fichiers de configuration et des scripts reliés à la CLI `clientvault`
...
* **package.json** : Déclare le nom de l’application, la version, les moteurs (Node 24+, Yarn 4), et ajoute `clientvault-sdk` ainsi que des scripts comme `app:dev`, `entity:add`, `function:logs`, `function:execute`, `app:uninstall` et `auth:login` qui délèguent à la CLI locale `clientvault`.
...
* `yarn app:dev` génère automatiquement le client ClientVault typé dans `node_modules/clientvault-sdk/generated`.
...
Vos identifiants sont stockés par utilisateur dans `~/.clientvault/config.json`. Vous pouvez gérer plusieurs profils et basculer entre eux.
...
Le paquet clientvault-sdk fournit des blocs de construction typés et des fonctions utilitaires que vous utilisez dans votre application. Voici les éléments clés que vous manipulerez le plus souvent.
...
<Note>
  **Les champs de base sont créés automatiquement.** Lorsque vous définissez un objet personnalisé, ClientVault ajoute automatiquement des champs standard tels que `name`, `createdAt`, `updatedAt`, `createdBy`, `position` et `deletedAt`. Vous n'avez pas besoin de les définir dans votre tableau `fields` — ajoutez uniquement vos champs personnalisés.
</Note>
...
* La clé API d’exécution injectée sous `CLIENTVAULT_API_KEY` est dérivée de ce rôle de fonction par défaut.
...
* Voir un exemple fonctionnel dans l’application Hello World : [`packages/clientvault-apps/hello-world/src/roles/function-role.ts`](https://github.com/clientvaulthq/clientvault/blob/main/packages/clientvault-apps/hello-world/src/roles/function-role.ts).
...
Lorsqu’un déclencheur de route appelle votre fonction logique, elle reçoit un objet `RoutePayload` qui suit le format AWS HTTP API v2. Importez le type depuis `clientvault-sdk` :
...
`yarn app:dev` génère automatiquement le client ClientVault typé dans `node_modules/clientvault-sdk/generated`. Utilisez-le dans vos fonctions :
...
Lorsque votre fonction s’exécute sur ClientVault, la plateforme injecte des identifiants sous forme de variables d’environnement avant l’exécution de votre code :

* `CLIENTVAULT_API_URL` : URL de base de l’API ClientVault ciblée par votre application.
* `CLIENTVAULT_API_KEY` : Clé de courte durée limitée au rôle de fonction par défaut de votre application.

Notes:

* Vous n’avez pas besoin de passer l’URL ou la clé API au client généré. Il lit `CLIENTVAULT_API_URL` et `CLIENTVAULT_API_KEY` depuis process.env à l’exécution.
...
Découvrez un exemple minimal de bout en bout qui démontre des objets, des fonctions et plusieurs déclencheurs [ici](https://github.com/clientvaulthq/clientvault/tree/main/packages/clientvault-apps/hello-world) :

## Configuration manuelle (sans l’outil de scaffolding)

Même si nous recommandons d’utiliser `create-clientvault-app` pour une expérience de démarrage optimale, vous pouvez également configurer un projet manuellement. N’installez pas la CLI globalement. Ajoutez plutôt `clientvault-sdk` comme dépendance locale et reliez des scripts dans votre package.json :
...
* Impossible de se connecter au serveur : vérifiez l’URL de l’API et que le serveur ClientVault est accessible.
