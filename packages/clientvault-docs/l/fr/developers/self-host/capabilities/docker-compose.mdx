...
Ce guide fournit des instructions pas-à-pas pour installer et configurer l'application ClientVault à l'aide de Docker Compose. L'objectif est de simplifier le processus et d'éviter les erreurs courantes qui pourraient compromettre votre configuration.
...
Installez la dernière version stable de ClientVault avec une seule commande :
...
Si vous hébergez clientvaultCRM sur votre ordinateur, ouvrez votre navigateur et allez à [http://localhost:3000](http://localhost:3000).
...
### Exposer ClientVault à un Accès Externe

Par défaut, ClientVault fonctionne sur `localhost` au port `3000`. Pour y accéder via un domaine externe ou une adresse IP, vous devez configurer le `SERVER_URL` dans votre fichier `.env`.
...
Nous vous recommandons vivement de mettre en place ClientVault derrière un proxy inverse avec terminaison SSL pour une sécurité et une fonctionnalité optimales.
