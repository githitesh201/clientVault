---
title: Importer les données de produits dans ClientVault
...
Utilisez ce modèle pour garder ClientVault synchronisé avec les données produits de votre entrepôt de données (par ex. Snowflake, BigQuery, PostgreSQL).
...
5. **Upsert Record** : Créer ou mettre à jour dans ClientVault
...
export const main = async (params: {
  ...
  return {
    users: usersFormatted.map((user) => ({
      ...
      isClientVault: toBool(user.isClientVault),
    })),
  };
};
```
