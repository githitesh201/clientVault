...
| **Intégrations personnalisées** | Votre application → automatisation dans ClientVault                 |
...
3. Vous recevrez une URL de webhook unique, par exemple :
   ```
   https://api.clientvault.com/webhooks/workflow/abc123...
   ```
...
```bash
curl -X POST https://api.clientvault.com/webhooks/workflow/abc123... \
