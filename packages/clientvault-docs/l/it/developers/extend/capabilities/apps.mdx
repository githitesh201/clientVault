---
title: App di ClientVault
description: Crea e gestisci le personalizzazioni di ClientVault come codice.
---
...
Le app ti consentono di creare e gestire le personalizzazioni di ClientVault **come codice**. Invece di configurare tutto tramite l'interfaccia utente, definisci in codice il modello dati e le funzioni logiche — rendendo più veloce creare, mantenere e distribuire su più spazi di lavoro.
...
* Node.js 24+ e Yarn 4
* Uno spazio di lavoro ClientVault e una chiave API (creane una su https://app.clientvault.com/settings/api-webhooks)
...
Vedi anche: le pagine di riferimento della CLI per [create-clientvault-app](https://www.npmjs.com/package/create-clientvault-app) e [clientvault-sdk CLI](https://www.npmjs.com/package/clientvault-sdk).

## Struttura del progetto (generata dallo scaffolder)

Quando esegui `npx create-clientvault-app@latest my-clientvault-app`, lo scaffolder:

* Copia un'applicazione base minimale in `my-clientvault-app/`
* Aggiunge una dipendenza locale `clientvault-sdk` e la configurazione di Yarn 4
* Crea file di configurazione e script collegati alla CLI `clientvault`
...
* **package.json**: Dichiara il nome dell'app, la versione, i motori (Node 24+, Yarn 4) e aggiunge `clientvault-sdk` più uno script `clientvault` che delega alla CLI locale `clientvault`. Esegui `yarn clientvault help` per elencare tutti i comandi disponibili.
...
L'SDK rileva le entità analizzando i tuoi file TypeScript alla ricerca di chiamate **`export default define<Entity>({...})`**. Ogni tipo di entità ha una corrispondente funzione helper esportata da `clientvault-sdk`:
...
* `yarn clientvault app:dev` genererà automaticamente un client API tipizzato in `node_modules/clientvault-sdk/generated` (client ClientVault tipizzato + tipi dell'area di lavoro).
* `yarn clientvault entity:add` aggiungerà file di definizione delle entità sotto `src/` per i tuoi oggetti, funzioni, componenti front-end o ruoli personalizzati.

## Autenticazione

La prima volta che esegui `yarn clientvault auth:login`, ti verranno richiesti:

* URL dell'API (predefinito a http://localhost:3000 o al profilo dello spazio di lavoro corrente)
* Chiave API

Le tue credenziali sono archiviate per utente in `~/.clientvault/config.json`. Puoi mantenere più profili e passare da uno all'altro.
...
Una volta che hai cambiato area di lavoro con `yarn clientvault auth:switch`, tutti i comandi successivi utilizzeranno quell'area di lavoro per impostazione predefinita. Puoi comunque sovrascriverla temporaneamente con `--workspace <name>`.

## Usa le risorse dell'SDK (tipi e configurazione)

Il pacchetto clientvault-sdk fornisce blocchi tipizzati e funzioni helper da usare nella tua app. Di seguito gli elementi principali con cui interagirai più spesso.
...
* Puoi generare nuovi oggetti con `yarn clientvault entity:add`, che ti guida nella denominazione, nei campi e nelle relazioni.

<Note>
  **I campi base vengono creati automaticamente.** Quando definisci un oggetto personalizzato, ClientVault aggiunge automaticamente i campi standard
...
* La chiave API di runtime iniettata come `CLIENTVAULT_API_KEY` è derivata da questo ruolo funzione predefinito.
...
* Vedi un esempio funzionante nell'app Hello World: [`packages/clientvault-apps/hello-world/src/roles/function-role.ts`](https://github.com/clientvaulthq/clientvault/blob/main/packages/clientvault-apps/hello-world/src/roles/function-role.ts).
...
Quando esegui lo scaffolding di una nuova app con `create-clientvault-app`, viene generata automaticamente una funzione di post-installazione in `src/logic-functions/post-install.ts`:
...
Quando un trigger di route invoca la tua funzione logica, questa riceve un oggetto `RoutePayload` che segue il formato AWS HTTP API v2. Importa il tipo da `clientvault-sdk`:
...
* **Generata dallo scaffolder**: Esegui `yarn clientvault entity:add` e scegli l'opzione per aggiungere una nuova funzione logica. Questo genera un file iniziale con un handler e una configurazione.
...
Le funzioni logiche possono essere esposte come **strumenti** per gli agenti di IA e i flussi di lavoro. Quando una funzione è contrassegnata come strumento, diventa individuabile dalle funzionalità di IA di ClientVault e può essere selezionata come passaggio nelle automazioni dei flussi di lavoro.
...
I componenti front-end ti consentono di creare componenti React personalizzati che vengono renderizzati all'interno dell'interfaccia di ClientVault. Usa `defineFrontComponent()` per definire componenti con convalida integrata:
...
* I componenti front-end sono componenti React che eseguono il rendering in contesti isolati all'interno di ClientVault.
...
* I componenti vengono compilati e sincronizzati automaticamente durante `yarn clientvault app:dev`.

Puoi creare nuovi componenti front-end in due modi:

* **Generata dallo scaffolder**: Esegui `yarn clientvault entity:add` e scegli l'opzione per aggiungere un nuovo componente front-end.
* **Manuale**: Crea un nuovo file `*.front-component.tsx` e usa `defineFrontComponent()`.

### Client tipizzato generato

Il client tipizzato è generato automaticamente da `yarn clientvault app:dev` e salvato in `node_modules/clientvault-sdk/generated` in base allo schema della tua area di lavoro. Usalo nelle tue funzioni:
...
Il client viene rigenerato automaticamente da `yarn clientvault app:dev` ogni volta che i tuoi oggetti o campi cambiano.

#### Credenziali di runtime nelle funzioni logiche

Quando la tua funzione viene eseguita su ClientVault, la piattaforma inietta le credenziali come variabili d'ambiente prima dell'esecuzione del tuo codice:

* `CLIENTVAULT_API_URL`: URL di base dell'API ClientVault a cui punta la tua app.
* `CLIENTVAULT_API_KEY`: Chiave a breve durata con ambito al ruolo funzione predefinito della tua applicazione.

Note:

* Non è necessario passare URL o chiave API al client generato. Legge `CLIENTVAULT_API_URL` e `CLIENTVAULT_API_KEY` da process.env in fase di esecuzione.
...
Esplora un esempio minimale end-to-end che dimostra oggetti, funzioni logiche, componenti front-end e trigger multipli [qui](https://github.com/clientvaulthq/clientvault/tree/main/packages/clientvault-apps/hello-world):

## Configurazione manuale (senza lo scaffolder)

Sebbene consigliamo di utilizzare `create-clientvault-app` per la migliore esperienza iniziale, puoi anche configurare un progetto manualmente. Non installare la CLI globalmente. Invece, aggiungi `clientvault-sdk` come dipendenza locale e collega un unico script nel tuo package.json:

```bash filename="Terminal"
yarn add -D clientvault-sdk
```

Quindi aggiungi uno script `clientvault`:
...
Ora puoi eseguire tutti i comandi tramite `yarn clientvault <command>`, ad es. `yarn clientvault app:dev`, `yarn clientvault help`, ecc.

## Risoluzione dei problemi

* Errori di autenticazione: esegui `yarn clientvault auth:login` e assicurati che la tua chiave API abbia i permessi richiesti.
* Impossibile connettersi al server: verifica l'URL dell'API e che il server ClientVault sia raggiungibile.
* Types or client missing/outdated: restart `yarn clientvault app:dev` — it auto-generates the typed client.
* Modalità di sviluppo non sincronizzata: assicurati che `yarn clientvault app:dev` sia in esecuzione e che le modifiche non vengano ignorate dal tuo ambiente.
