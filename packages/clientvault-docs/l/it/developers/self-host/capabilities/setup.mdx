...
<Warning>
  **Prima volta che installi?** Segui la [guida all'installazione di Docker Compose](/l/it/developers/self-host/capabilities/docker-compose) per far funzionare ClientVault, quindi torna qui per la configurazione.
</Warning>

ClientVault offre **due modalità di configurazione** per soddisfare diverse esigenze di distribuzione:
...
1. Accedi alla tua istanza ClientVault (solitamente `http://localhost:3000`)
...
<Warning>
  ...
  [Riferimento tecnico completo →](https://github.com/clientvaulthq/clientvault/blob/main/packages/clientvault-server/src/engine/core-modules/clientvault-config/config-variables.ts)
</Warning>
...
Per impostazione predefinita, ClientVault viene eseguito in **modalità a workspace singolo** — ideale per la maggior parte delle distribuzioni self-hosted in cui è necessaria un'istanza di CRM per la tua organizzazione.
...
* Un workspace per istanza di ClientVault
...
Abilita la modalità multi-workspace per distribuzioni in stile SaaS in cui più team indipendenti necessitano dei propri workspace sulla stessa istanza di ClientVault.
...
**Scope richiesti** (configurati automaticamente): [Vedi il codice sorgente pertinente](https://github.com/clientvaulthq/clientvault/blob/main/packages/clientvault-server/src/engine/core-modules/auth/utils/get-google-apis-oauth-scopes.ts#L4-L10)
...
<Warning>
  Gli utenti devono avere una [Licenza Microsoft 365](https://admin.microsoft.com/Adminportal/Home) per poter utilizzare l'API Calendario e Messaggi. Non potranno sincronizzare il loro account su ClientVault senza una.
</Warning>
...
[Vedi il codice sorgente pertinente](https://github.com/clientvaulthq/clientvault/blob/main/packages/clientvault-server/src/engine/core-modules/auth/utils/get-microsoft-apis-oauth-scopes.ts#L2-L9)
...
ClientVault supporta le funzioni logiche per i workflow e la logica personalizzata. L'ambiente di esecuzione è configurato tramite la variabile di ambiente `SERVERLESS_TYPE`.
...
<Note>
  Quando si utilizza `SERVERLESS_TYPE=DISABLED`, qualsiasi tentativo di eseguire una funzione logica restituirà un errore. Ciò è utile se si desidera eseguire ClientVault senza il supporto per le funzioni logiche.
</Note>
