...
Se hai un'istanza di ClientVault esistente con dati di produzione, **NON** seguire questi passaggi poiché elimineranno permanentemente il tuo database!

Durante l'installazione di ClientVault per la prima volta, potresti voler cambiare la password del database predefinita.
...
Durante l'installazione di ClientVault, devi fornire il tuo database postgres con gli schemi, le estensioni e gli utenti corretti.
...
#### Cannot find module 'clientvault-emails' or its corresponding type declarations.

You have to build the package `clientvault-emails` before running the initialization of the database with `npx nx run clientvault-emails:build`

#### Pacchetto clientvault-x mancante

Assicurati di eseguire yarn nella directory principale e poi esegui `npx nx server:dev clientvault-server`. Se ancora non funziona prova a costruire manualmente il pacchetto mancante.
...
#### Durante l'esecuzione di `npx nx start` o `npx nx start clientvault-front`, viene generato un errore di memoria insufficiente

In `packages/clientvault-front/.env` decommentare `VITE_DISABLE_TYPESCRIPT_CHECKER=true` e `VITE_DISABLE_ESLINT_CHECKER=true` per disabilitare i controlli in background riducendo così la quantità di RAM necessaria.

**If it does not work:**
Run only the services you need, instead of `npx nx start`. Ad esempio, se lavori sul server, esegui solo `npx nx worker clientvault-server`

**If it does not work:**
If you tried to run only `npx nx run clientvault-server:start` on WSL and it's failing with the below memory error:
...
**If it does not work:**
Investigate which processes are taking you most of your machine RAM. In ClientVault, abbiamo notato che alcune estensioni di VScode stavano occupando molta RAM quindi le disabilitiamo temporaneamente.
...
* Apri la tua app ClientVault, naviga su `/settings` e abilita il toggle **Avanzate** in basso a sinistra dello schermo.
* Crea un nuovo webhook.
* Incolla **Your Unique Webhook URL** nel campo **Endpoint Url** in ClientVault. Imposta i **Filtri** su `Companies` e `Created`.
...
Commenta il plugin checker in `packages/clientvault-ui/vite-config.ts` come mostrato nell'esempio sotto
...
Se stai eseguendo ClientVault dietro un reverse proxy e stai riscontrando problemi di connessione:
...
2. **Controlla le impostazioni del reverse proxy:**
   ...
   * Conferma che il tuo reverse proxy sta inoltrando correttamente le richieste al server ClientVault.
   * Assicurati che le intestazioni come `X-Forwarded-For` e `X-Forwarded-Proto` siano impostate correttamente.

3. **Riavvia i servizi:**

   Dopo aver apportato modifiche, riavvia sia il reverse proxy che i container ClientVault.
...
* Supporto comunitario:
  ...
  Contatta la [comunità ClientVault](https://github.com/clientvaulthq/clientvault/issues) o [canali di supporto](https://discord.gg/cx5n4Jzs57) per assistenza.
