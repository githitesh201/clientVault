...
```bash
npx nx run clientvault-front:lint # aggiungere --fix per correggere gli errori di lint
```

## Traduzioni

```bash
npx nx run clientvault-front:lingui:extract
npx nx run clientvault-front:lingui:compile
```

### Test

```bash
npx nx run clientvault-front:test # eseguire i test jest
npx nx run clientvault-front:storybook:serve:dev # eseguire storybook
npx nx run clientvault-front:storybook:test # eseguire i test # (è necessario che yarn storybook:serve:dev sia in esecuzione)
npx nx run clientvault-front:storybook:coverage # (è necessario che yarn storybook:serve:dev sia in esecuzione)
```
