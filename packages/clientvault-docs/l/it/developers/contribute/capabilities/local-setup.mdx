...
description: La guida per i collaboratori (o sviluppatori curiosi) che vogliono eseguire ClientVault localmente.
---
...
<Tabs>
  <Tab title="Linux e MacOS">
    Prima di poter installare e usare ClientVault, assicurati di installare quanto segue sul tuo computer:
...
ClientVault richiede una cache Redis per offrire le migliori prestazioni
...
## Passo 8: Utilizza ClientVault

**Frontend**

Il frontend di ClientVault sarà in esecuzione su [http://localhost:3001](http://localhost:3001).
Puoi accedere utilizzando l'account demo predefinito: `tim@apple.dev` (password: `tim@apple.dev`)

**Backend**

* Il server di ClientVault sarà attivo e funzionante su [http://localhost:3000](http://localhost:3000)
