...
Dopo aver caricato il file e mappato le colonne, ClientVault convalida i tuoi dati:

1. **Esecuzione della convalida** — ClientVault controlla ogni riga per individuare errori
...
| ---------------------------------------------------------------------------------- | --------------------------------------------------------------- |
| Valori duplicati evidenziati in giallo                                             | Il valore esiste già in ClientVault o compare due volte nel tuo file |
...
**Opzione 3: Consenti a ClientVault di aggiornare il record esistente**

1. Assicurati che il tuo file includa un identificatore univoco (`email`, `domain` o `id`)
2. Mappa il campo identificatore univoco
3. ClientVault aggiornerà il record esistente invece di creare un duplicato
...
<Note>
  ...
  * Se mantieni l'`email` ma cambi l'`id` → l'id verrà aggiornato
  ...
  Finché almeno un identificatore univoco corrisponde, ClientVault aggiorna il record.
</Note>
...
3. Verifica se i record esistono già in ClientVault
...
Ciò significa che non esiste alcuna Azienda in ClientVault con quello specifico identificatore.
...
Stai tentando di collegarti a un record che non esiste in ClientVault.
...
Una colonna del tuo file non può essere mappata perché il campo non esiste in ClientVault.
...
1. **L'utente non ha accettato l'invito** — l'utente non esiste ancora in ClientVault
2. **Uso dell'ID utente dal vecchio sistema** — ClientVault non può abbinare gli ID di un altro sistema
3. **Formato email errato** — l'email non corrisponde all'account ClientVault dell'utente

### Come risolvere

1. Assicurati che tutti gli utenti abbiano **accettato il loro invito** al tuo spazio di lavoro ClientVault
2. Usa l'**indirizzo email** dell'utente (non il nome o l'ID del vecchio sistema)
3. Usa la stessa email che hanno utilizzato per aderire a ClientVault
...
<Check>Il formato del dominio corrisponde esattamente al formato di ClientVault</Check>
...
1. **Scarica il file di esempio** — vedi il formato esatto che ClientVault si aspetta
