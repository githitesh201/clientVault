...
1. ClientVaultを実行しているホスト上のターミナルで、ClientVaultを停止します：`docker compose down`
...
3. `docker compose up -d` でClientVaultを再起動します。
...
アップグレード後にランタイムの問題が発生した場合は、キャッシュをフラッシュして最新の変更と同期させる必要があるかもしれません。 clientvault-serverコンテナ内で次のコマンドを実行します： clientvault-serverコンテナ内で次のコマンドを実行します：
...
ClientVaultインスタンスをv0.55イメージにアップグレードします。
...
あなたの `clientvault-server` コンテナ内で次のコマンドを実行します：

```bash
yarn command:prod cache:flush
```

この問題は特定のClientVaultバージョンに固有であり、今後のアップグレードでは不要なはずです。
...
それは常にあなたの現在のClientVaultのインスタンスの `major.minor` バージョンの範囲内である必要があります。あなたのインスタンスバージョンは、管理パネル（ `/settings/admin-panel` にあり、データベース内でユーザーが `canAccessFullAdminPanel` プロパティを持っている場合にアクセス可能）にまたは `clientvault-server` コンテナ内で `echo $APP_VERSION` を実行することで確認できます。

非同期化されたワークスペースバージョンを修正するには、対応するClientVaultのバージョンから関連するアップグレードガイドに従って順次アップグレードし、それが目的のバージョンに到達するまで続ける必要があります。
...
ClientVaultインスタンスをv0.52イメージにアップグレードします。
...
残念ながら、`0.52.0` と `0.52.6` はdockerHubから完全に削除されました。
データベースでワークスペースバージョンを手動で `0.51.0` に更新し、clientvaultバージョン `0.52.11` を使用して、その直上のアップグレードガイドに従ってアップグレードする必要があります。
残念ながら、`0.52.0` と `0.52.6` はdockerHubから完全に削除されました。
データベースでワークスペースバージョンを手動で `0.51.0` に更新し、clientvaultバージョン `0.52.11` を使用して、その直上のアップグレードガイドに従ってアップグレードする必要があります。

### v0.50からv0.51

ClientVaultインスタンスをv0.51イメージにアップグレードします。
...
ClientVaultインスタンスをv0.50.0イメージにアップグレードします。
...
ローカルの `docker-compose.yml` を[v0.50.0 docker-compose.yml](https://github.com/clientvaulthq/clientvault/blob/v0.50.0/packages/clientvault-docker/docker-compose.yml)に更新してください。
ローカルの `docker-compose.yml` を[v0.50.0 docker-compose.yml](https://github.com/clientvaulthq/clientvault/blob/v0.50.0/packages/clientvault-docker/docker-compose.yml)に更新してください。
ローカルの `docker-compose.yml` を[v0.50.0 docker-compose.yml](https://github.com/clientvaulthq/clientvault/blob/v0.50.0/packages/clientvault-docker/docker-compose.yml)に更新してください。

### v0.43.0からv0.44.0

ClientVaultインスタンスをv0.44.0イメージにアップグレードします。
...
ClientVaultインスタンスをv0.43.0イメージにアップグレードします。
...
2. docker-compose.ymlを、新しいpostgres:16イメージを使用するようにアップグレードします。[docker-compose.yml](https://raw.githubusercontent.com/clientvaulthq/clientvault/main/packages/clientvault-docker/docker-compose.yml)を参照してください。
...
ClientVaultインスタンスをv0.42.0イメージにアップグレードします。
...
ClientVaultインスタンスをv0.41.0イメージにアップグレードします。
...
ClientVaultインスタンスをv0.40.0イメージにアップグレードします。
...
ClientVaultインスタンスをv0.35.0イメージにアップグレードします。
...
ClientVaultインスタンスをv0.34.0イメージにアップグレードします。
...
ClientVaultインスタンスをv0.33.0イメージにアップグレードします。
...
このバージョンから、DB用のclientvault-postgresイメージは廃止され、clientvault-postgres-spiloが代わりに使用されるようになりました。
このバージョンから、DB用のclientvault-postgresイメージは廃止され、clientvault-postgres-spiloが代わりに使用されるようになりました。
このバージョンから、DB用のclientvault-postgresイメージは廃止され、clientvault-postgres-spiloが代わりに使用されるようになりました。
clientvault-postgresイメージを使い続けたい場合は、docker-compose.yml内の`clientvaultcrm/clientvault-postgres:${TAG}`を`clientvaultcrm/clientvault-postgres`に置き換えるだけで済みます。

### v0.31.0からv0.32.0

ClientVaultインスタンスをv0.32.0イメージにアップグレードします。
...
ClientVaultインスタンスをv0.31.0イメージにアップグレードします。
...
ClientVaultインスタンスをv0.30.0イメージにアップグレードします。

**破壊的な変更**:
パフォーマンスを向上させるために、ClientVaultはRedisキャッシュの設定を必要とするようになりました。 これを反映した[docker-compose.yml](https://raw.githubusercontent.com/clientvaulthq/clientvault/main/packages/clientvault-docker/docker-compose.yml)を更新しました。
...
ClientVaultインスタンスをv0.24.0イメージにアップグレードします。
...
ClientVaultインスタンスをv0.23.0イメージにアップグレードします。
...
ClientVaultインスタンスをv0.22.0イメージにアップグレードします。
