...
### Relações Comuns no ClientVault
...
O pai (lado "um") deve existir no ClientVault antes de poder referenciá-lo.
...
1. Exporte os registos pai do ClientVault
...
<Warning>
  **O formato do domínio é importante!**
  ...
  Use `https://domain.com` (não apenas `domain.com`). Isto corresponde à forma como a ClientVault armazena domínios de Empresas e evita erros de correspondência.
</Warning>
...
A ClientVault ligará automaticamente cada registo filho ao pai correspondente.
...
| **Registos inexistentes**                      | O registo pai não existe no ClientVault               | Importe primeiro os registos pai ou verifique se há erros ortográficos |
| **Incompatibilidade de maiúsculas/minúsculas** | `Acme.com` no ficheiro mas `acme.com` no ClientVault  | Garanta correspondência exata de maiúsculas/minúsculas                 |
...
<AccordionGroup>
  <Accordion title="E se o meu registo pai não tiver um domínio ou email?">
    ...
    1. Use o `id` da ClientVault (exporte os registos pai para obter os respetivos IDs)
