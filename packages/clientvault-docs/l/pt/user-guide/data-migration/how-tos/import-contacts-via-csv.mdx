...
description: Guia completo passo a passo para importar pessoas/contactos para o ClientVault.
---

## Visão Geral

Este guia explica como importar os seus contactos (People) para o ClientVault. **Importe Empresas primeiro** se quiser associar Pessoas a Empresas.
...
* **Deve ser único** — sem duplicados no seu ficheiro ou no ClientVault
...
<Note>
  **Use o formato de URL de domínio** (`https://acme.com`), não o rótulo. Isto corresponde à forma como as Empresas são armazenadas no ClientVault.
</Note>
...
<Note>
  **Dica profissional:** Clique em **Download sample file** durante a importação ou exporte algumas Pessoas existentes para ver os nomes de colunas exatos que o ClientVault espera.
</Note>
...
3. Aguarde que o ClientVault analise o seu ficheiro

## Passo 4: Mapeie as suas colunas

O ClientVault tenta automaticamente corresponder as suas colunas aos campos. Revise e ajuste:
...
3. O ClientVault irá associar cada Pessoa à Empresa correspondente
...
1. O ClientVault exibe seus valores ao lado das opções existentes
2. Relacione cada valor no seu arquivo a uma opção do ClientVault
...
Antes de concluir a importação, o ClientVault valida seus dados:
...
| Erro                           | Causa                                      | Solução                                              |
| ------------------------------ | ------------------------------------------ | ---------------------------------------------------- |
| **E-mail duplicado**           | O e-mail já existe no ClientVault ou no arquivo | Remova o duplicado ou atualize o registro existente  |
...
Seus contatos agora estão no ClientVault! Próximos passos:
...
2. O ClientVault faz a correspondência dos registros por esse identificador único
...
<AccordionGroup>
  <Accordion title="Por que duas Pessoas não podem ter o mesmo e-mail?">
    O e-mail é um identificador único no ClientVault. Isso evita contatos duplicados e garante que a sincronização de e-mail vincule corretamente os e-mails à pessoa certa.
  </Accordion>
  ...
  <Accordion title="Posso importar o ID do meu CRM anterior?">
    Sim! Crie um campo personalizado marcado como "único" no seu modelo de dados para armazenar o ID externo. Observação: o nome do campo `id` é reservado para o ID interno do ClientVault.
  </Accordion>
