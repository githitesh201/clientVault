---
title: Aplicativos ClientVault
description: Crie e gerencie personalizações do ClientVault como código.
---
...
Os aplicativos permitem criar e gerenciar personalizações do ClientVault **como código**. Em vez de configurar tudo pela UI, você define seu modelo de dados e funções de lógica em código — tornando mais rápido criar, manter e distribuir para vários workspaces.
...
* Node.js 24+ e Yarn 4
* Um espaço de trabalho do ClientVault e uma chave de API (crie uma em https://app.clientvault.com/settings/api-webhooks)
...
Veja também: as páginas de referência da CLI para [create-clientvault-app](https://www.npmjs.com/package/create-clientvault-app) e [clientvault-sdk CLI](https://www.npmjs.com/package/clientvault-sdk).

## Estrutura do projeto (com scaffold)

Ao executar `npx create-clientvault-app@latest my-clientvault-app`, o gerador:

* Copia um aplicativo base mínimo para `my-clientvault-app/`
* Adiciona uma dependência local `clientvault-sdk` e a configuração do Yarn 4
* Cria arquivos de configuração e scripts conectados à CLI `clientvault`
...
* **package.json**: Declara o nome do app, versão, engines (Node 24+, Yarn 4), e adiciona `clientvault-sdk` além de um script `clientvault` que delega para a CLI `clientvault` local. Execute `yarn clientvault help` para listar todos os comandos disponíveis.
...
O SDK detecta entidades analisando seus arquivos TypeScript em busca de chamadas **`export default define<Entity>({...})`**. Cada tipo de entidade tem uma função utilitária correspondente exportada de `clientvault-sdk`:
...
* `yarn clientvault app:dev` vai gerar automaticamente um cliente de API tipado em `node_modules/clientvault-sdk/generated` (cliente ClientVault tipado + tipos do espaço de trabalho).
* `yarn clientvault entity:add` adicionará arquivos de definição de entidade em `src/` para seus objetos, funções, componentes de front-end ou papéis personalizados.

## Autenticação

Na primeira vez que você executar `yarn clientvault auth:login`, será solicitado o seguinte:

* URL da API (padrão: http://localhost:3000 ou o perfil do seu espaço de trabalho atual)
* Chave de API

Suas credenciais são armazenadas por usuário em `~/.clientvault/config.json`. Você pode manter vários perfis e alternar entre eles.
...
Depois que você alternar os espaços de trabalho com `yarn clientvault auth:switch`, todos os comandos subsequentes usarão esse espaço de trabalho por padrão. Você ainda pode substituí-lo temporariamente com `--workspace <name>`.

## Use os recursos do SDK (tipos e configuração)

O clientvault-sdk fornece blocos de construção tipados e funções utilitárias que você usa dentro do seu aplicativo. A seguir estão as partes principais que você usará com mais frequência.
...
* Você pode criar novos objetos usando `yarn clientvault entity:add`, que orienta você sobre nomeação, campos e relacionamentos.

<Note>
  **Os campos base são criados automaticamente.** Quando você define um objeto personalizado, o ClientVault adiciona automaticamente campos padrão
...
* A chave de API em tempo de execução, injetada como `CLIENTVAULT_API_KEY`, é derivada desse papel padrão de função.
...
* Veja um exemplo funcional no app Hello World: [`packages/clientvault-apps/hello-world/src/roles/function-role.ts`](https://github.com/clientvaulthq/clientvault/blob/main/packages/clientvault-apps/hello-world/src/roles/function-role.ts).
...
Ao criar a estrutura de um novo app com `create-clientvault-app`, uma função de pós-instalação é gerada para você em `src/logic-functions/post-install.ts`:
...
Quando um gatilho de rota invoca sua função de lógica, ela recebe um objeto `RoutePayload` que segue o formato do AWS HTTP API v2. Importe o tipo de `clientvault-sdk`:
...
* **Gerado automaticamente**: Execute `yarn clientvault entity:add` e escolha a opção para adicionar uma nova função de lógica. Isso gera um arquivo inicial com um handler e configuração.
...
Funções lógicas podem ser expostas como **ferramentas** para agentes de IA e fluxos de trabalho. Quando uma função é marcada como ferramenta, ela fica disponível para os recursos de IA do ClientVault e pode ser selecionada como uma etapa em automações de fluxos de trabalho.
...
Componentes de front-end permitem criar componentes React personalizados que são renderizados na UI do ClientVault. Use `defineFrontComponent()` para definir componentes com validação integrada:
...
* Componentes de front-end são componentes React que renderizam em contextos isolados dentro do ClientVault.
...
* Os componentes são compilados e sincronizados automaticamente durante `yarn clientvault app:dev`.

Você pode criar novos componentes de front-end de duas formas:

* **Gerado automaticamente**: Execute `yarn clientvault entity:add` e escolha a opção para adicionar um novo componente de front-end.
* **Manual**: Crie um novo arquivo `*.front-component.tsx` e use `defineFrontComponent()`.

### Cliente tipado gerado

O cliente tipado é gerado automaticamente pelo `yarn clientvault app:dev` e armazenado em `node_modules/clientvault-sdk/generated` com base no esquema do seu espaço de trabalho. Use-o em suas funções:
...
O cliente é regenerado automaticamente pelo `yarn clientvault app:dev` sempre que seus objetos ou campos forem alterados.

#### Credenciais em tempo de execução em funções de lógica

Quando sua função é executada no ClientVault, a plataforma injeta credenciais como variáveis de ambiente antes da execução do seu código:

* `CLIENTVAULT_API_URL`: URL base da API do ClientVault que seu aplicativo usa como alvo.
* `CLIENTVAULT_API_KEY`: Chave de curta duração com escopo para o papel de função padrão do seu aplicativo.

Notas:

* Você não precisa passar a URL ou a chave de API para o cliente gerado. Ele lê `CLIENTVAULT_API_URL` e `CLIENTVAULT_API_KEY` de process.env em tempo de execução.
...
Explore um exemplo mínimo de ponta a ponta que demonstra objetos, funções de lógica, componentes de front-end e vários gatilhos [aqui](https://github.com/clientvaulthq/clientvault/tree/main/packages/clientvault-apps/hello-world):

## Configuração manual (sem o gerador)

Embora recomendemos usar `create-clientvault-app` para a melhor experiência inicial, você também pode configurar um projeto manualmente. Não instale a CLI globalmente. Em vez disso, adicione `clientvault-sdk` como uma dependência local e configure um único script no seu package.json:

```bash filename="Terminal"
yarn add -D clientvault-sdk
```

Em seguida, adicione um script `clientvault`:
...
Agora você pode executar todos os comandos via `yarn clientvault <command>`, por exemplo, `yarn clientvault app:dev`, `yarn clientvault help`, etc.

## Resolução de Problemas

* Erros de autenticação: execute `yarn clientvault auth:login` e certifique-se de que sua chave de API tenha as permissões necessárias.
* Não é possível conectar ao servidor: verifique a URL da API e se o servidor do ClientVault está acessível.
* Tipos ou cliente ausentes/desatualizados: reinicie `yarn clientvault app:dev` — ele gera automaticamente o cliente tipado.
* Modo de desenvolvimento não sincronizando: certifique-se de que `yarn clientvault app:dev` esteja em execução e de que as alterações não estejam sendo ignoradas pelo seu ambiente.
