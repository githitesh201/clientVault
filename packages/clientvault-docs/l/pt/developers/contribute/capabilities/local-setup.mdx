...
description: O guia para contribuidores (ou desenvolvedores curiosos) que desejam executar o ClientVault localmente.
---
...
<Tabs>
  <Tab title="Linux e MacOS">
    Antes de instalar e usar o ClientVault, certifique-se de instalar o seguinte em seu computador:
...
O ClientVault requer um cache redis para oferecer o melhor desempenho
...
<Info>
  **Modo Multiworkspace:** Por padrão, o ClientVault é executado no modo de workspace único, em que apenas um workspace pode ser criado. Para ativar o suporte a multiworkspace (útil para testar recursos baseados em subdomínio), defina `IS_MULTIWORKSPACE_ENABLED=true` no arquivo `.env` do seu servidor. Veja [Modo Multiworkspace](/l/pt/developers/self-host/capabilities/setup#multi-workspace-mode) para obter detalhes.
</Info>

## Passo 6: Instalando dependências

Para compilar o servidor ClientVault e popular seu banco de dados com alguns dados, execute o seguinte comando:
...
## Passo 8: Use ClientVault

**Frontend**

O frontend do ClientVault estará em execução em [http://localhost:3001](http://localhost:3001).
Você pode fazer login usando a conta demo padrão: `tim@apple.dev` (senha: `tim@apple.dev`)

**Backend**

* O servidor do ClientVault estará ativo em [http://localhost:3000](http://localhost:3000)
