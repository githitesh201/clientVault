...
```bash
npx nx run clientvault-front:lint # pass --fix to fix lint errors
```
...
```bash
npx nx run clientvault-front:test # run jest tests
npx nx run clientvault-front:storybook:serve:dev # run storybook
npx nx run clientvault-front:storybook:test # run tests # (needs yarn storybook:serve:dev to be running)
npx nx run clientvault-front:storybook:coverage # (needs yarn storybook:serve:dev to be running)
```
