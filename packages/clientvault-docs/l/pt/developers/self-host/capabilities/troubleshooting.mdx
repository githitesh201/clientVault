...
Se você tiver uma instância existente do ClientVault com dados em produção, **NÃO** siga estes passos, pois eles excluirão permanentemente seu banco de dados!

Ao instalar o ClientVault pela primeira vez, você pode querer alterar a senha padrão do banco de dados.
...
Durante a instalação do ClientVault, é necessário configurar seu banco de dados postgres com os esquemas, extensões e usuários corretos.
...
#### Não é possível encontrar o módulo 'clientvault-emails' ou suas declarações de tipo correspondentes.

É preciso compilar o pacote `clientvault-emails` antes de iniciar a inicialização do banco de dados com `npx nx run clientvault-emails:build`

#### Pacote clientvault-x ausente

Certifique-se de executar o yarn no diretório raiz e depois executar `npx nx server:dev clientvault-server`. Se isso ainda não funcionar, tente compilar manualmente o pacote ausente.
...
#### Ao executar `npx nx start` ou `npx nx start clientvault-front`, é lançada uma mensagem de erro de falta de memória

No `packages/clientvault-front/.env` descomente `VITE_DISABLE_TYPESCRIPT_CHECKER=true` para desativar verificações em segundo plano, reduzindo assim a quantidade de RAM necessária.

**Se não funcionar:**
Execute apenas os serviços que precisar, em vez de `npx nx start`. Por exemplo, se estiver trabalhando no servidor, execute apenas `npx nx worker clientvault-server`

**Se não funcionar:**
Se você tentou executar apenas `npx nx run clientvault-server:start` no WSL e está falhando com o erro de memória abaixo:
...
**Se não funcionar:**
Investigue quais processos estão consumindo a maior parte da RAM do seu computador. No ClientVault, percebemos que algumas extensões do VScode estavam consumindo muita RAM, então as desativamos temporariamente.
...
* Abra seu aplicativo ClientVault, navegue até `/settings`, e ative a alternância **Avançado** na parte inferior esquerda da tela.
* Crie um novo webhook.
* Cole **Sua URL Webhook Única** no campo **Endpoint Url** no ClientVault. Defina os **Filtros** para `Companies` e `Created`.
...
Comente o plugin checker em `packages/clientvault-ui/vite-config.ts` como no exemplo abaixo
...
Se você está executando o ClientVault por trás de um proxy reverso e está enfrentando problemas de conexão:
...
2. **Verifique as configurações do Proxy Reverso:**
   ...
   * Confirme que seu proxy reverso está encaminhando corretamente as solicitações para o servidor ClientVault.
   * Certifique-se de que cabeçalhos como `X-Forwarded-For` e `X-Forwarded-Proto` estão configurados corretamente.

3. **Reinicie os Serviços:**

   Após fazer as alterações, reinicie tanto o proxy reverso quanto os contêineres do ClientVault.
...
* Suporte Comunitário:
  ...
  Entre em contato com a [comunidade ClientVault](https://github.com/clientvaulthq/clientvault/issues) ou [canais de suporte](https://discord.gg/cx5n4Jzs57) para obter assistência.
