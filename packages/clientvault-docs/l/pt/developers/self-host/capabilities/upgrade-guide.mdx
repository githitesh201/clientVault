...
1. No terminal, no host onde o ClientVault está em execução, desligue o ClientVault: `docker compose down`
...
3. Traga o ClientVault de volta ao ar com `docker compose up -d`
...
Se você encontrar problemas de execução após a atualização, pode ser necessário limpar seu cache para garantir que esteja sincronizado com as alterações mais recentes. Execute este comando em seu contêiner do clientvault-server:
...
Atualize sua instância do ClientVault para usar a imagem v0.55
...
Em seu contêiner `clientvault-server`, execute:

```bash
yarn command:prod cache:flush
```

Este problema é específico para esta versão do ClientVault e não deverá ser necessário em futuras atualizações.
...
Deve estar sempre na faixa da versão `major.minor` atual da instância do ClientVault, você pode ver a versão de sua instância no painel de administração (em `/settings/admin-panel`, acessível se seu usuário tiver a propriedade `canAccessFullAdminPanel` definida como verdadeira no banco de dados) ou executando `echo $APP_VERSION` em seu contêiner `clientvault-server`.

Para corrigir uma versão de workspace dessincronizada, você terá que atualizar da versão correspondente do clientvault seguindo o guia de atualização relacionado sequencialmente e assim por diante até alcançar a versão desejada.
...
Atualize sua instância do ClientVault para usar a imagem v0.52
...
Você terá que atualizar manualmente a versão do espaço de trabalho para `0.51.0` no banco de dados e atualizar usando a versão clientvault `0.52.11` seguindo o guia de atualização logo acima.

### v0.50 para v0.51

Atualize sua instância do ClientVault para usar a imagem v0.51
...
Atualize sua instância do ClientVault para usar a imagem v0.50.0
...
Por favor, atualize seu `docker-compose.yml` local com [v0.50.0 docker-compose.yml](https://github.com/clientvaulthq/clientvault/blob/v0.50.0/packages/clientvault-docker/docker-compose.yml)

### v0.43.0 para v0.44.0

Atualize sua instância do ClientVault para usar a imagem v0.44.0
...
Atualize sua instância do ClientVault para usar a imagem v0.43.0
...
2. Atualize seu docker-compose.yml para usar a imagem postgres:16 como no arquivo [docker-compose.yml](https://raw.githubusercontent.com/clientvaulthq/clientvault/main/packages/clientvault-docker/docker-compose.yml).
...
Atualize sua instância do ClientVault para usar a imagem v0.42.0
...
Atualize sua instância do ClientVault para usar a imagem v0.41.0
...
Atualize sua instância do ClientVault para usar a imagem v0.40.0
...
Atualize sua instância do ClientVault para usar a imagem v0.35.0
...
Atualize sua instância do ClientVault para usar a imagem v0.34.0
...
Atualize sua instância do ClientVault para usar a imagem v0.33.0
...
A partir desta versão, a imagem clientvault-postgres para DB tornou-se obsoleta e o clientvault-postgres-spilo é usado em vez disso.
Se você quiser continuar usando a imagem clientvault-postgres, basta substituir `clientvaultcrm/clientvault-postgres:${TAG}` por `clientvaultcrm/clientvault-postgres` em docker-compose.yml.

### v0.31.0 para v0.32.0

Atualize sua instância do ClientVault para usar a imagem v0.32.0
...
Atualize sua instância do ClientVault para usar a imagem v0.31.0
...
Atualize sua instância do ClientVault para usar a imagem v0.30.0

**Mudança radical**:
Para melhorar o desempenho, o ClientVault agora requer que o cache redis seja configurado. Atualizamos nosso [docker-compose.yml](https://raw.githubusercontent.com/clientvaulthq/clientvault/main/packages/clientvault-docker/docker-compose.yml) para refletir isso.
...
Atualize sua instância do ClientVault para usar a imagem v0.24.0
...
Atualize sua instância do ClientVault para usar a imagem v0.23.0
...
Atualize sua instância do ClientVault para usar a imagem v0.22.0
