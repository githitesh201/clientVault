...
Nadřazený („jedna“ strana) musí v ClientVault existovat, než na něj budete moci odkazovat.
...
1. Exportujte nadřazené záznamy z ClientVault
...
<Warning>
  **Na formátu domény záleží!**
  ...
  Použijte `https://domain.com` (ne pouze `domain.com`). To odpovídá způsobu, jak ClientVault ukládá domény společností, a předchází chybám při párování.
</Warning>
...
ClientVault automaticky propojí každý podřízený záznam s odpovídajícím nadřazeným.
...
| **Neexistující záznamy**     | Nadřazený záznam v ClientVault neexistuje            | Nejprve importujte nadřazené záznamy nebo zkontrolujte překlepy |
| **Rozdílná velikost písmen** | `Acme.com` v souboru, ale v ClientVault `acme.com`   | Zajistěte přesnou shodu velikosti písmen                        |
...
<AccordionGroup>
  <Accordion title="Co když můj nadřazený záznam nemá doménu ani e-mail?">
    ...
    1. Použijte ClientVault `id` (exportujte nadřazené záznamy, abyste získali jejich ID)
