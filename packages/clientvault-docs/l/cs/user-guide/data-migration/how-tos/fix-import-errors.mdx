...
Po nahrání souboru a namapování sloupců ClientVault vaše data ověří:

1. **Proběhne validace** — ClientVault zkontroluje každý řádek na chyby
...
| --------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| Duplicitní hodnoty zvýrazněné žlutě                                   | Hodnota již existuje v ClientVault nebo se ve vašem souboru vyskytuje dvakrát |
...
**Možnost 3: Nechte ClientVault aktualizovat existující záznam**
...
3. ClientVault aktualizuje existující záznam místo vytvoření duplicity
...
<Note>
  ...
  * Pokud ponecháte `email`, ale změníte `id` → ID bude aktualizováno
  ...
  Pokud se shoduje alespoň jeden jedinečný identifikátor, ClientVault záznam aktualizuje.
</Note>
...
3. Zkontrolujte, zda záznamy v ClientVault již existují
...
To znamená, že v ClientVault neexistuje společnost s tímto konkrétním identifikátorem.
...
Pokoušíte se propojit se záznamem, který v ClientVault neexistuje.
...
Sloupec ve vašem souboru nelze namapovat, protože pole v ClientVault neexistuje.
...
1. **Uživatel nepřijal pozvánku** — uživatel zatím v ClientVault neexistuje
2. **Používáte uživatelské ID ze starého systému** — ClientVault nedokáže spárovat ID z jiného systému
3. **Nesprávný formát e-mailu** — e-mail neodpovídá účtu uživatele v ClientVault

### Jak to opravit

1. Ujistěte se, že všichni uživatelé **přijali svou pozvánku** do vašeho pracovního prostoru ClientVault
2. Použijte **e-mailovou adresu** uživatele (ne jeho jméno nebo ID ze starého systému)
3. Použijte stejný e-mail, jaký použili k připojení do ClientVault
...
<Check>Formát domény přesně odpovídá formátu ClientVault</Check>
...
1. **Stáhněte si ukázkový soubor** — uvidíte přesný formát, který ClientVault očekává
