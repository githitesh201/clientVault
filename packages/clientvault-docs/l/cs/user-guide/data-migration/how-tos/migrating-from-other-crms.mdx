...
description: Podrobný průvodce krok za krokem pro migraci vašich dat z libovolného CRM do ClientVault.
---

## Přehled

Tento průvodce vás provede migrací vašich dat z libovolného CRM do ClientVault. Proces zahrnuje audit vašich dat, přípravu vašeho pracovního prostoru ClientVault, export z vašeho současného systému a import do ClientVault.
...
Vytvořte mapovací dokument mezi vaším současným CRM a ClientVault:

| Vaše CRM             | ClientVault                     |
| -------------------- | -------------------------- |
...
## Krok 3: Nastavte svůj pracovní prostor ClientVault

Před importem dat připravte svůj pracovní prostor ClientVault:
...
<Warning>
  **Pozvěte uživatele PŘED importem dat.**
  ...
  Pokud vaše data obsahují odkazy na uživatele (vlastník účtu, přiřazený uživatel apod.), tito uživatelé musí v ClientVault existovat před importem. Jinak tyto relace nelze namapovat.
</Warning>
...
Otevřete každý exportovaný soubor CSV v tabulkovém editoru a připravte jej pro ClientVault.

### Odstraňte duplicity

1. Seřaďte podle jedinečného pole (e-mail u lidí, doména u společností)
2. Odstraňte nebo sloučte duplicitní řádky
3. Ověřte, že v ClientVault již neexistují žádné duplicity
...
<Warning>
  **Formát domény je zásadní.**
  ...
  Použijte `https://domain.com` (ne `domain.com` ani `www.domain.com`). To odpovídá formátu ClientVault a předchází duplicitám, když připojíte synchronizaci e-mailu/kalendáře.
</Warning>
...
2. Použijte stejné e-mailové adresy, které uživatelé použili při připojení do vašeho pracovního prostoru ClientVault
...
1. Přejděte k objektu v ClientVault
...
* ClientVault začne synchronizovat e-maily k záznamům kontaktů
...
<AccordionGroup>
  <Accordion title="Mohu migrovat pracovní postupy automaticky?">
    V tuto chvíli ne. Pracovní postupy je nutné v ClientVault znovu vytvořit ručně.
  </Accordion>
...
Kontaktujte [contact@clientvault.com](mailto:contact@clientvault.com) nebo si prohlédněte naše [Implementační služby](/l/cs/user-guide/getting-started/capabilities/implementation-services).
