---
title: Glosář
description: Seznamte se se základní terminologií používanou v aplikaci ClientVault.
---

## API

API (rozhraní pro programování aplikací) umožňuje propojení ClientVault s dalšími softwarovými systémy a vytváření vlastních integrací.

## Aplikace

Aplikace jsou vlastní rozšíření vytvořená jako kód, která mohou definovat datové modely a logické funkce. Umožňují vývojářům vytvářet znovupoužitelné přizpůsobení, které lze nasadit do více pracovních prostorů.

## Akce kódu

Akce kódu jsou kroky pracovního postupu, které vám umožní psát vlastní JavaScript pro transformaci dat, provádět výpočty nebo realizovat složitou logiku, kterou není možné dosáhnout pomocí vestavěných akcí.

## Nabídka příkazů

Nabídka příkazů je rozhraní pro rychlý přístup (otevírá se `Cmd + K` na Macu a `Ctrl + K` na Windows), které vám umožní provádět akce, vytvářet záznamy a efektivně se pohybovat ve vašem pracovním prostoru.

## Společnost & Lidé

CRM má dva základní typy záznamů:

* "Společnost" představuje podnik nebo organizaci.
* "Lidé" představují vaše aktuální a potenciální zákazníky nebo klienty.

## Vlastní pole

Vlastní pole jsou datová pole, která vytváříte k zachycení informací specifických pro vaše obchodní potřeby a procesy.

## Datový model

Datový model je struktura, která definuje, jak jsou informace v CRM uspořádány, včetně toho, jaké objekty existují, jejich vlastnosti (pole) a jak se navzájem vztahují.

## Oblíbené

Oblíbené jsou záznamy, které jste označili pro rychlý přístup, objevují se na postranní liště pro okamžitou navigaci k důležitým datům.

## Pole

Pole představuje konkrétní oblast, kde jsou u entity uložena určitá data.

## Iterátor

Iterátor je akce pracovního postupu, která prochází pole položek a pro každou položku v seznamu spouští následující akce.

## Kanban

"Kanban" je vizuální způsob sledování obchodních procesů pomocí karet a sloupců. Každý sloupec představuje fázi vašeho procesu (například: nové, probíhající, vyhrané, ztracené) a záznamy přesouváte prostřednictvím těchto fází podle postupu.

## Objekt

Objekt je datová struktura, která představuje specifický typ entity ve vašem CRM (např. Lidé, Společnosti nebo Příležitosti). Objekty mohou být standardní (vestavěné) nebo vlastní (vytvořené vámi).

## Příležitosti

Příležitosti v ClientVault CRM jsou potenciální obchody nebo prodeje s účty či kontakty.

## Záznam

Záznam označuje instanci objektu, například konkrétní účet nebo kontakt.

## Relační pole

Relační pole vytvářejí propojení mezi různými objekty, což umožňuje propojovat záznamy mezi sebou (například propojit osobu se společností).

## Standardní pole

Standardní pole jsou předem vytvořená datová pole, která jsou s objekty přítomná již ve výchozím stavu a poskytují běžnou funkčnost v rámci všech pracovních prostorů.

## Úkoly

Úkoly v ClientVault CRM jsou přiřazené aktivity týkající se kontaktů, účtů nebo příležitostí.

## Spouštěče

Spouštěče jsou výchozím bodem pracovního postupu — událost nebo podmínka, která spustí automatizaci. Patří sem například vytvoření záznamu, aktualizace záznamu, webhooky nebo naplánované časy.

## Pohledy

Zobrazení záznamů si můžete přizpůsobit pomocí pohledů a pro každý pohled nastavit různé filtry, rozvržení a možnosti řazení.

## Upsert

Upsert je operace, která kombinuje "update" a "insert" — pokud se najde shoda, aktualizuje existující záznam, nebo vytvoří nový záznam, pokud shoda neexistuje.

## Webhooky

Webhooky jsou automatizované zprávy odesílané z ClientVault na jiné aplikace při výskytu specifických událostí, což umožňuje synchronizaci dat v reálném čase.

## Pracovní postupy

Pracovní postupy jsou automatizované procesy, které spouštějí akce na základě specifických podmínek a pomáhají vám automatizovat opakující se úkoly a obchodní procesy.

## Pracovní prostor

"Pracovní prostor" obvykle představuje společnost používající ClientVault. Obsahuje všechny záznamy a data, která vy a členové vašeho týmu do ClientVault přidáte.
Má jedno doménové jméno, kterým je obvykle doména, kterou vaše společnost používá pro e-mailové adresy zaměstnanců.

## Členové pracovního prostoru

Členové pracovního prostoru jsou uživatelé ClientVault z vašeho týmu, kteří mají přístup k vašemu pracovnímu prostoru. Mohou být u záznamů přiřazeni jako vlastníci nebo přiřazené osoby.
