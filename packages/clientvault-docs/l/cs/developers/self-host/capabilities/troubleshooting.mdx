...
Pokud máte existující instanci ClientVault s produkčními daty, **NEPROVÁDĚJTE** tyto kroky, protože trvale smažou vaši databázi!

Při první instalaci ClientVault můžete chtít změnit výchozí heslo pro databázi.
...
Během instalace ClientVault musíte nastavit svou postgres databázi s správnými schématy, rozšířeními a uživateli.
...
#### Nelze najít modul 'clientvault-emails' ani jeho odpovídající deklarace typů.

Před spuštěním inicializace databáze musíte sestavit balíček `clientvault-emails` s `npx nx run clientvault-emails:build`

#### Chybí balíček clientvault-x

Ujistěte se, že v kořenovém adresáři spouštíte `yarn` a poté spusťte `npx nx server:dev clientvault-server`. Pokud stále nefunguje, zkuste chybějící balíček sestavit ručně.
...
#### Při spuštění `npx nx start` nebo `npx nx start clientvault-front` došlo k chybě nedostatku paměti

In `packages/clientvault-front/.env` uncomment `VITE_DISABLE_TYPESCRIPT_CHECKER=true` and `VITE_DISABLE_ESLINT_CHECKER=true` to disable background checks thus reducing amount of needed RAM.

**Pokud to nefunguje:**
Spusťte pouze služby, které potřebujete, místo `npx nx start`. Například pokud pracujete na serveru, spusťte pouze `npx nx worker clientvault-server`

**Pokud to nefunguje:**
Pokud jste se pokusili spustit pouze `npx nx run clientvault-server:start` na WSL a selhává to s následující chybou paměti:
...
**Pokud to nefunguje:**
Analyzujte, které procesy vám zabírají většinu paměti na stroji. V ClientVault jsme si všimli, že některá rozšíření VScode zabírala hodně paměti RAM, takže jsme je dočasně deaktivovali.
...
* Otevřete svou ClientVault aplikaci, přejděte na `/settings` a na levé dolní části obrazovky aktivujte přepínač **Pokročilé**.
* Vytvořte nový webhook.
* Vložte **Svou unikátní webhook URL** do pole **Endpoint Url** ve ClientVault. Nastavte **Filtry** na `Companies` a `Created`.
...
Zakomentujte plugin checker v `packages/clientvault-ui/vite-config.ts` jako v příkladu níže
...
Pokud provozujete ClientVault za reverzní proxy a máte problémy s připojením:
...
2. **Zkontrolujte nastavení reverzní proxy:**
   ...
   * Potvrďte, že vaše reverzní proxy správně směruje požadavky na server ClientVault.
   * Ujistěte se, že záhlaví jako `X-Forwarded-For` a `X-Forwarded-Proto` jsou správně nastaveny.

3. **Restartujte služby:**

   Po provedení změn restartujte jak reverzní proxy, tak kontejnery ClientVault.
...
* Podpora komunity:
  ...
  Obraťte se na [komunitu ClientVault](https://github.com/clientvaulthq/clientvault/issues) nebo [podpůrné kanály](https://discord.gg/cx5n4Jzs57) pro pomoc.
