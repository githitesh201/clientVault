...
1. V terminálu, na hostiteli, kde ClientVault běží, vypněte ClientVault: `docker compose down`
...
3. Opětovně zapněte ClientVault pomocí `docker compose up -d`
...
Pokud po upgradu narazíte na problémy s výkonem, může být nutné vyprázdnit cache, aby bylo zajištěno její sladění s nejnovějšími změnami. Spusťte tento příkaz v kontejneru clientvault-server:
...
Upgradujte svou instanci ClientVault pro použití v0.55 image
...
Ve svém kontejneru `clientvault-server` spusťte:

```bash
yarn command:prod cache:flush
```

Tento problém je specifický pro tuto verzi ClientVault a neměl by být vyžadován pro budoucí upgrady.
...
Měla by se vždy nacházet v rozmezí vaší aktuální instance ClientVault `major.minor` verze. Můžete zkontrolovat verzi instance na ovládacím panelu administrátora (na `/settings/admin-panel`, přístupné, pokud máte v databázi nastaveno uživatelské vlastnosti `canAccessFullAdminPanel` na true) nebo spuštěním `echo $APP_VERSION` ve vašem kontejneru `clientvault-server`.

Chcete-li opravit asynchronizaci verzí pracovního prostoru, budete muset upgradovat z odpovídající verze clientvault podle souvisejícího průvodce upgradem po jednotlivých krocích, až do dosažení požadované verze.
...
Upgradujte svou instanci ClientVault pro použití v0.52 image
...
Budete muset ručně změnit verzi pracovního prostoru na `0.51.0` v databázi a upgradovat pomocí clientvault verze `0.52.11` podle jejího výše uvedeného průvodce upgradem.

### v0.50 až v0.51

Upgradujte svou instanci ClientVault pro použití v0.51 image
...
Upgradujte svou instanci ClientVault pro použití v0.50.0 image
...
Aktualizujte svůj místní `docker-compose.yml` pomocí [docker-compose.yml v0.50.0](https://github.com/clientvaulthq/clientvault/blob/v0.50.0/packages/clientvault-docker/docker-compose.yml)

### v0.43.0 až v0.44.0

Upgradujte svou instanci ClientVault pro použití v0.44.0 image
...
Upgradujte svou instanci ClientVault pro použití v0.43.0 image
...
2. Upgradujte svůj `docker-compose.yml` na použití obrazu postgres:16 podle [docker-compose.yml](https://raw.githubusercontent.com/clientvaulthq/clientvault/main/packages/clientvault-docker/docker-compose.yml) soubor.
...
Upgradujte svou instanci ClientVault pro použití v0.42.0 image
...
Upgradujte svou instanci ClientVault pro použití v0.41.0 image
...
Upgradujte svou instanci ClientVault pro použití v0.40.0 image
...
Upgradujte svou instanci ClientVault pro použití v0.35.0 image
...
Upgradujte svou instanci ClientVault pro použití v0.34.0 image
...
Upgradujte svou instanci ClientVault pro použití v0.33.0 image
...
Od této verze se obraz clientvault-postgres pro DB stal zastaralým a místo něj se používá clientvault-postgres-spilo.
Pokud chcete pokračovat v používání obrazu clientvault-postgres, jednoduše nahraďte `clientvaultcrm/clientvault-postgres:${TAG}` za `clientvaultcrm/clientvault-postgres` v docker-compose.yml.

### v0.31.0 až v0.32.0

Upgradujte svou instanci ClientVault pro použití v0.32.0 image
...
Upgradujte svou instanci ClientVault pro použití v0.31.0 image
...
Upgradujte svou instanci ClientVault pro použití v0.30.0 image

**Změna, která způsobí nekompatibilitu**:
Pro zvýšení výkonu nyní vyžaduje ClientVault konfiguraci cache Redis. Aktualizovali jsme náš [docker-compose.yml](https://raw.githubusercontent.com/clientvaulthq/clientvault/main/packages/clientvault-docker/docker-compose.yml), aby to odrážel.
...
Upgradujte svou instanci ClientVault pro použití v0.24.0 image
...
Upgradujte svou instanci ClientVault pro použití v0.23.0 image
...
Upgradujte svou instanci ClientVault pro použití v0.22.0 image
