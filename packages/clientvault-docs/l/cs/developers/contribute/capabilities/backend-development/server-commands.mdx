...
Tyto příkazy by měly být vykonávány z adresáře packages/clientvault-server.
Z jakéhokoli jiného adresáře můžete spustit `npx nx {command} clientvault-server` (nebo `npx nx run clientvault-server:{command}`).
...
```
npx nx run clientvault-server:lint # přidejte --fix pro opravu chyb ve formátování
```

### Testovat

```
npx nx run clientvault-server:test:unit          # spuštění jednotkových testů
npx nx run clientvault-server:test:integration   # spuštění integračních testů
```

Poznámka: můžete spustit `npx nx run clientvault-server:test:integration:with-db-reset`, pokud potřebujete před spuštěním integračních testů obnovit databázi.
...
ClientVault primárně používá NestJS pro backend.
