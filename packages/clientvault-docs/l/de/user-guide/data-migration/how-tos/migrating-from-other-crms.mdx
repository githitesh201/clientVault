...
description: Schritt-für-Schritt-Anleitung zur Migration Ihrer Daten von jedem CRM zu ClientVault.
---

## Übersicht

Diese Anleitung führt Sie durch die Migration Ihrer Daten von jedem CRM zu ClientVault. Der Prozess umfasst die Überprüfung Ihrer Daten, die Vorbereitung Ihres ClientVault-Arbeitsbereichs, den Export aus Ihrem aktuellen System und den Import in ClientVault.
...
Erstellen Sie ein Zuordnungsdokument zwischen Ihrem aktuellen CRM und ClientVault:
...
## Schritt 3: Richten Sie Ihren ClientVault-Arbeitsbereich ein

Bereiten Sie Ihren ClientVault-Arbeitsbereich vor, bevor Sie Daten importieren:
...
<Warning>
  **Laden Sie Benutzer ein, BEVOR Sie Daten importieren.**
  ...
  Wenn Ihre Daten Benutzerreferenzen enthalten (Account Owner, Assignee usw.), müssen diese Benutzer vor dem Import in ClientVault existieren. Andernfalls können diese Beziehungen nicht zugeordnet werden.
</Warning>
...
Öffnen Sie jede exportierte CSV in einer Tabellenkalkulationsanwendung und bereiten Sie sie für ClientVault vor.
...
3. Überprüfen Sie, dass in ClientVault keine Duplikate vorhanden sind
...
<Warning>
  **Das Domain-Format ist entscheidend.**
  ...
  Verwenden Sie `https://domain.com` (nicht `domain.com` oder `www.domain.com`). Dies entspricht dem Format von ClientVault und verhindert Duplikate, wenn Sie die E-Mail-/Kalendersynchronisierung verbinden.
</Warning>
...
2. Verwenden Sie dieselben E-Mail-Adressen, mit denen die Benutzer Ihrem ClientVault-Arbeitsbereich beigetreten sind
...
## Schritt 6: In ClientVault importieren
...
1. Navigieren Sie zum Objekt in ClientVault
...
* ClientVault beginnt, E-Mails mit Kontaktdatensätzen zu synchronisieren
...
<AccordionGroup>
  <Accordion title="Kann ich Workflows automatisch migrieren?">
    Derzeit nicht. Workflows müssen in ClientVault manuell neu erstellt werden.
  </Accordion>
...
Kontaktieren Sie [contact@clientvault.com](mailto:contact@clientvault.com) oder informieren Sie sich über unsere [Implementierungsservices](/l/de/user-guide/getting-started/capabilities/implementation-services).
