...
Nachdem Sie Ihre Datei hochgeladen und die Spalten zugeordnet haben, validiert ClientVault Ihre Daten:

1. **Validierung läuft** — ClientVault prüft jede Zeile auf Fehler
...
| ------------------------------------------------------------------------------ | ---------------------------------------------------------------------- |
| Doppelte Werte, gelb hervorgehoben                                             | Wert existiert bereits in ClientVault oder kommt in Ihrer Datei doppelt vor |
...
* Existiert bereits in ClientVault
...
**Option 3: ClientVault den vorhandenen Datensatz aktualisieren lassen**
...
3. ClientVault aktualisiert den vorhandenen Datensatz, statt ein Duplikat zu erstellen
...
<Note>
  ...
  * Wenn Sie die `email` beibehalten, aber die `id` ändern → die ID wird aktualisiert
  ...
  Solange ein eindeutiger Bezeichner übereinstimmt, aktualisiert ClientVault den Datensatz.
</Note>
...
3. Prüfen Sie, ob Datensätze bereits in ClientVault existieren
...
Das bedeutet, dass es in ClientVault kein Unternehmen mit diesem spezifischen Bezeichner gibt.
...
Sie versuchen, auf einen Datensatz zu verweisen, der in ClientVault nicht existiert.
...
Eine Spalte in Ihrer Datei kann nicht zugeordnet werden, weil das Feld in ClientVault nicht existiert.
...
1. **Benutzer hat seine Einladung nicht angenommen** — der Benutzer existiert noch nicht in ClientVault
2. **Benutzer-ID aus altem System verwendet** — ClientVault kann IDs aus einem anderen System nicht zuordnen
3. **Falsches E-Mail-Format** — die E-Mail entspricht nicht dem ClientVault-Konto des Benutzers

### So beheben Sie den Fehler

1. Stellen Sie sicher, dass alle Benutzer **ihre Einladung** zu Ihrem ClientVault-Arbeitsbereich **angenommen haben**
2. Verwenden Sie die **E-Mail-Adresse** des Benutzers (nicht seinen Namen oder die alte System-ID)
3. Verwenden Sie dieselbe E-Mail, mit der sie ClientVault beigetreten sind
...
<Check>Domain-Format entspricht exakt dem Format von ClientVault</Check>
...
1. **Beispieldatei herunterladen** — sehen Sie das genaue Format, das ClientVault erwartet
