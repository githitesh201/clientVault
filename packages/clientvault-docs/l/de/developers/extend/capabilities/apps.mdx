...
description: ClientVault-Anpassungen als Code erstellen und verwalten.
---
...
Mit Apps können Sie ClientVault-Anpassungen **als Code** erstellen und verwalten. Anstatt alles über die UI zu konfigurieren, definieren Sie Ihr Datenmodell und Logikfunktionen im Code — das beschleunigt Entwicklung, Wartung und den Rollout auf mehrere Workspaces.
...
* Ein ClientVault-Workspace und ein API-Schlüssel (unter https://app.clientvault.com/settings/api-webhooks erstellen)
...
Siehe auch: die CLI-Referenzseiten für [create-clientvault-app](https://www.npmjs.com/package/create-clientvault-app) und [clientvault-sdk CLI](https://www.npmjs.com/package/clientvault-sdk).

## Projektstruktur (vom Scaffolder erzeugt)

Wenn Sie `npx create-clientvault-app@latest my-clientvault-app` ausführen, erledigt der Scaffolder Folgendes:

* Kopiert eine minimale Basisanwendung nach `my-clientvault-app/`
* Fügt eine lokale `clientvault-sdk`-Abhängigkeit und die Yarn-4-Konfiguration hinzu
* Erstellt Konfigurationsdateien und Skripte, die an die `clientvault`-CLI angebunden sind
...
* **package.json**: Deklariert den App-Namen, die Version und die Engines (Node 24+, Yarn 4) und fügt `clientvault-sdk` sowie ein `clientvault`-Skript hinzu, das an die lokale `clientvault`-CLI delegiert. Führe `yarn clientvault help` aus, um alle verfügbaren Befehle aufzulisten.
...
Das SDK erkennt Entitäten, indem es Ihre TypeScript-Dateien nach Aufrufen von **`export default define<Entity>({...})`** parst. Für jeden Entitätstyp gibt es eine entsprechende Hilfsfunktion, die aus `clientvault-sdk` exportiert wird:
...
* `yarn clientvault app:dev` generiert automatisch einen typisierten API-Client in `node_modules/clientvault-sdk/generated` (typisierter ClientVault-Client + Arbeitsbereichs-Typen).
* `yarn clientvault entity:add` fügt unter `src/` Entitätsdefinitionsdateien für benutzerdefinierte Objekte, Funktionen, Frontend-Komponenten oder Rollen hinzu.

## Authentifizierung

Wenn Sie `yarn clientvault auth:login` zum ersten Mal ausführen, werden Sie nach Folgendem gefragt:

* API-URL (standardmäßig http://localhost:3000 oder Ihr aktuelles Workspace-Profil)
* API-Schlüssel

Ihre Anmeldedaten werden pro Benutzer in `~/.clientvault/config.json` gespeichert. Sie können mehrere Profile verwalten und zwischen ihnen wechseln.
...
Sobald Sie mit `yarn clientvault auth:switch` den Arbeitsbereich gewechselt haben, verwenden alle nachfolgenden Befehle standardmäßig diesen Arbeitsbereich. Sie können es weiterhin vorübergehend mit `--workspace <name>` überschreiben.

## SDK-Ressourcen verwenden (Typen & Konfiguration)

Das clientvault-sdk stellt typisierte Bausteine und Hilfsfunktionen bereit, die Sie in Ihrer App verwenden. Im Folgenden finden Sie die wichtigsten Bausteine, mit denen Sie am häufigsten arbeiten.
...
* Sie können mit `yarn clientvault entity:add` neue Objekte erzeugen; der Assistent führt Sie durch Benennung, Felder und Beziehungen.

<Note>
  **Basisfelder werden automatisch erstellt.** Wenn Sie ein benutzerdefiniertes Objekt definieren, fügt ClientVault automatisch Standardfelder hinzu
...
* Der zur Laufzeit als `CLIENTVAULT_API_KEY` injizierte API-Schlüssel wird von dieser Standard-Funktionsrolle abgeleitet.
...
* Ein funktionierendes Beispiel finden Sie in der Hello-World-App: [`packages/clientvault-apps/hello-world/src/roles/function-role.ts`](https://github.com/clientvaulthq/clientvault/blob/main/packages/clientvault-apps/hello-world/src/roles/function-role.ts).
...
Wenn du mit `create-clientvault-app` eine neue App erstellst, wird für dich eine Post-Installationsfunktion unter `src/logic-functions/post-install.ts` erzeugt:
...
Wenn ein Routen-Trigger Ihre Logikfunktion aufruft, erhält sie ein `RoutePayload`-Objekt, das dem AWS HTTP API v2-Format entspricht. Importieren Sie den Typ aus `clientvault-sdk`:
...
* **Generiert**: Führen Sie `yarn clientvault entity:add` aus und wählen Sie die Option zum Hinzufügen einer neuen Logikfunktion. Dadurch wird eine Starterdatei mit Handler und Konfiguration erzeugt.
...
Logikfunktionen können als **Tools** für KI-Agenten und Workflows verfügbar gemacht werden. Wenn eine Funktion als Tool markiert ist, wird sie von den KI-Funktionen von ClientVault auffindbar und kann als Schritt in Workflow-Automatisierungen ausgewählt werden.
...
Frontend-Komponenten ermöglichen es Ihnen, benutzerdefinierte React-Komponenten zu erstellen, die innerhalb der ClientVault-UI gerendert werden. Verwenden Sie `defineFrontComponent()`, um Komponenten mit eingebauter Validierung zu definieren:
...
* Frontend-Komponenten sind React-Komponenten, die in isolierten Kontexten innerhalb von ClientVault gerendert werden.
...
* Komponenten werden während `yarn clientvault app:dev` automatisch gebaut und synchronisiert.

Sie können neue Frontend-Komponenten auf zwei Arten erstellen:

* **Generiert**: Führen Sie `yarn clientvault entity:add` aus und wählen Sie die Option zum Hinzufügen einer neuen Frontend-Komponente.
...
Der typisierte Client wird von `yarn clientvault app:dev` automatisch generiert und basierend auf Ihrem Arbeitsbereichs-Schema in `node_modules/clientvault-sdk/generated` gespeichert. Verwenden Sie ihn in Ihren Funktionen:
...
Der Client wird von `yarn clientvault app:dev` automatisch neu generiert, sobald sich Ihre Objekte oder Felder ändern.

#### Laufzeit-Anmeldedaten in Logikfunktionen

Wenn Ihre Funktion auf ClientVault läuft, injiziert die Plattform vor der Ausführung Ihres Codes Anmeldedaten als Umgebungsvariablen:

* `CLIENTVAULT_API_URL`: Basis-URL der ClientVault-API, auf die Ihre App abzielt.
* `CLIENTVAULT_API_KEY`: Kurzlebiger Schlüssel, der auf die Standard-Funktionsrolle Ihrer Anwendung begrenzt ist.

Notizen:

* Sie müssen dem generierten Client weder URL noch API-Schlüssel übergeben. Er liest `CLIENTVAULT_API_URL` und `CLIENTVAULT_API_KEY` zur Laufzeit aus process.env.
...
Ein minimales End-to-End-Beispiel, das Objekte, Logikfunktionen, Frontend-Komponenten und mehrere Trigger demonstriert, finden Sie [hier](https://github.com/clientvaulthq/clientvault/tree/main/packages/clientvault-apps/hello-world):

## Manuelle Einrichtung (ohne Scaffolder)

Wir empfehlen zwar `create-clientvault-app` für das beste Einstiegserlebnis, Sie können ein Projekt aber auch manuell einrichten. Installieren Sie die CLI nicht global. Fügen Sie stattdessen `clientvault-sdk` als lokale Abhängigkeit hinzu und binden Sie ein einzelnes Skript in Ihrer package.json ein:

```bash filename="Terminal"
yarn add -D clientvault-sdk
```

Fügen Sie dann ein `clientvault`-Skript hinzu:
...
Jetzt können Sie alle Befehle über `yarn clientvault <command>` ausführen, z. B. `yarn clientvault app:dev`, `yarn clientvault help` usw.

## Fehlerbehebung

* Authentifizierungsfehler: Führen Sie `yarn clientvault auth:login` aus und stellen Sie sicher, dass Ihr API-Schlüssel die erforderlichen Berechtigungen hat.
* Verbindung zum Server nicht möglich: Überprüfen Sie die API-URL und dass der ClientVault-Server erreichbar ist.
* Typen oder Client fehlen/veraltet: Starten Sie `yarn clientvault app:dev` neu — der typisierte Client wird automatisch generiert.
* Dev-Modus synchronisiert nicht: Stellen Sie sicher, dass `yarn clientvault app:dev` läuft und dass Änderungen von Ihrer Umgebung nicht ignoriert werden.
