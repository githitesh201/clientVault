...
<Warning>
  **Erstinstallation?** Folgen Sie dem [Docker Compose-Installationshandbuch](/l/de/developers/self-host/capabilities/docker-compose), um ClientVault zum Laufen zu bringen, und kehren Sie dann hierher zurück, um die Konfiguration fortzusetzen.
</Warning>

ClientVault bietet **zwei Konfigurationsmodi**, um unterschiedlichen Implementierungsbedürfnissen gerecht zu werden:
...
1. Greifen Sie auf Ihre ClientVault-Instanz zu (normalerweise `http://localhost:3000`)
...
<Warning>
  ...
  [Vollständige technische Referenz →](https://github.com/clientvaulthq/clientvault/blob/main/packages/clientvault-server/src/engine/core-modules/clientvault-config/config-variables.ts)
</Warning>
...
Standardmäßig läuft ClientVault im **Single-Workspace-Modus** — ideal für die meisten selbstgehosteten Bereitstellungen, bei denen Sie eine CRM-Instanz für Ihre Organisation benötigen.
...
* Ein Workspace pro ClientVault-Instanz
...
Aktivieren Sie den Multi-Workspace-Modus für SaaS-ähnliche Bereitstellungen, bei denen mehrere unabhängige Teams eigene Workspaces auf derselben ClientVault-Instanz benötigen.
...
[Siehe relevanten Quellcode](https://github.com/clientvaulthq/clientvault/blob/main/packages/clientvault-server/src/engine/core-modules/auth/utils/get-google-apis-oauth-scopes.ts#L4-L10)
...
<Warning>
  Benutzer müssen eine [Microsoft 365-Lizenz](https://admin.microsoft.com/Adminportal/Home) besitzen, um die Kalender- und Messaging-API verwenden zu können. Ohne eine solche Lizenz können sie ihr Konto nicht mit ClientVault synchronisieren.
</Warning>
...
[Siehe relevanten Quellcode](https://github.com/clientvaulthq/clientvault/blob/main/packages/clientvault-server/src/engine/core-modules/auth/utils/get-microsoft-apis-oauth-scopes.ts#L2-L9)
...
ClientVault unterstützt Logikfunktionen für Workflows und benutzerdefinierte Logik. Die Ausführungsumgebung wird über die Umgebungsvariable `SERVERLESS_TYPE` konfiguriert.
...
<Note>
  Bei Verwendung von `SERVERLESS_TYPE=DISABLED` führt jeder Versuch, eine Logikfunktion auszuführen, zu einem Fehler. Dies ist nützlich, wenn Sie ClientVault ohne Unterstützung für Logikfunktionen betreiben möchten.
</Note>
