...
description: Der Leitfaden für Mitwirkende (oder neugierige Entwickler), die ClientVault lokal ausführen möchten.
---
...
<Tabs>
  <Tab title="Linux und MacOS">
    Bevor Sie ClientVault installieren und verwenden können, stellen Sie sicher, dass Sie Folgendes auf Ihrem Computer installiert haben:
...
ClientVault benötigt einen Redis-Cache, um die beste Leistung zu bieten
...
## Schritt 8: Verwenden Sie ClientVault

**Frontend**

Das Frontend von ClientVault läuft unter [http://localhost:3001](http://localhost:3001).
Sie können sich mit dem Standard-Demokonto anmelden: `tim@apple.dev` (Passwort: `tim@apple.dev`)

**Backend**

* Der Server von ClientVault wird unter [http://localhost:3000](http://localhost:3000) verfügbar sein.
