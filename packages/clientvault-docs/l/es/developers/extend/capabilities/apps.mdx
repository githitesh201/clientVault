---
title: Aplicaciones de ClientVault
description: Crea y gestiona personalizaciones de ClientVault como código.
---
...
Las aplicaciones te permiten crear y administrar personalizaciones de ClientVault **como código**. En lugar de configurar todo a través de la interfaz de usuario, defines tu modelo de datos y funciones de lógica en código, lo que hace más rápido crear, mantener y desplegar en múltiples espacios de trabajo.
...
* Node.js 24+ y Yarn 4
* Un espacio de trabajo de ClientVault y una clave de API (créala en https://app.clientvault.com/settings/api-webhooks)
...
Consulta también: las páginas de referencia de la CLI para [create-clientvault-app](https://www.npmjs.com/package/create-clientvault-app) y [clientvault-sdk CLI](https://www.npmjs.com/package/clientvault-sdk).

## Estructura del proyecto (generada)

Cuando ejecutas `npx create-clientvault-app@latest my-clientvault-app`, el generador:

* Copia una aplicación base mínima en `my-clientvault-app/`
* Añade una dependencia local de `clientvault-sdk` y la configuración de Yarn 4
* Crea archivos de configuración y scripts vinculados a la CLI `clientvault`
...
* **package.json**: Declara el nombre de la aplicación, la versión, los entornos (Node 24+, Yarn 4) y agrega `clientvault-sdk` además de scripts como `app:dev`, `entity:add`, `function:logs`, `function:execute`, `app:uninstall` y `auth:login` que delegan en la CLI local `clientvault`.
...
* `yarn app:dev` genera automáticamente el cliente ClientVault tipado en `node_modules/clientvault-sdk/generated`.
...
Tus credenciales se almacenan por usuario en `~/.clientvault/config.json`. Puedes mantener varios perfiles y cambiar entre ellos.
...
El clientvault-sdk proporciona bloques de construcción tipados y funciones auxiliares que utilizas dentro de tu aplicación. A continuación, las partes clave que usarás con más frecuencia.
...
<Note>
  **Los campos base se crean automáticamente.** Cuando defines un objeto personalizado, ClientVault añade automáticamente campos estándar como `name`, `createdAt`, `updatedAt`, `createdBy`, `position` y `deletedAt`. No necesitas definir estos en tu matriz `fields` — solo agrega tus campos personalizados.
</Note>
...
* La clave de API en tiempo de ejecución inyectada como `CLIENTVAULT_API_KEY` se deriva de este rol de función predeterminado.
...
* Consulta un ejemplo funcional en la aplicación Hello World: [`packages/clientvault-apps/hello-world/src/roles/function-role.ts`](https://github.com/clientvaulthq/clientvault/blob/main/packages/clientvault-apps/hello-world/src/roles/function-role.ts).
...
Cuando un disparador de ruta invoca tu función de lógica, esta recibe un objeto `RoutePayload` que sigue el formato de AWS HTTP API v2. Importa el tipo desde `clientvault-sdk`:

```typescript
import { defineFunction, type RoutePayload } from 'clientvault-sdk';
...
`yarn app:dev` genera automáticamente el cliente ClientVault tipado en `node_modules/clientvault-sdk/generated`. Úsalo en tus funciones:
...
Cuando tu función se ejecuta en ClientVault, la plataforma inyecta credenciales como variables de entorno antes de que tu código se ejecute:

* `CLIENTVAULT_API_URL`: URL base de la API de ClientVault a la que apunta tu aplicación.
* `CLIENTVAULT_API_KEY`: Clave de corta duración con alcance al rol de función predeterminado de tu aplicación.

Notas:

* No necesitas pasar la URL ni la clave de API al cliente generado. Lee `CLIENTVAULT_API_URL` y `CLIENTVAULT_API_KEY` de process.env en tiempo de ejecución.
...
Explora un ejemplo mínimo de extremo a extremo que demuestra objetos, funciones y múltiples desencadenadores [aquí](https://github.com/clientvaulthq/clientvault/tree/main/packages/clientvault-apps/hello-world):

## Configuración manual (sin el generador)

Aunque recomendamos usar `create-clientvault-app` para la mejor experiencia de inicio, también puedes configurar un proyecto manualmente. No instales la CLI globalmente. En su lugar, agrega `clientvault-sdk` como dependencia local y conecta scripts en tu package.json:
...
{
  "scripts": {
    "auth:login": "clientvault auth:login",
    "auth:logout": "clientvault auth:logout",
    "auth:status": "clientvault auth:status",
    "auth:switch": "clientvault auth:switch",
    "auth:list": "clientvault auth:list",
    "app:dev": "clientvault app:dev",
    "app:uninstall": "clientvault app:uninstall",
    "entity:add": "clientvault entity:add",
    "function:logs": "clientvault function:logs",
    "function:execute": "clientvault function:execute",
    "help": "clientvault help"
  }
}
```
...
* No se puede conectar al servidor: verifica la URL de la API y que el servidor de ClientVault sea accesible.
