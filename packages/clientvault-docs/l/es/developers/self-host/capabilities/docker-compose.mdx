...
Esta guía proporciona instrucciones paso a paso para instalar y configurar la aplicación ClientVault utilizando Docker Compose. El objetivo es simplificar el proceso y prevenir errores comunes que podrían arruinar tu configuración.
...
Instala la última versión estable de ClientVault con un solo comando:
...
Si alojas clientvaultCRM en tu propia computadora, abre tu navegador y navega a [http://localhost:3000](http://localhost:3000).
...
### Exponer ClientVault para acceso externo

Por defecto, ClientVault se ejecuta en `localhost` en el puerto `3000`. Para acceder a él mediante un dominio externo o dirección IP, necesitas configurar `SERVER_URL` en tu archivo `.env`.
...
Recomendamos encarecidamente configurar ClientVault detrás de un proxy inverso con terminación SSL para una seguridad y funcionalidad óptimas.
