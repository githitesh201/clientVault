...
```bash
npx nx run clientvault-front:lint # pasar --fix para corregir errores de lint
```
...
```bash
npx nx run clientvault-front:test # ejecutar pruebas con Jest
npx nx run clientvault-front:storybook:serve:dev # ejecutar Storybook
npx nx run clientvault-front:storybook:test # ejecutar pruebas # (requiere que yarn storybook:serve:dev esté en ejecución)
npx nx run clientvault-front:storybook:coverage # (requiere que yarn storybook:serve:dev esté en ejecución)
```
