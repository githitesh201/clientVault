...
Estos comandos deben ejecutarse desde la carpeta packages/clientvault-server.
Desde cualquier otra carpeta, puedes ejecutar `npx nx {command} clientvault-server` (o `npx nx run clientvault-server:{command}`).
...
Nota: puedes ejecutar `npx nx run clientvault-server:test:integration:with-db-reset` en caso de que necesites restablecer la base de datos antes de ejecutar las pruebas de integración.
...
ClientVault utiliza principalmente NestJS para el backend.
