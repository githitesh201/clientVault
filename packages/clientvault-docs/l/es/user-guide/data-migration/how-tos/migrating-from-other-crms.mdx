...
description: Guía paso a paso para migrar tus datos desde cualquier CRM a ClientVault.
---

## Resumen

Esta guía te lleva paso a paso para migrar tus datos desde cualquier CRM a ClientVault. El proceso implica auditar tus datos, preparar tu espacio de trabajo de ClientVault, exportar desde tu sistema actual e importar en ClientVault.
...
Crea un documento de mapeo entre tu CRM actual y ClientVault:
...
* El campo de destino en ClientVault
...
## Paso 3: Configura tu espacio de trabajo en ClientVault

Antes de importar datos, prepara tu espacio de trabajo en ClientVault:
...
<Warning>
  **Invita a los usuarios ANTES de importar datos.**
  ...
  Si tus datos incluyen referencias a usuarios (propietario de la cuenta, asignado, etc.), esos usuarios deben existir en ClientVault antes de la importación. De lo contrario, esas relaciones no se podrán asignar.
</Warning>
...
Abre cada CSV exportado en una aplicación de hojas de cálculo y prepáralo para ClientVault.

### Elimina duplicados

1. Ordena por el campo único (correo electrónico para Personas, dominio para Empresas)
2. Elimina o combina filas duplicadas
3. Verifica que no existan duplicados ya en ClientVault
...
<Warning>
  **El formato del dominio es fundamental.**
  ...
  Usa `https://domain.com` (no `domain.com` ni `www.domain.com`). Esto coincide con el formato de ClientVault y evita duplicados cuando conectas la sincronización de correo/calendario.
</Warning>
...
2. Usa las mismas direcciones de correo electrónico que los usuarios utilizaron para unirse a tu espacio de trabajo en ClientVault
...
1. Navega hasta el objeto en ClientVault
...
* ClientVault comenzará a sincronizar los correos electrónicos con los registros de contactos
...
<AccordionGroup>
  <Accordion title="¿Puedo migrar los flujos de trabajo automáticamente?">
    Actualmente no. Los flujos de trabajo deben recrearse manualmente en ClientVault.
  </Accordion>
...
Contacta con [contact@clientvault.com](mailto:contact@clientvault.com) o explora nuestros [Servicios de implementación](/l/es/user-guide/getting-started/capabilities/implementation-services).
