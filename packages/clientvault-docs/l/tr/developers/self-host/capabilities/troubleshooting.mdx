...
Mevcut bir ClientVault örneğiniz varsa ve üretim verileri varsa, bu adımları **TAKİP ETMEYİN**, çünkü bu veritabanınızı kalıcı olarak silecektir!

ClientVault'yi ilk kez kurarken, varsayılan veritabanı parolasını değiştirmek isteyebilirsiniz.
...
ClientVault kurulumu sırasında, postgres veritabanınızı doğru şemalar, uzantılar ve kullanıcılarla hazırlamanız gerekir.
...
#### Modül 'clientvault-emails' veya buna karşılık gelen tür açıklamaları bulunamıyor.

`clientvault-emails` paketini veritabanını `npx nx run clientvault-emails:build` ile başlatmadan önce derlemeniz gerekiyor.

#### Eksik clientvault-x paketi

Kök dizinde yarn çalıştırdığınızdan ve ardından `npx nx server:dev clientvault-server` çalıştırdığınızdan emin olun. Bu hala çalışmıyorsa eksik paketi manuel olarak derlemeyi deneyin.
...
#### `npx nx start` veya `npx nx start clientvault-front` çalıştırırken, bellek hatası verildi

`packages/clientvault-front/.env` içinde `VITE_DISABLE_TYPESCRIPT_CHECKER=true` ve `VITE_DISABLE_ESLINT_CHECKER=true`'i açarak arka plan kontrollerini devre dışı bırakın, böylece gerekli RAM miktarını azaltın.

**Bu işe yaramazsa:**
Yalnızca ihtiyaç duyduğunuz hizmetleri çalıştırın, `npx nx start` yerine. Örneğin, sunucuda çalışıyorsanız yalnızca `npx nx worker clientvault-server` çalıştırın.

**Bu işe yaramazsa:**
WSL'de yalnızca `npx nx run clientvault-server:start` çalıştırmayı denediyseniz ve aşağıdaki bellek hatasıyla karşılaşıyorsanız:
...
**Bu işe yaramazsa:**
Hangi işlemlerin makinenizin RAM'inin çoğunu kullandığını araştırın. ClientVault'de bazı VScode uzantılarının çok fazla RAM aldığını fark ettik, bu yüzden onları geçici olarak devre dışı bırakıyoruz.
...
* ClientVault uygulamanızı açın, `/settings` sayfasına gidin ve ekranın sol alt köşesinde bulunan **Gelişmiş** seçeneğini etkinleştirin.
* Yeni bir webhook oluşturun.
* **Benzersiz Webhook URL'nizi** ClientVault'deki **Endpoint URL** alanına yapıştırın. **Filtreleri** `Şirketler` ve `Oluşturuldu` olarak ayarlayın.
...
Aşağıdaki örnekte olduğu gibi `packages/clientvault-ui/vite-config.ts` içindeki checker eklentisini yorum satırına alın
...
Eğer ClientVault'yi ters proxy'nin arkasında çalıştırıyorsanız ve bağlantı sorunları yaşıyorsanız:
...
2. **Ters Proxy Ayarlarını Kontrol Edin:**
   ...
   * Ters proxy'nizin ClientVault sunucusuna talepleri düzgün bir şekilde yönlendirdiğinden emin olun.
...
3. **Servisleri Yeniden Başlatın:**
   ...
   Değişiklik yaptıktan sonra hem ters proxy'yi hem de ClientVault konteynerlerini yeniden başlatın.
...
* Topluluk Desteği:
  ...
  [ClientVault topluluğuna](https://github.com/clientvaulthq/clientvault/issues) veya [destek kanallarına](https://discord.gg/cx5n4Jzs57) yardım için ulaşın.
