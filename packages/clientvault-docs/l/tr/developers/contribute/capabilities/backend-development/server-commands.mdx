...
Bu komutlar packages/clientvault-server klasöründen çalıştırılmalıdır.
Diğer herhangi bir klasörden `npx nx {command} clientvault-server` (veya `npx nx run clientvault-server:{command}`) çalıştırabilirsiniz.

### İlk Kurulum

```
npx nx database:reset clientvault-server # veritabanını geliştirme tohum verileriyle kur
```
...
```
npx nx run clientvault-server:lint # lint hatalarını düzeltmek için --fix'i geç
```

### Test

```
npx nx run clientvault-server:test:unit # birim testleri çalıştır
npx nx run clientvault-server:test:integration # entegrasyon testlerini çalıştır
```

Not: Entegrasyon testlerini çalıştırmadan önce veritabanını sıfırlamanız gerekirse `npx nx run clientvault-server:test:integration:with-db-reset` komutunu kullanabilirsiniz.
...
ClientVault, arka uç için öncelikle NestJS kullanır.
