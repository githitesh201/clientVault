---
title: Yerel Kurulum
description: ClientVault'i yerel olarak çalıştırmak isteyen katkıda bulunanlar (veya meraklı geliştiriciler) için kılavuz.
---
...
<Tabs>
  <Tab title="Linux ve MacOS">
    ClientVault'i yüklemeden ve kullanmadan önce bilgisayarınıza aşağıdakileri yüklediğinizden emin olun:
...
ClientVault, en iyi performansı sağlamak için bir redis önbelleğe ihtiyaç duyar
...
## Adım 8: ClientVault'yi kullanın

**Ön Uç**

ClientVault'nin ön ucu [http://localhost:3001](http://localhost:3001) adresinde çalışıyor olacak.
Varsayılan demo hesabıyla giriş yapabilirsiniz: `tim@apple.dev` (şifre: `tim@apple.dev`)

**Arka Uç**

* ClientVault'nin sunucusu [http://localhost:3000](http://localhost:3000) adresinde çalışır durumda olacak
