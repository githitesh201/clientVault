...
```bash
npx nx run clientvault-front:lint # lint hatalarını düzeltmek için --fix'i geç
```
...
```bash
npx nx run clientvault-front:test # jest testlerini çalıştır
npx nx run clientvault-front:storybook:serve:dev # storybook'u çalıştır
npx nx run clientvault-front:storybook:test # testleri çalıştır # (yarn storybook:serve:dev'nin çalışıyor olması gerekir)
npx nx run clientvault-front:storybook:coverage # (yarn storybook:serve:dev'nin çalışıyor olması gerekir)
```
