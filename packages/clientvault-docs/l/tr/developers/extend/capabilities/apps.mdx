---
title: ClientVault Uygulamaları
description: ClientVault özelleştirmelerini kod olarak oluşturun ve yönetin.
---
...
Uygulamalar, ClientVault özelleştirmelerini **kod olarak** oluşturup yönetmenizi sağlar. Her şeyi UI üzerinden yapılandırmak yerine, veri modelinizi ve mantık fonksiyonlarınızı kodla tanımlarsınız — bu da oluşturmayı, bakımı ve birden çok çalışma alanına dağıtmayı hızlandırır.
...
* Bir ClientVault çalışma alanı ve bir API anahtarı (https://app.clientvault.com/settings/api-webhooks adresinde oluşturun)
...
Ayrıca bkz.: [create-clientvault-app](https://www.npmjs.com/package/create-clientvault-app) ve [clientvault-sdk CLI](https://www.npmjs.com/package/clientvault-sdk) için CLI başvuru sayfaları.

## Proje yapısı (şablondan oluşturulmuş)

`npx create-clientvault-app@latest my-clientvault-app` komutunu çalıştırdığınızda scaffolder şunları yapar:

* Minimal bir temel uygulamayı `my-clientvault-app/` içine kopyalar
* Yerel bir `clientvault-sdk` bağımlılığı ve Yarn 4 yapılandırması ekler
* `clientvault` CLI ile bağlantılı yapılandırma dosyaları ve betikler oluşturur
...
* **package.json**: Uygulama adını, sürümünü, motorları (Node 24+, Yarn 4) bildirir ve `clientvault-sdk` ile yerel `clientvault` CLI'sine yetki devreden bir `clientvault` betiği ekler. Tüm mevcut komutları listelemek için `yarn clientvault help` komutunu çalıştırın.
...
SDK, TypeScript dosyalarınızı **`export default define<Entity>({...})`** çağrılarını arayarak ayrıştırıp varlıkları algılar. Her varlık türünün, `clientvault-sdk` tarafından dışa aktarılan karşılık gelen bir yardımcı fonksiyonu vardır:
...
* `yarn clientvault app:dev`, `node_modules/clientvault-sdk/generated` içinde tipli bir API istemcisini otomatik olarak oluşturur (tipli ClientVault istemcisi + çalışma alanı türleri).
* `yarn clientvault entity:add`, özel nesneleriniz, fonksiyonlarınız, ön bileşenleriniz veya rolleriniz için `src/` altında varlık tanım dosyaları ekler.

## Kimlik Doğrulama

`yarn clientvault auth:login` komutunu ilk kez çalıştırdığınızda, sizden şunlar istenir:

* API URL’si (varsayılan: http://localhost:3000 veya mevcut çalışma alanı profiliniz)
* API anahtarı

Kimlik bilgileriniz kullanıcı başına `~/.clientvault/config.json` içinde saklanır. You can maintain multiple profiles and switch between them.
...
`yarn clientvault auth:switch` ile çalışma alanlarını değiştirdikten sonra, sonraki tüm komutlar varsayılan olarak o çalışma alanını kullanacaktır. You can still override it temporarily with `--workspace <name>`.

## SDK kaynaklarını kullanın (türler ve yapılandırma)

clientvault-sdk, uygulamanız içinde kullandığınız türlendirilmiş yapı taşları ve yardımcı fonksiyonlar sağlar. Aşağıda en sık dokunacağınız başlıca parçalar yer alıyor.
...
* `yarn clientvault entity:add` kullanarak, adlandırma, alanlar ve ilişkiler konusunda sizi yönlendirerek yeni nesneler oluşturabilirsiniz.

<Note>
  **Temel alanlar otomatik olarak oluşturulur.** Özel bir nesne tanımladığınızda ClientVault, standart alanları otomatik olarak ekler
...
* `CLIENTVAULT_API_KEY` olarak enjekte edilen çalışma zamanı API anahtarı bu varsayılan fonksiyon rolünden türetilir.
...
* Çalışan bir örneği Hello World uygulamasında görün: [`packages/clientvault-apps/hello-world/src/roles/function-role.ts`](https://github.com/clientvaulthq/clientvault/blob/main/packages/clientvault-apps/hello-world/src/roles/function-role.ts).
...
`create-clientvault-app` ile yeni bir uygulama iskeleti oluşturduğunuzda, `src/logic-functions/post-install.ts` konumunda sizin için bir kurulum sonrası işlevi oluşturulur:
...
Bir rota tetikleyicisi mantık fonksiyonunuzu çağırdığında, AWS HTTP API v2 formatını izleyen bir `RoutePayload` nesnesi alır. Türü `clientvault-sdk` içinden içe aktarın:
...
* **Şablondan**: `yarn clientvault entity:add` çalıştırın ve yeni bir mantık fonksiyonu ekleme seçeneğini seçin. Bu, bir işleyici ve yapılandırma içeren bir başlangıç dosyası oluşturur.
...
Mantık işlevleri, yapay zeka ajanları ve iş akışları için **araçlar** olarak sunulabilir. Bir işlev bir araç olarak işaretlendiğinde, ClientVault'nin yapay zeka özellikleri tarafından keşfedilebilir hâle gelir ve iş akışı otomasyonlarında bir adım olarak seçilebilir.
...
Ön uç bileşenleri, ClientVault'nin kullanıcı arayüzünde görüntülenen özel React bileşenleri oluşturmanıza olanak tanır. Yerleşik doğrulamayla bileşenleri tanımlamak için `defineFrontComponent()` kullanın:
...
* Ön uç bileşenleri, ClientVault içinde yalıtılmış bağlamlarda görüntülenen React bileşenleridir.
* Otomatik algılama için `*.front-component.tsx` dosya soneğini kullanın.
* `component` alanı, React bileşeninize referans verir.
* Bileşenler, `yarn clientvault app:dev` sırasında otomatik olarak oluşturulur ve senkronize edilir.

Yeni ön uç bileşenlerini iki şekilde oluşturabilirsiniz:

* **Şablondan**: `yarn clientvault entity:add` çalıştırın ve yeni bir ön uç bileşeni ekleme seçeneğini seçin.
...
Tipli istemci, `yarn clientvault app:dev` tarafından otomatik olarak oluşturulur ve çalışma alanı şemanıza göre `node_modules/clientvault-sdk/generated` içine kaydedilir. Fonksiyonlarınızda kullanın:
...
Nesneleriniz veya alanlarınız değiştiğinde, istemci `yarn clientvault app:dev` tarafından otomatik olarak yeniden oluşturulur.

#### Mantık fonksiyonlarında çalışma zamanı kimlik bilgileri

Fonksiyonunuz ClientVault üzerinde çalıştığında, platform kodunuz yürütülmeden önce kimlik bilgilerini ortam değişkenleri olarak enjekte eder:

* `CLIENTVAULT_API_URL`: Uygulamanızın hedeflediği ClientVault API'nin temel URL’si.
* `CLIENTVAULT_API_KEY`: Uygulamanızın varsayılan fonksiyon rolü kapsamına sahip kısa ömürlü anahtar.

Notlar:

* Oluşturulan istemciye URL veya API anahtarı geçirmeniz gerekmez. Çalışma zamanında `CLIENTVAULT_API_URL` ve `CLIENTVAULT_API_KEY` değerlerini process.env üzerinden okur.
...
Nesneleri, mantık fonksiyonlarını, ön uç bileşenlerini ve birden çok tetikleyiciyi gösteren minimal, uçtan uca bir örneği [buradan](https://github.com/clientvaulthq/clientvault/tree/main/packages/clientvault-apps/hello-world) inceleyin:

## Manuel kurulum (scaffolder olmadan)

En iyi başlangıç deneyimi için `create-clientvault-app` kullanmanızı önersek de, bir projeyi manuel olarak da kurabilirsiniz. CLI'yi global olarak kurmayın. Bunun yerine `clientvault-sdk`'yi yerel bir bağımlılık olarak ekleyin ve package.json içinde tek bir betik tanımlayın:

```bash filename="Terminal"
yarn add -D clientvault-sdk
```

Ardından bir `clientvault` betiği ekleyin:
...
Artık tüm komutları `yarn clientvault <command>` üzerinden çalıştırabilirsiniz; örn. `yarn clientvault app:dev`, `yarn clientvault help` vb.

## Sorun Giderme

* Kimlik doğrulama hataları: `yarn clientvault auth:login` çalıştırın ve API anahtarınızın gerekli izinlere sahip olduğundan emin olun.
* Sunucuya bağlanılamıyor: API URL’sini ve ClientVault sunucusunun erişilebilir olduğunu doğrulayın.
* Türler veya istemci eksik/eski: `yarn clientvault app:dev` komutunu yeniden çalıştırın — tip tanımlı istemciyi otomatik olarak oluşturur.
* Geliştirme modu eşitlenmiyor: `yarn clientvault app:dev`'in çalıştığından ve değişikliklerin ortamınız tarafından yok sayılmadığından emin olun.
