...
description: Verilerinizi herhangi bir CRM'den ClientVault'ye taşımak için adım adım kılavuz.
---

## Genel Bakış

Bu kılavuz, verilerinizi herhangi bir CRM'den ClientVault'ye taşıma sürecinde size yol gösterir. Süreç, verilerinizi denetlemeyi, ClientVault çalışma alanınızı hazırlamayı, mevcut sisteminizden dışa aktarmayı ve ClientVault'ye içe aktarmayı içerir.
...
Mevcut CRM'iniz ile ClientVault arasında bir eşleme belgesi oluşturun:

| CRM'iniz         | ClientVault                 |
| ---------------- | ---------------------- |
...
## Adım 3: ClientVault Çalışma Alanınızı Kurun

Verileri içe aktarmadan önce ClientVault çalışma alanınızı hazırlayın:
...
<Warning>
  **Kullanıcıları verileri içe aktarmadan ÖNCE davet edin.**
  ...
  Verileriniz kullanıcı referansları içeriyorsa (Hesap Sahibi, Atanan vb.), bu kullanıcıların içe aktarmadan önce ClientVault'de mevcut olması gerekir. Aksi halde bu ilişkiler eşlenemez.
</Warning>
...
Dışa aktarılan her CSV'yi bir hesap tablosu uygulamasında açın ve ClientVault için hazırlayın.
...
3. ClientVault'de halihazırda yinelenen olmadığını doğrulayın
...
<Warning>
  **Alan adı biçimi kritiktir.**
  ...
  `https://domain.com` kullanın (`domain.com` veya `www.domain.com` değil). Bu, ClientVault'nin biçimiyle eşleşir ve e-posta/takvim eşzamanlamasını bağladığınızda yinelenmeleri önler.
</Warning>
...
2. Kullanıcıların ClientVault çalışma alanınıza katılmak için kullandıklarıyla aynı e-posta adreslerini kullanın
...
## Adım 6: ClientVault'ye İçe Aktarın
...
1. ClientVault'de nesneye gidin
...
* ClientVault, e-postaları kişi kayıtlarıyla eşzamanlamaya başlayacaktır
...
<AccordionGroup>
  <Accordion title="İş akışlarını otomatik olarak taşıyabilir miyim?">
    Şu anda mümkün değil. İş akışları ClientVault'de manuel olarak yeniden oluşturulmalıdır.
  </Accordion>
...
[contact@clientvault.com](mailto:contact@clientvault.com) adresinden bizimle iletişime geçin veya [Uygulama Hizmetlerimizi](/l/tr/user-guide/getting-started/capabilities/implementation-services) keşfedin.
