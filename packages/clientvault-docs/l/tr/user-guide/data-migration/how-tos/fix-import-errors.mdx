...
Dosyanızı yükledikten ve sütunları eşledikten sonra, ClientVault verilerinizi doğrular:

1. **Doğrulama çalışır** — ClientVault her satırı hatalar için kontrol eder
...
| ------------------------------------------------------------------------------ | -------------------------------------------------------------- |
| Yinelenen değerler sarı renkle vurgulanır                                      | Değer ClientVault'de zaten mevcut veya dosyanızda iki kez görünüyor |
...
**Seçenek 3: Mevcut kaydı ClientVault'nin güncellemesine izin verin**
...
3. ClientVault, yinelenen bir kayıt oluşturmak yerine mevcut kaydı güncelleyecektir
...
<Note>
  ...
  * `email`'i korur ancak `id`'yi değiştirirseniz → id güncellenir
  ...
  Benzersiz tanımlayıcılardan biri eşleştiği sürece ClientVault kaydı günceller.
</Note>
...
3. Kayıtların ClientVault'de zaten mevcut olup olmadığını kontrol edin
...
Bu, ClientVault'de o özel tanımlayıcıya sahip bir Şirket olmadığı anlamına gelir.
...
ClientVault'de mevcut olmayan bir kayda bağlanmaya çalışıyorsunuz.
...
Dosyanızdaki bir sütun eşlenemiyor çünkü alan ClientVault'de mevcut değil.
...
1. **Kullanıcı davetini kabul etmedi** — kullanıcı henüz ClientVault'de mevcut değil
2. **Eski sistemdeki kullanıcı kimliğini kullanma** — ClientVault başka bir sistemin kimliklerini eşleştiremez
3. **Yanlış e-posta biçimi** — e-posta, kullanıcının ClientVault hesabıyla eşleşmiyor

### Nasıl Düzeltilir

1. Tüm kullanıcıların ClientVault çalışma alanınıza **davetlerini kabul ettiğinden** emin olun
2. Kullanıcının **e-posta adresini** kullanın (adlarını veya eski sistem kimliğini değil)
3. ClientVault'ye katılmak için kullandıkları aynı e-postayı kullanın
...
<Check>Alan adı biçimi, ClientVault'nin biçimiyle birebir eşleşir</Check>
...
1. **Örnek dosyayı indirin** — ClientVault'nin beklediği tam biçimi görün
