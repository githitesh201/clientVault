...
<AccordionGroup>
  <Accordion title="What roles come with ClientVault by default?">
    ClientVault comes with an **Admin** and **Member** roles by default. You can create additional custom roles based on your team's needs (e.g., Sales Rep, Manager, Read-Only User).
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="What action permissions are available?">
    ...
    * **Send Email**: Ability to send emails from ClientVault
...
<AccordionGroup>
  ...
  <Accordion title="What is Just-in-Time (JIT) provisioning?">
    With JIT provisioning, user accounts are automatically created in ClientVault when someone logs in via SSO for the first time. They're assigned the default role automatically.
  </Accordion>
