...
Tek Oturum Açma (SSO), ekip üyelerinizin kuruluşunuzun kimlik sağlayıcısını kullanarak ClientVault'ye giriş yapmasına olanak tanır. Bu şunları sağlar:
...
ClientVault, SSO'yu şu sağlayıcılarla destekler:
...
* ClientVault çalışma alanına yönetici erişimi

<Note>
  **SSO'yu kurmak isteyen öz barındırma kullanıcıları için**, contact@clientvault.com ile iletişime geçin
</Note>
...
* **Entity ID**: ClientVault tarafından sağlanır
* **ACS URL**: Kimlik doğrulaması için geri çağrı URL'si
* **Certificate**: Güvenli iletişim için

#### 4. Sağlayıcı Ayrıntılarını ClientVault'ye Girin
...
* ClientVault çalışma alanından kaldırın
...
* **Grup eşleme**: IdP gruplarını ClientVault rollerine eşleyin (varsa)
* **Ayrılış süreci**: Erişim kaldırma iş akışınıza ClientVault'yi dahil edin
