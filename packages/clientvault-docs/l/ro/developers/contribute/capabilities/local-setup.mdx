...
description: Ghidul pentru contribuitori (sau dezvoltatori curioși) care doresc să ruleze ClientVault local.
---
...
<Tabs>
  <Tab title="Linux și MacOS">
    Înainte de a instala și utiliza ClientVault, asigurați-vă că instalați următoarele pe computerul dvs.:
...
ClientVault necesită un cache Redis pentru a oferi cea mai bună performanță
...
## Pasul 8: Utilizați ClientVault

**Front-end**

Frontend-ul ClientVault va rula la [http://localhost:3001](http://localhost:3001).
Vă puteți loga folosind contul demo implicit: `tim@apple.dev` (parolă: `tim@apple.dev`)

**Back-end**

* Serverul ClientVault va fi operativ la [http://localhost:3000](http://localhost:3000)
