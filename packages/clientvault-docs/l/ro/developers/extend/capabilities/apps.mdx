---
title: Aplicații ClientVault
description: Construiți și gestionați personalizările ClientVault sub formă de cod.
---
...
* Un spațiu de lucru ClientVault și o cheie API (creați una la https://app.clientvault.com/settings/api-webhooks)
...
Consultați și: paginile de referință CLI pentru [create-clientvault-app](https://www.npmjs.com/package/create-clientvault-app) și [clientvault-sdk CLI](https://www.npmjs.com/package/clientvault-sdk).

## Structura proiectului (generată)

Când rulați `npx create-clientvault-app@latest my-clientvault-app`, generatorul:

* Copiază o aplicație de bază minimală în `my-clientvault-app/`
* Adaugă o dependență locală `clientvault-sdk` și configurația Yarn 4
* Creează fișiere de configurare și scripturi conectate la CLI-ul `clientvault`
...
* **package.json**: Declară numele aplicației, versiunea, motoarele (Node 24+, Yarn 4) și adaugă `clientvault-sdk` plus un script `clientvault` care deleagă către CLI-ul local `clientvault`. Rulează `yarn clientvault help` pentru a lista toate comenzile disponibile.
...
SDK-ul detectează entitățile analizând fișierele TypeScript pentru apeluri **`export default define<Entity>({...})`**. Fiecare tip de entitate are o funcție ajutătoare corespunzătoare, exportată din `clientvault-sdk`:
...
* `yarn clientvault app:dev` va genera automat un client API tipizat în `node_modules/clientvault-sdk/generated` (client ClientVault tipizat + tipuri ale spațiului de lucru).
* `yarn clientvault entity:add` va adăuga fișiere de definire a entităților în `src/` pentru obiectele, funcțiile, componentele front-end sau rolurile personalizate.

## Autentificare

Prima dată când rulați `yarn clientvault auth:login`, vi se vor solicita:

* URL-ul API (implicit http://localhost:3000 sau profilul spațiului de lucru curent)
* Cheie API

Acreditările dvs. sunt stocate per utilizator în `~/.clientvault/config.json`. Puteți menține mai multe profiluri și comuta între ele.
...
După ce ați schimbat spațiul de lucru cu `yarn clientvault auth:switch`, toate comenzile ulterioare vor folosi implicit acel spațiu de lucru. Îl puteți totuși suprascrie temporar cu `--workspace <name>`.

## Utilizați resursele SDK (tipuri și configurare)

Biblioteca clientvault-sdk oferă blocuri de bază tipizate și funcții ajutătoare pe care le utilizați în aplicația dvs. Mai jos sunt elementele cheie cu care veți interacționa cel mai des.
...
* Puteți genera obiecte noi folosind `yarn clientvault entity:add`, care vă ghidează prin denumire, câmpuri și relații.

<Note>
  **Câmpurile de bază sunt create automat.** Când definiți un obiect personalizat, ClientVault adaugă automat câmpuri standard
...
* Cheia API de runtime injectată ca `CLIENTVAULT_API_KEY` este derivată din acest rol implicit pentru funcții.
...
* Vedeți un exemplu funcțional în aplicația Hello World: [`packages/clientvault-apps/hello-world/src/roles/function-role.ts`](https://github.com/clientvaulthq/clientvault/blob/main/packages/clientvault-apps/hello-world/src/roles/function-role.ts).
...
Când creezi scheletul unei aplicații noi cu `create-clientvault-app`, este generată o funcție post-instalare la `src/logic-functions/post-install.ts`:
...
Când un declanșator de rută apelează funcția dvs. de logică, aceasta primește un obiect `RoutePayload` care urmează formatul AWS HTTP API v2. Importă tipul din `clientvault-sdk`:
...
* **Generat**: Rulați `yarn clientvault entity:add` și alegeți opțiunea de a adăuga o funcție logică nouă. Aceasta generează un fișier inițial cu un handler și o configurație.
...
Funcțiile logice pot fi expuse ca **instrumente** pentru agenți de IA și fluxuri de lucru. Când o funcție este marcată ca instrument, poate fi descoperită de funcționalitățile de IA ale ClientVault și poate fi selectată ca pas în automatizări ale fluxurilor de lucru.
...
Componentele Front vă permit să construiți componente React personalizate care sunt randate în interfața ClientVault. Utilizați `defineFrontComponent()` pentru a defini componente cu validare încorporată:
...
* Componentele Front sunt componente React care sunt randate în contexte izolate în cadrul ClientVault.
...
* Componentele sunt construite și sincronizate automat în timpul `yarn clientvault app:dev`.

Puteți crea componente Front noi în două moduri:

* **Generat**: Rulați `yarn clientvault entity:add` și alegeți opțiunea de a adăuga o componentă frontend nouă.
* **Manual**: Creați un fișier nou `*.front-component.tsx` și folosiți `defineFrontComponent()`.

### Client tipizat generat

Clientul tipizat este generat automat de `yarn clientvault app:dev` și stocat în `node_modules/clientvault-sdk/generated`, pe baza schemei spațiului tău de lucru. Folosiți-l în funcțiile dvs.:
...
Clientul este regenerat automat de `yarn clientvault app:dev` ori de câte ori obiectele sau câmpurile tale se schimbă.

#### Acreditări la runtime în funcțiile de logică

Când funcția rulează pe ClientVault, platforma injectează acreditări ca variabile de mediu înainte de execuția codului:

* `CLIENTVAULT_API_URL`: URL-ul de bază al API-ului ClientVault către care țintește aplicația.
* `CLIENTVAULT_API_KEY`: Cheie cu durată scurtă, limitată la rolul implicit de funcție al aplicației.

Notițe:

* Nu trebuie să transmiteți URL-ul sau cheia API către clientul generat. Acesta citește `CLIENTVAULT_API_URL` și `CLIENTVAULT_API_KEY` din process.env la runtime.
...
Explorați un exemplu minim, cap la cap, care demonstrează obiecte, funcții de logică, componente Front și declanșatoare multiple [aici](https://github.com/clientvaulthq/clientvault/tree/main/packages/clientvault-apps/hello-world):

## Configurare manuală (fără generator)

Deși recomandăm utilizarea `create-clientvault-app` pentru cea mai bună experiență de început, puteți configura și un proiect manual. Nu instalați CLI-ul global. În schimb, adăugați `clientvault-sdk` ca dependență locală și conectați un singur script în package.json-ul dvs.:

```bash filename="Terminal"
yarn add -D clientvault-sdk
```

Apoi adăugați un script `clientvault`:
...
Acum poți rula toate comenzile prin `yarn clientvault <command>`, de ex. `yarn clientvault app:dev`, `yarn clientvault help`, etc.

## Depanare

* Erori de autentificare: rulați `yarn clientvault auth:login` și asigurați-vă că cheia API are permisiunile necesare.
* Nu se poate conecta la server: verificați URL-ul API și că serverul ClientVault este accesibil.
* Types or client missing/outdated: restart `yarn clientvault app:dev` — it auto-generates the typed client.
* Modul dev nu sincronizează: asigurați-vă că `yarn clientvault app:dev` rulează și că modificările nu sunt ignorate de mediul dvs.
