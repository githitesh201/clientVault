...
1. Într-un terminal, pe găzduitorul unde rulează ClientVault, opriți ClientVault: `docker compose down`.
...
3. Repuneți ClientVault în funcțiune cu comanda `docker compose up -d`.
...
Dacă întâmpinați probleme de runtime după actualizare, s-ar putea să fie nevoie să goliți cache-ul pentru a vă asigura că este sincronizat cu ultimele schimbări. Rulați această comandă în containerul dvs. clientvault-server:
...
Actualizați instanța ClientVault pentru a utiliza imaginea v0.55
...
În containerul dvs. `clientvault-server`, rulați:

```bash
yarn command:prod cache:flush
```

Această problemă este specifică versiunii ClientVault și nu ar trebui să fie necesară pentru actualizările viitoare.
...
Ar trebui să fie mereu în intervalul versiunii ClientVault curente `major.minor`, puteți vizualiza versiunea instanței dvs. în panoul de administrare (la `/settings/admin-panel`, accesibil dacă utilizatorul dvs. are proprietatea `canAccessFullAdminPanel` setată la adevărat în baza de date) sau executând `echo $APP_VERSION` în containerul dvs. `clientvault-server`.

Pentru a remedia o desincronizare a versiunii spațiului de lucru, va trebui să actualizați din versiunea corespunzătoare ClientVault urmând ghidul de actualizare relevant secvențial și așa mai departe până când ajungeți la versiunea dorită.
...
Actualizați instanța ClientVault pentru a utiliza imaginea v0.52
...
Va trebui să actualizați manual versiunea spațiului de lucru la `0.51.0` în baza de date și să actualizați utilizând versiunea clientvault `0.52.11` urmând ghidul de actualizare de mai sus.

### v0.50 la v0.51

Actualizați instanța ClientVault pentru a utiliza imaginea v0.51
...
Actualizați instanța ClientVault pentru a utiliza imaginea v0.50.0
...
Vă rugăm să actualizați docker-compose.yml-ul dvs. local cu [v0.50.0 docker-compose.yml](https://github.com/clientvaulthq/clientvault/blob/v0.50.0/packages/clientvault-docker/docker-compose.yml)

### v0.43.0 la v0.44.0

Actualizați instanța ClientVault pentru a utiliza imaginea v0.44.0
...
Actualizați instanța ClientVault pentru a utiliza imaginea v0.43.0
...
2. Actualizați docker-compose.yml-ul pentru a utiliza imaginea postgres:16 din fișierul [docker-compose.yml](https://raw.githubusercontent.com/clientvaulthq/clientvault/main/packages/clientvault-docker/docker-compose.yml).
...
Actualizați instanța ClientVault pentru a utiliza imaginea v0.42.0
...
Actualizați instanța ClientVault pentru a utiliza imaginea v0.41.0
...
Actualizați instanța ClientVault pentru a utiliza imaginea v0.40.0
...
Actualizați instanța ClientVault pentru a utiliza imaginea v0.35.0
...
Actualizați instanța ClientVault pentru a utiliza imaginea v0.34.0
...
Actualizați instanța ClientVault pentru a utiliza imaginea v0.33.0
...
Începând cu această versiune, imaginea clientvault-postgres pentru DB a devenit depășită și se utilizează clientvault-postgres-spilo.
Dacă doriți să păstrați utilizarea imaginii clientvault-postgres, înlocuiți pur și simplu `clientvaultcrm/clientvault-postgres:${TAG}` cu `clientvaultcrm/clientvault-postgres` în docker-compose.yml.

### v0.31.0 la v0.32.0

Actualizați instanța ClientVault pentru a utiliza imaginea v0.32.0
...
Actualizați instanța ClientVault pentru a utiliza imaginea v0.31.0
...
Actualizați instanța ClientVault pentru a utiliza imaginea v0.30.0

**Schimbare importantă**:
Pentru a îmbunătăți performanțele, ClientVault necesită acum cache de tip Redis configurat. Am actualizat [docker-compose.yml](https://raw.githubusercontent.com/clientvaulthq/clientvault/main/packages/clientvault-docker/docker-compose.yml) pentru a reflecta acest lucru.
...
Actualizați instanța ClientVault pentru a utiliza imaginea v0.24.0
...
Actualizați instanța ClientVault pentru a utiliza imaginea v0.23.0
...
Actualizați instanța ClientVault pentru a utiliza imaginea v0.22.0
