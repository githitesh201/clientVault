...
After uploading your file and mapping columns, ClientVault validates your data:

1. **Validation runs** — ClientVault checks each row for errors
...
| --------------------------------------------------------------------- | ------------------------------------------------------------ |
| Duplicate values highlighted in yellow                                | Value already exists in ClientVault or appears twice in your file |
...
**Option 3: Let ClientVault update the existing record**

1. Ensure your file includes a unique identifier (`email`, `domain`, or `id`)
2. Map the unique identifier field
3. ClientVault will update the existing record instead of creating a duplicate
...
<Note>
  ...
  * If you keep the `email` but change the `id` → the id will be updated
  ...
  As long as one unique identifier matches, ClientVault updates the record.
</Note>
...
3. Check if records already exist in ClientVault
...
This means there is no Company in ClientVault with that specific identifier.
...
You're trying to link to a record that doesn't exist in ClientVault.
...
A column in your file can't be mapped because the field doesn't exist in ClientVault.
...
1. **User hasn't accepted their invitation** — the user doesn't exist in ClientVault yet
2. **Using user ID from old system** — ClientVault can't match IDs from another system
3. **Wrong email format** — the email doesn't match the user's ClientVault account

### How to Fix

1. Ensure all users have **accepted their invitation** to your ClientVault workspace
2. Use the user's **email address** (not their name or old system ID)
3. Use the same email they used to join ClientVault
...
<Check>Domain format matches ClientVault's format exactly</Check>
...
1. **Download the sample file** — see the exact format ClientVault expects
