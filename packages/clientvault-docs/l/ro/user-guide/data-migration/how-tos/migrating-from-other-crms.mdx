...
description: Ghid pas cu pas pentru migrarea datelor din orice CRM către ClientVault.
---

## Prezentare generală

Acest ghid vă ghidează prin procesul de migrare a datelor din orice CRM către ClientVault. Procesul implică auditarea datelor, pregătirea spațiului de lucru ClientVault, exportul din sistemul curent și importul în ClientVault.
...
Creați un document de mapare între CRM-ul curent și ClientVault:

| CRM-ul dvs.               | ClientVault                   |
| ------------------------- | ------------------------ |
...
## Pasul 3: Configurați spațiul dvs. de lucru ClientVault

Înainte de a importa date, pregătiți spațiul dvs. de lucru ClientVault:
...
<Warning>
  **Invitați utilizatorii ÎNAINTE de a importa datele.**
  ...
  Dacă datele dvs. includ referințe la utilizatori (Proprietar cont, Atribuit etc.), acei utilizatori trebuie să existe în ClientVault înainte de import. În caz contrar, acele relații nu pot fi mapate.
</Warning>
...
Deschideți fiecare CSV exportat într-o aplicație de calcul tabelar și pregătiți-l pentru ClientVault.
...
3. Verificați că nu există deja duplicate în ClientVault
...
<Warning>
  **Formatul domeniului este critic.**
  ...
  Folosiți `https://domain.com` (nu `domain.com` sau `www.domain.com`). Acest lucru corespunde formatului ClientVault și previne duplicatele când conectați sincronizarea e-mailului/calendarului.
</Warning>
...
2. Folosiți aceleași adrese de e-mail pe care utilizatorii le-au folosit pentru a se alătura spațiului dvs. de lucru ClientVault
...
## Pasul 6: Importați în ClientVault
...
1. Navigați la obiect în ClientVault
...
* ClientVault va începe să sincronizeze e-mailurile cu înregistrările de contact
...
<AccordionGroup>
  <Accordion title="Pot migra fluxurile de lucru automat?">
    Momentan, nu. Fluxurile de lucru trebuie recreate manual în ClientVault.
  </Accordion>
...
Contactați [contact@clientvault.com](mailto:contact@clientvault.com) sau explorați [Serviciile noastre de implementare](/l/ro/user-guide/getting-started/capabilities/implementation-services).
