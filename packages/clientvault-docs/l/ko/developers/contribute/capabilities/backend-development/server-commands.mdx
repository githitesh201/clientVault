...
이 명령어들은 packages/clientvault-server 폴더에서 실행되어야 합니다.
다른 폴더에서 `npx nx {command} clientvault-server` (또는 `npx nx run clientvault-server:{command}`)를 실행할 수 있습니다.
...
```
npx nx run clientvault-server:lint # lint 오류를 수정하려면 --fix를 사용
```
...
참고: 데이터베이스를 재설정한 뒤 통합 테스트를 실행해야 하는 경우 `npx nx run clientvault-server:test:integration:with-db-reset`을 실행할 수 있습니다.
...
ClientVault는 주로 NestJS를 백엔드에 사용합니다.
