---
title: CSV 파일 준비
description: ClientVault로 가져오기 위한 데이터 형식 지정 단계별 가이드.
---
...
**가장 중요한 단계입니다.** 샘플 파일은 ClientVault가 기대하는 정확한 열 이름과 형식을 보여줍니다.
...
ClientVault는 일부 필드에 대해 고유성을 적용합니다. 중복 항목은 가져오기 오류를 유발합니다.
...
2. 중복 행을 제거하거나 병합하세요
3. ClientVault에 이미 존재하는 중복 항목이 있는지 확인하세요
...
* 고유해야 합니다(파일 또는 ClientVault에 중복이 없어야 함)
...
* **선택 사항**: 제공하지 않으면 ClientVault가 ID를 자동 생성합니다
...
* 상위 레코드는 ClientVault에 이미 존재해야 합니다
