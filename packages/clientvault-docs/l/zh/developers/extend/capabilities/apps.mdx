---
title: ClientVault 应用
description: 以代码的形式构建并管理 ClientVault 自定义项。
---
...
应用使你能够**以代码的形式**构建和管理 ClientVault 自定义项。 无需通过 UI 配置所有内容，你可以在代码中定义数据模型和逻辑函数——从而更快地构建、维护，并推广到多个工作空间。
...
* Node.js 24+ 和 Yarn 4
* 一个 ClientVault 工作空间和一个 API 密钥（在 https://app.clientvault.com/settings/api-webhooks 创建）
...
另请参阅：[create-clientvault-app](https://www.npmjs.com/package/create-clientvault-app) 和 [clientvault-sdk CLI](https://www.npmjs.com/package/clientvault-sdk) 的 CLI 参考页面。

## 项目结构（脚手架生成）

当你运行 `npx create-clientvault-app@latest my-clientvault-app` 时，脚手架将：

* 将一个最小的基础应用复制到 `my-clientvault-app/` 中
* 添加本地 `clientvault-sdk` 依赖和 Yarn 4 配置
* 创建与 `clientvault` CLI 关联的配置文件和脚本
...
* **package.json**：声明应用名称、版本、引擎（Node 24+、Yarn 4），并添加 `clientvault-sdk` 以及一个 `clientvault` 脚本，该脚本会委托给本地的 `clientvault` CLI。 运行 `yarn clientvault help` 以列出所有可用命令。
...
该 SDK 通过在你的 TypeScript 文件中解析 **`export default define<Entity>({...})`** 调用来检测实体。 每种实体类型都有一个从 `clientvault-sdk` 导出的对应辅助函数：
...
* `yarn clientvault app:dev` 将在 `node_modules/clientvault-sdk/generated` 中自动生成一个类型化的 API 客户端（类型化的 ClientVault 客户端 + 工作区类型）。
* `yarn clientvault entity:add` 会在 `src/` 下为你的自定义对象、函数、前端组件或角色添加实体定义文件。

## 身份验证

首次运行 `yarn clientvault auth:login` 时，你将被提示输入：

* API URL（默认为 http://localhost:3000 或你当前的工作空间配置）
* API 密钥

你的凭据按用户存储在 `~/.clientvault/config.json` 中。 你可以维护多个配置文件并在它们之间切换。
...
使用 `yarn clientvault auth:switch` 切换工作空间后，后续所有命令将默认使用该工作空间。 你仍可通过 `--workspace <name>` 临时覆盖。

## 使用 SDK 资源（类型与配置）

clientvault-sdk 提供你在应用中使用的类型化构件和辅助函数。 以下是你最常接触的关键部分。
...
* 你可以使用 `yarn clientvault entity:add` 脚手架创建新对象，它会引导你完成命名、字段和关系。

<Note>
  **基础字段会自动创建。** 当你定义自定义对象时，ClientVault 会自动添加标准字段
...
* 作为 `CLIENTVAULT_API_KEY` 注入的运行时 API 密钥源自该默认函数角色。
* 类型化客户端将受限于该角色授予的权限。
...
* 在 Hello World 应用中查看可运行示例：[`packages/clientvault-apps/hello-world/src/roles/function-role.ts`](https://github.com/clientvaulthq/clientvault/blob/main/packages/clientvault-apps/hello-world/src/roles/function-role.ts)。
...
当你使用 `create-clientvault-app` 脚手架创建一个新应用时，会在 `src/logic-functions/post-install.ts` 为你生成一个安装后函数：
...
当路由触发器调用你的逻辑函数时，它会接收一个遵循 AWS HTTP API v2 格式的 `RoutePayload` 对象。 从 `clientvault-sdk` 导入该类型：
...
* **脚手架生成**：运行 `yarn clientvault entity:add` 并选择添加新逻辑函数的选项。 这将生成一个包含处理程序和配置的入门文件。
* **手动**：创建一个新的 `*.logic-function.ts` 文件，并使用 `defineLogicFunction()`，遵循相同的模式。

### 将逻辑函数标记为工具

逻辑函数可以作为供 AI 智能体和工作流使用的**工具**对外提供。 当函数被标记为工具时，ClientVault 的 AI 功能即可发现它，并可在工作流自动化中将其选作一个步骤。
...
前端组件使你可以构建在 ClientVault 的 UI 中渲染的自定义 React 组件。 使用 `defineFrontComponent()` 以内置校验定义组件：
...
* 前端组件是在 ClientVault 中的隔离上下文中渲染的 React 组件。
* 使用 `*.front-component.tsx` 文件后缀以便自动检测。
* `component` 字段引用你的 React 组件。
* 组件会在 `yarn clientvault app:dev` 期间自动构建并同步。

你可以通过两种方式创建新的前端组件：

* **脚手架生成**：运行 `yarn clientvault entity:add` 并选择添加新前端组件的选项。
* **手动**：创建一个新的 `*.front-component.tsx` 文件，并使用 `defineFrontComponent()`。

### 生成的类型化客户端

类型化客户端由 `yarn clientvault app:dev` 自动生成，并基于你的工作区架构存放在 `node_modules/clientvault-sdk/generated`。 在你的函数中使用它：
...
每当你的对象或字段发生变化时，`yarn clientvault app:dev` 都会自动重新生成该客户端。

#### 逻辑函数中的运行时凭据

当你的函数在 ClientVault 上运行时，平台会在代码执行前将凭据作为环境变量注入：

* `CLIENTVAULT_API_URL`：你的应用所针对的 ClientVault API 的基础 URL。
* `CLIENTVAULT_API_KEY`：作用域限定于你的应用默认函数角色的短期密钥。

备注:

* 你无需向生成的客户端传递 URL 或 API 密钥。 它会在运行时从 process.env 读取 `CLIENTVAULT_API_URL` 和 `CLIENTVAULT_API_KEY`。
...
在[此处](https://github.com/clientvaulthq/clientvault/tree/main/packages/clientvault-apps/hello-world)查看一个最小的端到端示例，展示对象、逻辑函数、前端组件和多种触发器：

## 手动设置（不使用脚手架）

虽然我们建议使用 `create-clientvault-app` 以获得最佳的上手体验，但你也可以手动设置项目。 不要全局安装 CLI。 相反，请将 `clientvault-sdk` 添加为本地依赖，并在你的 package.json 中配置一个脚本：

```bash filename="Terminal"
yarn add -D clientvault-sdk
```

然后添加一个 `clientvault` 脚本：
...
现在你可以通过 `yarn clientvault <command>` 运行所有命令，例如 `yarn clientvault app:dev`、`yarn clientvault help` 等。

## 故障排除

* 身份验证错误：运行 `yarn clientvault auth:login`，并确保你的 API 密钥具有所需权限。
* 无法连接到服务器：请验证 API URL，并确保 ClientVault 服务器可达。
* 类型或客户端缺失/过期：重启 `yarn clientvault app:dev` — 它会自动生成类型化客户端。
* 开发模式未同步：确保 `yarn clientvault app:dev` 正在运行，并且你的环境不会忽略变更。
