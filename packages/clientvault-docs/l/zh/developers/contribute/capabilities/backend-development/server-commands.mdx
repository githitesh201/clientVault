...
这些命令应从 packages/clientvault-server 文件夹中执行。
从其他文件夹，您可以运行 `npx nx {command} clientvault-server`（或 `npx nx run clientvault-server:{command}`）。
...
注意：如果需要在运行集成测试之前重置数据库，可以运行 `npx nx run clientvault-server:test:integration:with-db-reset`。
