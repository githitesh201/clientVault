---
title: تطبيقات ClientVault
description: أنشئ وأدِر تخصيصات ClientVault على هيئة كود.
---
...
تتيح لك التطبيقات إنشاء وإدارة تخصيصات ClientVault **ككود**. بدلًا من تكوين كل شيء عبر واجهة المستخدم، تُعرِّف نموذج بياناتك ووظائف المنطق في الكود — مما يجعل البناء والصيانة والنشر إلى مساحات عمل متعددة أسرع.
...
* Node.js 24+ وYarn 4
* مساحة عمل ClientVault ومفتاح واجهة برمجة التطبيقات (أنشئ واحدًا على https://app.clientvault.com/settings/api-webhooks)
...
راجع أيضًا: صفحات مرجع CLI لـ [create-clientvault-app](https://www.npmjs.com/package/create-clientvault-app) و[clientvault-sdk CLI](https://www.npmjs.com/package/clientvault-sdk).

## هيكل المشروع (مُنشأ بالقالب)

عند تشغيل `npx create-clientvault-app@latest my-clientvault-app`، يقوم المُهيئ بما يلي:

* ينسخ تطبيقًا أساسيًا مصغّرًا إلى `my-clientvault-app/`
* يضيف اعتمادًا محليًا `clientvault-sdk` وتهيئة Yarn 4
* ينشئ ملفات ضبط ونصوصًا مرتبطة بـ `clientvault` CLI
...
* **package.json**: يصرّح باسم التطبيق والإصدار والمحرّكات (Node 24+، Yarn 4)، ويضيف `clientvault-sdk` بالإضافة إلى نص برمجي `clientvault` يفوِّض إلى `clientvault` CLI المحلي. شغِّل `yarn clientvault help` لعرض جميع الأوامر المتاحة.
...
يكتشف SDK الكيانات عبر تحليل ملفات TypeScript الخاصة بك بحثًا عن استدعاءات **`export default define<Entity>({...})`**. يحتوي كل نوع كيان على دالة مساعدة مقابلة يتم تصديرها من `clientvault-sdk`:
...
* `yarn clientvault app:dev` سيولّد تلقائيًا عميل API مضبوط الأنواع في `node_modules/clientvault-sdk/generated` (عميل ClientVault مضبوط الأنواع + أنواع مساحة العمل).
* `yarn clientvault entity:add` سيضيف ملفات تعريف الكيانات تحت `src/` لكائناتك المخصصة أو الوظائف أو المكونات الواجهية أو الأدوار.

## المصادقة

في المرة الأولى التي تشغّل فيها `yarn clientvault auth:login`، سيُطلب منك إدخال:

* عنوان URL لواجهة برمجة التطبيقات (الافتراضي http://localhost:3000 أو ملف تعريف مساحة العمل الحالية لديك)
* مفتاح واجهة برمجة التطبيقات

تُخزَّن بيانات اعتمادك لكل مستخدم في `~/.clientvault/config.json`. You can maintain multiple profiles and switch between them.
...
بمجرد أن تقوم بالتبديل بين مساحات العمل باستخدام `yarn clientvault auth:switch`، ستستخدم جميع الأوامر اللاحقة تلك المساحة افتراضيًا. You can still override it temporarily with `--workspace <name>`.

## استخدم موارد SDK (الأنواع والتكوين)

يوفّر clientvault-sdk كتلَ بناءٍ مضبوطة الأنواع ودوال مساعدة تستخدمها داخل تطبيقك. فيما يلي الأجزاء الأساسية التي ستتعامل معها غالبًا.
...
* يمكنك إنشاء كائنات جديدة باستخدام `yarn clientvault entity:add`، والذي يرشدك خلال التسمية والحقول والعلاقات.

<Note>
  **يتم إنشاء الحقول الأساسية تلقائيًا.** عند تعريف كائن مخصص، يضيف ClientVault تلقائيًا حقولًا قياسية
...
* مفتاح واجهة البرمجة في وقت التشغيل المحقون باسم `CLIENTVAULT_API_KEY` مستمد من دور الوظيفة الافتراضي هذا.
...
* اطّلع على مثال عملي في تطبيق Hello World: [`packages/clientvault-apps/hello-world/src/roles/function-role.ts`](https://github.com/clientvaulthq/clientvault/blob/main/packages/clientvault-apps/hello-world/src/roles/function-role.ts).
...
عند إنشاء هيكل تطبيق جديد باستخدام `create-clientvault-app`، يتم إنشاء دالة ما بعد التثبيت لك في `src/logic-functions/post-install.ts`:
...
عندما يستدعي مشغّل المسار وظيفتك المنطقية، يتلقى كائنًا من النوع `RoutePayload` يتبع تنسيق AWS HTTP API v2. استورد النوع من `clientvault-sdk`:
...
* **مُنشأ بالقالب**: شغّل `yarn clientvault entity:add` واختر خيار إضافة وظيفة منطقية جديدة. يُولّد هذا ملفًا مبدئيًا مع معالج وتكوين.
* **يدوي**: أنشئ ملفًا جديدًا `*.logic-function.ts` واستخدم `defineLogicFunction()` مع اتباع النمط نفسه.

### تمييز دالة منطقية كأداة

يمكن إتاحة الدوال المنطقية بوصفها **أدوات** لوكلاء الذكاء الاصطناعي وسير العمل. عندما يتم تمييز دالة كأداة، تصبح قابلة للاكتشاف بواسطة ميزات الذكاء الاصطناعي الخاصة بـ ClientVault ويمكن اختيارها كخطوة في أتمتة سير العمل.
...
تتيح لك المكوّنات الأمامية إنشاء مكوّنات React مخصّصة تُعرَض داخل واجهة مستخدم ClientVault. استخدم `defineFrontComponent()` لتعريف مكوّنات مع تحقّق مدمج:
...
* المكوّنات الأمامية هي مكوّنات React تُعرَض ضمن سياقات معزولة داخل ClientVault.
* استخدم لاحقة الملف `*.front-component.tsx` للاكتشاف التلقائي.
* يشير الحقل `component` إلى مكوّن React الخاص بك.
* يتم بناء المكوّنات ومزامنتها تلقائيًا أثناء `yarn clientvault app:dev`.

يمكنك إنشاء مكوّنات أمامية جديدة بطريقتين:

* **مُنشأ بالقالب**: شغّل `yarn clientvault entity:add` واختر خيار إضافة مكوّن أمامي جديد.
* **يدوي**: أنشئ ملفًا جديدًا `*.front-component.tsx` واستخدم `defineFrontComponent()`.

### عميل مُولَّد مضبوط الأنواع

يُولَّد العميل مضبوط الأنواع تلقائيًا بواسطة `yarn clientvault app:dev` ويُخزَّن في `node_modules/clientvault-sdk/generated` استنادًا إلى مخطط مساحة العمل لديك. استخدمه في وظائفك:
...
يُعاد توليد العميل تلقائيًا بواسطة `yarn clientvault app:dev` كلما تغيّرت كائناتك أو حقولك.

#### بيانات الاعتماد وقت التشغيل في الوظائف المنطقية

عندما تعمل وظيفتك على ClientVault، يقوم النظام الأساسي بحقن بيانات الاعتماد كمتغيرات بيئة قبل تنفيذ كودك:

* `CLIENTVAULT_API_URL`: عنوان URL الأساسي لواجهة ClientVault البرمجية التي يستهدفها تطبيقك.
* `CLIENTVAULT_API_KEY`: مفتاح قصير العمر ذو نطاق يقتصر على الدور الافتراضي لوظيفة تطبيقك.

الملاحظات:

* لا تحتاج إلى تمرير عنوان URL أو مفتاح واجهة برمجة التطبيقات إلى العميل المُولَّد. يقوم بقراءة `CLIENTVAULT_API_URL` و`CLIENTVAULT_API_KEY` من process.env وقت التشغيل.
...
استكشف مثالًا بسيطًا شاملًا من البداية إلى النهاية يوضح الكائنات والوظائف المنطقية والمكوّنات الأمامية ومشغّلات متعددة [هنا](https://github.com/clientvaulthq/clientvault/tree/main/packages/clientvault-apps/hello-world):

## إعداد يدوي (بدون المهيئ)

بينما نوصي باستخدام `create-clientvault-app` للحصول على أفضل تجربة للبدء، يمكنك أيضًا إعداد مشروع يدويًا. لا تثبّت CLI عالميًا. بدل ذلك، أضف `clientvault-sdk` كاعتماد محلي واربط سكربتًا واحدًا في ملف package.json لديك:

```bash filename="Terminal"
yarn add -D clientvault-sdk
```

ثم أضف سكربتًا باسم `clientvault`:
...
الآن يمكنك تشغيل جميع الأوامر عبر `yarn clientvault <command>`، مثلًا: `yarn clientvault app:dev`، `yarn clientvault help`، إلخ.

## استكشاف الأخطاء وإصلاحها

* أخطاء المصادقة: شغّل `yarn clientvault auth:login` وتأكد من أن مفتاح واجهة برمجة التطبيقات لديك يمتلك الأذونات المطلوبة.
* يتعذّر الاتصال بالخادم: تحقق من عنوان URL لواجهة البرمجة وأن خادم ClientVault قابل للوصول.
* الأنواع أو العميل مفقود/قديم: أعد تشغيل `yarn clientvault app:dev` — فهو ينشئ العميل مضبوط الأنواع بشكل تلقائي.
* وضع التطوير لا يزامن: تأكد من أن `yarn clientvault app:dev` قيد التشغيل وأن التغييرات ليست متجاهلة من بيئتك.
