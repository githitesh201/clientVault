...
يوفر هذا الدليل تعليمات خطوة بخطوة لتثبيت وتكوين تطبيق "ClientVault" باستخدام Docker Compose. الهدف هو جعل العملية مباشرة ومنع المشاكل الشائعة التي يمكن أن تؤدي إلى تعطيل إعدادك.
...
قم بتثبيت أحدث إصدار مستقر من ClientVault باستخدام أمر واحد:
...
إذا كنت تستضيف clientvaultCRM على جهازك، فافتح المتصفح وانتقل إلى [http://localhost:3000](http://localhost:3000).
...
### جعل ClientVault متاحاً للوصول الخارجي

افتراضياً، يعمل ClientVault على `localhost` على المنفذ `3000`. للوصول إليه عبر نطاق خارجي أو عنوان IP، تحتاج إلى تكوين `SERVER_URL` في ملف `.env`.
...
SSL (HTTPS) مطلوب لعمل ميزات معينة في المتصفح بشكل صحيح. بينما قد تعمل هذه الميزات أثناء التطوير المحلي (بما أن المتصفحات تعامل localhost بشكل مختلف)، هناك حاجة إلى إعداد SSL صحيح عند استضافة ClientVault على نطاق عادي.
...
نوصي بشدة بإعداد ClientVault خلف وكيل عكسي مع إنهاء SSL للأمان والوظيفية المثلى.
