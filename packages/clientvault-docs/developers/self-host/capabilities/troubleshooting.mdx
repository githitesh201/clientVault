...
If you have an existing ClientVault instance with production data, **DO NOT** follow these steps as they will permanently delete your database!

While installing ClientVault for the first time, you might want to change the default database password.
...
During ClientVault installation, you need to provision your postgres database with the right schemas, extensions, and users.
...
#### Missing clientvault-x package

Make sure to run yarn in the root directory and then run `npx nx server:dev clientvault-server`. If this still doesn't work try building the missing package manually.
...
#### While running `npx nx start` or `npx nx start clientvault-front`, Out of memory error is thrown

In `packages/clientvault-front/.env` uncomment `VITE_DISABLE_TYPESCRIPT_CHECKER=true` to disable background checks thus reducing amount of needed RAM.

**If it does not work:**
Run only the services you need, instead of `npx nx start`. For instance, if you work on the server, run only `npx nx worker clientvault-server`
...
**If it does not work:**
Investigate which processes are taking you most of your machine RAM. At ClientVault, we noticed that some VScode extensions were taking a lot of RAM so we temporarily disable them.
...
- Open your ClientVault app, navigate to `/settings`, and enable the **Advanced** toggle at the bottom left of the screen.
- Create a new webhook.
- Paste **Your Unique Webhook URL** in the **Endpoint Url** field in ClientVault. Set the **Filters** to `Companies` and `Created`.
<div style={{textAlign: 'center'}}>
...
Comment out checker plugin in `packages/clientvault-ui/vite-config.ts` like in example below
```
plugins: [
...
If you're running ClientVault behind a reverse proxy and experiencing connection issues:
...
2. **Check Reverse Proxy Settings:**
    ...
    - Confirm that your reverse proxy is correctly forwarding requests to the ClientVault server.
    - Ensure headers like `X-Forwarded-For` and `X-Forwarded-Proto` are properly set.

3. **Restart Services:**

    After making changes, restart both the reverse proxy and ClientVault containers.
...
- Community Support:
    ...
    Reach out to the [ClientVault community](https://github.com/clientvaulthq/clientvault/issues) or [support channels](https://discord.gg/cx5n4Jzs57) for assistance.
