...
1. In a terminal, on the host where ClientVault is running, turn off ClientVault: `docker compose down`
...
3. Bring ClientVault back online with `docker compose up -d`
...
If you encounter any runtime issues after upgrading, you may need to flush your cache to ensure it's synchronized with the latest changes. Run this command in your clientvault-server container:
...
Upgrade your ClientVault instance to use v0.55 image
...
In your `clientvault-server` container, run:
```bash
yarn command:prod cache:flush
```

This issue is specific to this ClientVault version and should not be required for future upgrades.
...
It should always be in the range of your current ClientVault's instance `major.minor` version, you can view your instance version in the admin panel (at `/settings/admin-panel`, accessible if your user has `canAccessFullAdminPanel` property set to true in the database) or by running `echo $APP_VERSION` in your `clientvault-server` container.


To fix a desynchronized workspace version, you will have to upgrade from the corresponding clientvault's version following related upgrade guide sequentially and so on until it reaches desired version.
...
Upgrade your ClientVault instance to use v0.52 image
...
You will have to manually update your workspace version to `0.51.0` in database and upgrade using clientvault version `0.52.11` following its just above upgrade guide.


### v0.50 to v0.51

Upgrade your ClientVault instance to use v0.51 image
...
Upgrade your ClientVault instance to use v0.50.0 image
...
Please update your local `docker-compose.yml` with [v0.50.0 docker-compose.yml](https://github.com/clientvaulthq/clientvault/blob/v0.50.0/packages/clientvault-docker/docker-compose.yml)

### v0.43.0 to v0.44.0

Upgrade your ClientVault instance to use v0.44.0 image
...
Upgrade your ClientVault instance to use v0.43.0 image
...
2. Upgrade your docker-compose.yml to use postgres:16 image as in the [docker-compose.yml](https://raw.githubusercontent.com/clientvaulthq/clientvault/main/packages/clientvault-docker/docker-compose.yml) file.
...
Upgrade your ClientVault instance to use v0.42.0 image
...
Upgrade your ClientVault instance to use v0.41.0 image
...
Upgrade your ClientVault instance to use v0.40.0 image
...
Upgrade your ClientVault instance to use v0.35.0 image
...
Upgrade your ClientVault instance to use v0.34.0 image
...
Upgrade your ClientVault instance to use v0.33.0 image
...
Starting from this version, clientvault-postgres image for DB became deprecated and clientvault-postgres-spilo is used instead.
If you want to keep using clientvault-postgres image, simply replace `clientvaultcrm/clientvault-postgres:${TAG}` with `clientvaultcrm/clientvault-postgres` in docker-compose.yml.

### v0.31.0 to v0.32.0

Upgrade your ClientVault instance to use v0.32.0 image
...
Upgrade your ClientVault instance to use v0.31.0 image
...
Upgrade your ClientVault instance to use v0.30.0 image

**Breaking change**:
To enhance performances, ClientVault now requires redis cache to be configured. We have updated our [docker-compose.yml](https://raw.githubusercontent.com/clientvaulthq/clientvault/main/packages/clientvault-docker/docker-compose.yml) to reflect this.
...
Upgrade your ClientVault instance to use v0.24.0 image
...
Upgrade your ClientVault instance to use v0.23.0 image
...
Upgrade your ClientVault instance to use v0.22.0 image
