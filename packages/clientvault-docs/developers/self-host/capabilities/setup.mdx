...
<Warning>
**First time installing?** Follow the [Docker Compose installation guide](/developers/self-host/capabilities/docker-compose) to get ClientVault running, then return here for configuration.
</Warning>

ClientVault offers **two configuration modes** to suit different deployment needs:
...
1. Access your ClientVault instance (usually `http://localhost:3000`)
...
[Complete technical reference →](https://github.com/clientvaulthq/clientvault/blob/main/packages/clientvault-server/src/engine/core-modules/clientvault-config/config-variables.ts)
</Warning>
...
By default, ClientVault runs in **single-workspace mode** — ideal for most self-hosted deployments where you need one CRM instance for your organization.
...
- One workspace per ClientVault instance
...
Enable multi-workspace mode for SaaS-like deployments where multiple independent teams need their own workspaces on the same ClientVault instance.
...
[See relevant source code](https://github.com/clientvaulthq/clientvault/blob/main/packages/clientvault-server/src/engine/core-modules/auth/utils/get-google-apis-oauth-scopes.ts#L4-L10)
...
<Warning>
Users must have a [Microsoft 365 Licence](https://admin.microsoft.com/Adminportal/Home) to be able to use the Calendar and Messaging API. They will not be able to sync their account on ClientVault without one.
</Warning>
...
### Configure scopes
[See relevant source code](https://github.com/clientvaulthq/clientvault/blob/main/packages/clientvault-server/src/engine/core-modules/auth/utils/get-microsoft-apis-oauth-scopes.ts#L2-L9)
- 'openid'
- 'email'
- 'profile'
- 'offline_access'
...
ClientVault supports logic functions for workflows and custom logic. The execution environment is configured via the `SERVERLESS_TYPE` environment variable.
...
<Note>
When using `SERVERLESS_TYPE=DISABLED`, any attempt to execute a logic function will return an error. This is useful if you want to run ClientVault without logic function capabilities.
</Note>
