...
This guide provides step-by-step instructions to install and configure the ClientVault application using Docker Compose. The aim is to make the process straightforward and prevent common pitfalls that could break your setup.
...
Install the latest stable version of ClientVault with a single command:
```bash
...
If you host clientvaultCRM on your own computer, open your browser and navigate to [http://localhost:3000](http://localhost:3000).
...
### Expose ClientVault to External Access

By default, ClientVault runs on `localhost` at port `3000`. To access it via an external domain or IP address, you need to configure the `SERVER_URL` in your `.env` file.
...
SSL (HTTPS) is required for certain browser features to work properly. While these features might work during local development (as browsers treat localhost differently), a proper SSL setup is needed when hosting ClientVault on a regular domain.
...
We strongly recommend setting up ClientVault behind a reverse proxy with SSL termination for optimal security and functionality.
